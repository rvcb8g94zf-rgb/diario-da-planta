...
function GrowersPage({user,onBack,onViewProfile}){
  const [growers,setGrowers]=useState([]);const [loading,setLoading]=useState(true);const [search,setSearch]=useState("");
  useEffect(()=>{(async()=>{
    try{
      const profiles=await sb.from("profiles").select("*",`&order=created_at.desc`);
      const diaries=await sb.from("diaries").select("user_id",`&hidden=eq.false`);
      const countMap={};diaries.forEach(d=>{countMap[d.user_id]=(countMap[d.user_id]||0)+1;});
      setGrowers(profiles.map(p=>({...p,diaryCount:countMap[p.id]||0,level:getUserLevel(countMap[p.id]||0)})));
    }catch{}setLoading(false);
  })();},[]);
  const filtered=search?growers.filter(g=>(g.username||"").toLowerCase().includes(search.toLowerCase())):growers;
  return(
    <div style={{maxWidth:"800px",margin:"0 auto",padding:"24px"}}>
      <div style={{marginBottom:"24px"}}>
        <h1 style={{fontFamily:F.sans,fontSize:"28px",fontWeight:"800",margin:"0 0 4px"}}>👥 Cultivadores</h1>
        <p style={{fontFamily:F.sans,fontSize:"14px",color:C.muted,margin:"0 0 16px"}}>Conheça a comunidade</p>
        <input style={{...baseInput,marginBottom:"0"}} value={search} onChange={e=>setSearch(e.target.value)} placeholder="🔍 Buscar cultivador..."/>
      </div>
      {loading?<div style={{textAlign:"center",padding:"60px",color:C.dim}}>Carregando...</div>:
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(180px,1fr))",gap:"16px"}}>
        {filtered.map(g=>(
          <div key={g.id} onClick={()=>onViewProfile?.(g)} style={{background:C.cardBg,borderRadius:"16px",border:`1px solid ${C.border}`,padding:"20px 16px",textAlign:"center",cursor:"pointer",transition:"all 0.2s",boxShadow:"0 2px 8px rgba(0,0,0,0.04)"}}>
            ...
            <div style={{display:"flex",justifyContent:"center",gap:"12px",fontFamily:F.sans,fontSize:"11px",color:C.dim}}>
              <span>📔 {g.diaryCount} diários</span>
            </div>
...
function NavBar({ user, page, setPage, setShowCreate, myDiaries, onLogout, onNavigate, lang, setLang, unreadNotifs, unreadMsgs, notifs, onMarkNotifsRead, dark, onToggleDark }) {
  ...
  const sidebarItems=[
    ...
    {icon:"🐛",label:"Pragas e Fungos",action:()=>nav("pragas"),active:page==="pragas"},
...
function AppInner() {
  ...
  const renderPage=()=>{
    ...
    if(page==="cultivadores") return <GrowersPage user={user} onBack={()=>setPage("home")} onViewProfile={async(g)=>{try{const diaries=await sb.from("diaries").select("*",`&user_id=eq.${g.id}&hidden=eq.false&order=created_at.desc`);setPublicProfile({user:{...g,username:g.username,avatar:g.avatar,avatarImg:g.avatar_url,bio:g.bio,city:g.city,createdAt:g.created_at},diaries:diaries.map(d=>({...d,author:g.username,avatar:g.avatar,avatarImg:g.avatar_url}))});}catch{}}}/>;
