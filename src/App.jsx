...
function ForumPage({user,onBack}){
  ...
  const renderToolbar=(setter)=>(
    <div style={{display:"flex",gap:"4px",flexWrap:"wrap",marginBottom:"8px",padding:"6px 8px",background:C.surface2,borderRadius:"8px"}}>
      ...
      <label style={{...tbBtnSt,border:"1px solid "+C.accent,background:C.accentBg,color:C.accent}}>📷<input type="file" accept="image/*,video/*" style={{display:"none"}} onChange={e=>handleMediaUpload(e,setter)}/></label>
  ...
  const av=(p)=><div style={{width:"32px",height:"32px",borderRadius:"50%",background:C.surface2,overflow:"hidden",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"16px",border:"1px solid "+C.border,flexShrink:0}}>{p?.avatar_url?<img src={p.avatar_url} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:(p?.avatar||"🌱")}</div>;
