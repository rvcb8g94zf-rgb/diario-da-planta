        ...
        {entries.length>0?(
          <div style={{background:C.cardBg,borderRadius:"16px",border:`1px solid ${C.border}`,overflow:"hidden",marginBottom:"20px"}}>
            ...
            <div style={{position:"relative",background:"#000",minHeight:"300px",display:"flex",alignItems:"center",justifyContent:"center"}}>
              <img src={currentEntry?.photo_url} alt="" onClick={()=>onViewImage?.(currentEntry?.photo_url)} style={{maxWidth:"100%",maxHeight:"500px",objectFit:"contain",cursor:"pointer"}}/>
              {/* Arrows */}
              {entries.length>1&&<>
            ...
            <div style={{padding:"16px 20px"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"8px"}}>
                <div style={{display:"flex",alignItems:"center",gap:"8px"}}>
                  <div style={{width:"28px",height:"28px",borderRadius:"50%",background:C.surface2,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"14px",overflow:"hidden",border:`1px solid ${C.border}`}}>{currentEntry?.profiles?.avatar_url?<img src={currentEntry.profiles.avatar_url} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:(currentEntry?.profiles?.avatar||"🌱")}</div>
        ...
        {(phase==="voting"||phase==="ended")&&entries.length>0&&<div style={{background:C.cardBg,borderRadius:"16px",border:`1px solid ${C.border}`,padding:"16px 20px"}}>
          ...
          {entries.sort((a,b)=>(b.votes_count||0)-(a.votes_count||0)).map((e,i)=>(
            <div key={e.id} onClick={()=>setCarouselIdx(entries.indexOf(e)>=0?entries.indexOf(e):i)} style={{display:"flex",alignItems:"center",gap:"10px",padding:"8px 0",borderBottom:i<entries.length-1?`1px solid ${C.border22}`:"none",cursor:"pointer"}}>
              ...
              <img src={e.photo_url} alt="" style={{width:"40px",height:"40px",borderRadius:"8px",objectFit:"cover"}}/>
              <div style={{flex:1,minWidth:0}}>
...
function AppInner() {
    ...
    if(convIds.length>0){
      ...
      for(const cr of convRows){
        ...
        convList.push({
          ...
          avatarImg:!c.is_group&&other?.profiles?.avatar_url||null,
          isGroup:c.is_group,
          ...
          messages:sortedMsgs.map(m=>({id:m.id,from:m.sender_id===user.id?user.email:(m.profiles?.username||m.sender_id),text:m.text||"",media:m.media_url?{type:m.media_type||"image",data:m.media_url}:null,time:new Date(m.created_at).getTime(),forwarded:m.forwarded,isSystem:!!m.is_system,reportNumber:m.report_number||null})),
        });
      }
      setMsgs(convList);
    }
  ...
  const loadOlderMessages=async(convId)=>{
    ...
    try{
      ...
      const mapped=olderRows.reverse().map(m=>({id:m.id,from:m.sender_id===user.id?user.email:(m.profiles?.username||m.sender_id),text:m.text||"",media:m.media_url?{type:m.media_type||"image",data:m.media_url}:null,time:new Date(m.created_at).getTime(),forwarded:m.forwarded,isSystem:!!m.is_system,reportNumber:m.report_number||null}));
  ...
  useEffect(()=>{if(!user)return;(async()=>{
    try{
      ...
      const mapped=(rows||[]).map(d=>({
        id:d.id,name:d.name,strain:d.strain,strains:d.strains||[],
        author:d.profiles?.username||"",authorId:d.user_id,
        avatar:d.profiles?.avatar||"🌱",avatarImg:d.profiles?.avatar_url||null,
        phase:d.phase,week:d.current_week,env:d.environment,light:d.lighting,watts:d.watts,substrate:d.substrate,
        watering:d.watering,germination:d.germination,techniques:d.techniques||[],numPlants:d.num_plants,tags:d.tags||[],
  ...
  useEffect(()=>{if(!user)return;(async()=>{
    try{const bk=await sb.from("blocked_users").select("blocked_id,profiles:blocked_id(username,avatar,avatar_url)",`&blocker_id=eq.${user.id}`);setBlockedUsers(bk.map(b=>({id:b.blocked_id,username:b.profiles?.username||"",avatar:b.profiles?.avatar||"🌱",avatarImg:b.profiles?.avatar_url||null})));}catch{}
  })();},[user]);
  ...
  const doBlockUser=async(blockedId)=>{
    ...
    try{
      ...
      setBlockedUsers(prev=>[...prev,{id:blockedId,username:p?.[0]?.username||"",avatar:p?.[0]?.avatar||"🌱",avatarImg:p?.[0]?.avatar_url||null}]);
