...
const tryRefreshToken = async () => {
  ...
  _refreshing = (async () => {
    try {
      ...
      if(res.ok){
        const data = await res.json();
        if(data.access_token){ setTokens(data.access_token, data.refresh_token||_refreshToken); return true; }
      }
    } catch {}
    return false;
  })();
    ...
    <div style={{maxWidth:"700px",margin:"0 auto",padding:"24px"}}>
      ...
      {showNewGroup&&<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.4)",backdropFilter:"blur(8px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:300,padding:"20px"}} onClick={()=>setShowNewGroup(false)}>
        <div style={{...cardBase,maxWidth:"440px"}} onClick={e=>e.stopPropagation()}>
          ...
          <div style={{display:"flex",gap:"12px"}}><button style={{...btnSecondary,width:"auto",padding:"10px 20px"}} onClick={()=>setShowNewGroup(false)}>Cancelar</button><button style={{...btnPrimary,opacity:(!groupName.trim()||groupMemberChips.length===0)?0.4:1}} disabled={!groupName.trim()||groupMemberChips.length===0} onClick={handleCreateGroup}>Criar Grupo</button></div>
        </div>
      </div>}
...
function AdminPanel({ user, onBack, onNewPost }) {
  ...
  const doSaveEdit=async()=>{
    ...
    if(editForm.email&&editForm.email!==originalEmail){
      ...
      }catch(e){console.error("Email update failed:",e);showToast("Erro ao atualizar email: "+(e.message||""));}
    }else{
  ...
  const doSendWarning=async()=>{
    ...
    try{
      ...
      let convId=null;
      try{
        const myConvs=await sb.from("conversation_members").select("conversation_id,conversations(is_group)",`&user_id=eq.${user.id}`);
        const candidateIds=(myConvs||[]).filter(c=>c.conversations&&!c.conversations.is_group).map(c=>c.conversation_id);
        if(candidateIds.length>0){
          const targetConvs=await sb.from("conversation_members").select("conversation_id",`&user_id=eq.${warnTarget}&conversation_id=in.(${candidateIds.join(",")})`);
          if((targetConvs||[]).length>0)convId=targetConvs[0].conversation_id;
        }
        if(!convId){
          ...
          if(conv?.id){
            ...
            await sb.from("conversation_members").insert([
              {conversation_id:convId,user_id:user.id,read_at:new Date().toISOString()},
              {conversation_id:convId,user_id:warnTarget,read_at:null},
            ]);
          }
        }
        if(convId){
      ...
      }catch(e){console.error("DM warning failed:",e);}
    }catch{}
...
function ForumPage({user,onBack,onReport,pendingThreadId,onThreadOpened}){
  ...
  const postReply=async()=>{
    ...
    try{
      ...
      }catch(e){console.error("Forum notif failed:",e);}
...
function RecoveryForm({ dark, onDone }) {
  ...
  const doReset=async()=>{
    ...
    try{
      ...
      else{const d=await res.json().catch(()=>({}));setMsg(d.msg||d.error_description||"Erro ao alterar senha. O link pode ter expirado — solicite um novo.");}
...
function AppInner() {
  ...
  const newDM=async(username,firstMsg,media)=>{
    ...
    // Check existing conversation by username
    ...
    try{
      ...
      const conv=await sb.from("conversations").insert({is_group:false});
      if(!conv?.id){alert("Erro ao criar conversa. Tente novamente.");return;}
      await sb.from("conversation_members").insert([
        {conversation_id:conv.id,user_id:user.id,read_at:new Date().toISOString()},
        {conversation_id:conv.id,user_id:target.id,read_at:null},
      ]);
      ...
      // Insert first message with text and/or media
      if(cleanMsg){
        await sb.from("messages").insert({conversation_id:conv.id,sender_id:user.id,text:cleanMsg});
      }
      if(mediaUrl){
        await sb.from("messages").insert({conversation_id:conv.id,sender_id:user.id,text:"",media_url:mediaUrl,media_type:mediaType});
      }
  ...
  useEffect(()=>{if(!user)return;(async()=>{
    ...
    }catch(e){console.error("Load public diaries:",e);}
  })();},[user]);
  ...
  const doLike=async(id)=>{
    if(!user)return;
    if(!actionRateLimit("like",30,60000)){alert("Você está curtindo muito rápido. Aguarde um momento.");return;}
  ...
  const doReportGeneric=async({targetType,targetId,reason,targetName})=>{
    if(!user)return null;
    if(!actionRateLimit("report",5,300000)){alert("Você já fez muitas denúncias recentemente. Aguarde alguns minutos.");return null;}
    ...
    try{
      ...
      return num||null;
    }catch(e){console.error("Report failed:",e);alert("Erro ao registrar denúncia. Tente novamente.");return null;}
  };
  ...
  const doAddComment=async(diaryId,text,parentId)=>{
    ...
    if(!actionRateLimit("comment",10,60000)){alert("Você está comentando muito rápido. Aguarde um momento.");return;}
