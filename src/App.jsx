...
// ─── Supabase Config (env vars with validated fallback) ───
...
const SENTRY_DSN = (_dsn && _dsn.includes("sentry.io")) ? _dsn : "https://3cdfd5652417b5cabaddf0677781d77c@o4511092002324480.ingest.us.sentry.io/4511092012548096";
...
const setTokens = (access, refresh) => { _accessToken = access; _refreshToken = refresh; try { if(access) localStorage.setItem("sb-token", JSON.stringify({access,refresh})); else localStorage.removeItem("sb-token"); } catch{} if(access && navigator.serviceWorker?.controller){ navigator.serviceWorker.controller.postMessage({type:"SET_AUTH_TOKEN",token:access}); } };
const loadTokens = () => { try { const raw = localStorage.getItem("sb-token"); if(raw){ const t=JSON.parse(raw); _accessToken=t.access; _refreshToken=t.refresh; if(t.access && navigator.serviceWorker?.controller){ navigator.serviceWorker.controller.postMessage({type:"SET_AUTH_TOKEN",token:t.access}); } } } catch{} };
...
const tryRefreshToken = async () => {
  if(!_refreshToken) { fireSessionExpired(); return false; }
    ...
    try {
      ...
      if(res.ok){
        ...
        if(data.access_token){
          ...
          _sessionExpiredFired = false; // reset so future expirations can fire again
          return true;
        }
      } else {
        // Refresh token itself is invalid/expired (400/401 with invalid_grant)
        // Clear everything and notify the app to redirect to login
        try {
          const err = await res.json();
          if(res.status === 400 || res.status === 401 || err.error === "invalid_grant"){
            fireSessionExpired();
          }
        } catch {
          if(res.status === 400 || res.status === 401) fireSessionExpired();
        }
      }
    } catch {}
    return false;
...
const fetchWithRefresh = async (url, init) => {
  ...
  if(res.status === 401){
    if(_refreshToken){
      ...
      if(refreshed){
        ...
        const newInit = {...init, headers: {...init.headers, "Authorization": `Bearer ${_accessToken}`}};
        res = await fetch(url, newInit);
      }
    } else if(_accessToken) {
      // We had a token but it's invalid and no refresh token — session is dead
      fireSessionExpired();
    }
  }
  return res;
...
const sb = {
  from: (table) => ({
    select: async (cols="*", filters="") => {
      const res = await fetchWithRefresh(`${SB_URL}/rest/v1/${table}?select=${encodeURIComponent(cols)}${filters}`, { headers:sbHeaders() });
      return res.ok ? res.json() : [];
    },
    selectOne: async (cols="*", filters="") => {
      const res = await fetchWithRefresh(`${SB_URL}/rest/v1/${table}?select=${encodeURIComponent(cols)}${filters}`, { headers:{...sbHeaders(),"Accept":"application/vnd.pgrst.object+json"} });
      return res.ok ? res.json() : null;
    },
    insert: async (data) => {
      // Normalize: ensure all objects in a batch have the same keys (PostgREST requirement)
      let payload = Array.isArray(data) ? data : [data];
      if(payload.length > 1){
        const allKeys = new Set();
        payload.forEach(row => Object.keys(row||{}).forEach(k => allKeys.add(k)));
        payload = payload.map(row => {
          const normalized = {};
          allKeys.forEach(k => { normalized[k] = row[k] !== undefined ? row[k] : null; });
          return normalized;
        });
      }
      const res = await fetchWithRefresh(`${SB_URL}/rest/v1/${table}`, { method:"POST", headers:sbHeaders(), body:JSON.stringify(payload) });
    ...
    update: async (data, filters) => {
      const res = await fetchWithRefresh(`${SB_URL}/rest/v1/${table}?${filters}`, { method:"PATCH", headers:sbHeaders(), body:JSON.stringify(data) });
      return res.ok ? res.json() : [];
    },
    delete: async (filters) => {
      const res = await fetchWithRefresh(`${SB_URL}/rest/v1/${table}?${filters}`, { method:"DELETE", headers:sbHeaders() });
      return res.ok;
    },
    upsert: async (data) => {
      const res = await fetchWithRefresh(`${SB_URL}/rest/v1/${table}`, { method:"POST", headers:{...sbHeaders(),"Prefer":"return=representation,resolution=merge-duplicates"}, body:JSON.stringify(Array.isArray(data)?data:[data]) });
      return res.ok ? res.json() : [];
    },
  }),
  rpc: async (fn, params={}) => {
    const res = await fetchWithRefresh(`${SB_URL}/rest/v1/rpc/${fn}`, { method:"POST", headers:sbHeaders(), body:JSON.stringify(params) });
    return res.ok ? res.json() : null;
  },
};
...
const sbStorage = {
  upload: async (path, file) => {
    ...
    if(res.ok) {
      ...
      // Pre-sign immediately so getUrl() works right after upload
      await sbStorage.getSignedUrl(path);
    }
    return res.ok;
  },
  ...
  // Generate a signed URL (cached for 23h, expires in 24h)
  getSignedUrl: async (path) => {
    if(!path) return "";
    const cached = _signedUrlCache.get(path);
    if(cached && cached.expiresAt > Date.now()) return cached.url;
    try {
      const res = await fetch(`${SB_URL}/storage/v1/object/sign/media/${path}`, {
        method:"POST",
      ...
      if(res.ok) {
        const data = await res.json();
        const url = `${SB_URL}/storage/v1${data.signedURL}`;
        _signedUrlCache.set(path, {url, expiresAt: Date.now() + 82800000}); // cache 23h
        return url;
      }
    } catch {}
    // Signing failed — return empty (placeholder will show)
    return "";
  },
  // Synchronous URL: returns signed URL from cache, or placeholder + triggers async signing
  getUrl: (path) => {
    if(!path) return "";
    if(path.startsWith("http")||path.startsWith("data:")||path.startsWith("blob:")) return path;
    const cached = _signedUrlCache.get(path);
    if(cached && cached.expiresAt > Date.now()) return cached.url;
    // Cache miss: trigger async signing in background
    sbStorage.getSignedUrl(path).catch(()=>{});
    // Return transparent pixel placeholder (avoids broken image icon)
    return "data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7";
  },
  // Batch sign multiple paths at once (much faster than individual)
  signBatch: async (paths) => {
    if(!paths||paths.length===0) return {};
    const unsigned = paths.filter(p => {
      const c = _signedUrlCache.get(p);
      return !c || c.expiresAt <= Date.now();
    });
    if(unsigned.length === 0) {
      const result = {};
      paths.forEach(p => { result[p] = _signedUrlCache.get(p)?.url || ""; });
      return result;
    }
    // Try batch endpoint first
    let batchOk = false;
    try {
      const res = await fetch(`${SB_URL}/storage/v1/object/sign/media`, {
        method:"POST",
        headers:{...sbHeaders(),"Content-Type":"application/json"},
        body: JSON.stringify({paths: unsigned, expiresIn: 86400})
      });
      if(res.ok) {
        const data = await res.json();
        (data||[]).forEach((item,i) => {
          if(item.signedURL) {
            const url = `${SB_URL}/storage/v1${item.signedURL}`;
            _signedUrlCache.set(item.path||unsigned[i], {url, expiresAt: Date.now() + 82800000});
          }
        });
        batchOk = true;
      }
    } catch {}
    // Fallback: sign individually if batch failed
    if(!batchOk) {
      await Promise.allSettled(unsigned.map(p => sbStorage.getSignedUrl(p)));
    }
    const result = {};
    const placeholder = "data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7";
    paths.forEach(p => { result[p] = _signedUrlCache.get(p)?.url || placeholder; });
    return result;
  },
  delete: async (paths) => {
...
const compressImage=async(file,maxDim=1200,quality=0.8)=>{
  if(!file||!file.type?.startsWith("image/")||file.type==="image/gif")return file;
  if(file.size<500000&&file.type==="image/jpeg")return file; // already small jpeg
  return new Promise(resolve=>{
    const img=new Image();
    img.onload=()=>{
      let w=img.width,h=img.height;
      if(w<=maxDim&&h<=maxDim&&file.size<500000){resolve(file);return;}
...
// ─── Media URL resolver (converts public URLs to signed URLs) ───
function extractStoragePath(url) {
  if(!url||typeof url!=="string") return null;
  // Extract path from /storage/v1/object/public/media/PATH or /storage/v1/object/authenticated/media/PATH
  const m = url.match(/\/storage\/v1\/object\/(?:public|authenticated|sign)\/media\/(.+?)(?:\?|$)/);
  if(m) return m[1];
  // Already just a path (no URL prefix)
  if(!url.startsWith("http") && !url.startsWith("data:") && !url.startsWith("blob:")) return url;
  return null;
}

// Resolve a media URL: if it's a public URL, convert to signed; if already signed/cached, return as-is
async function resolveMediaUrl(url) {
  if(!url || url.startsWith("data:") || url.startsWith("blob:")) return url;
  const path = extractStoragePath(url);
  if(path) return await sbStorage.getSignedUrl(path);
  return url;
}

// Synchronous version — returns cached signed URL or triggers async signing
function resolveMediaUrlSync(url) {
  if(!url || url.startsWith("data:") || url.startsWith("blob:")) return url;
  const path = extractStoragePath(url);
  if(path) return sbStorage.getUrl(path);
  return url;
}
// Shorthand alias for use in JSX
const r=resolveMediaUrlSync;

// Hook to resolve a media URL reactively
function useSignedUrl(url) {
  const [signed, setSigned] = useState(()=>resolveMediaUrlSync(url));
  useEffect(()=>{
    if(!url) return;
    let cancelled = false;
    resolveMediaUrl(url).then(u => { if(!cancelled) setSigned(u); });
...
function SignedImg({src, ...props}) {
  const url = useSignedUrl(src);
  return <img src={url} {...props}/>;
}

// Batch-resolve all media URLs in an array of objects
async function signMediaUrls(items, ...fields) {
  if(!items||items.length===0) return items;
  const paths = [];
  items.forEach((item) => {
    fields.forEach(f => {
      // Support nested fields like "profiles.avatar_url"
      const parts = f.split(".");
      let val = item;
  ...
  if(paths.length > 0) await sbStorage.signBatch([...new Set(paths)]);
  return items; // cache is now populated, r() will find them
}

// Pre-sign a single path (async, populates cache)
async function preSign(url) {
  if(!url) return;
  const path = extractStoragePath(url);
  if(path) await sbStorage.getSignedUrl(path);
}

// ─── Draft save/load (localStorage) ───
function saveDraft(key,val){try{if(val&&val.trim())localStorage.setItem("dp-draft-"+key,val);else localStorage.removeItem("dp-draft-"+key);}catch{}}
function loadDraft(key){try{return localStorage.getItem("dp-draft-"+key)||"";}catch{return "";}}
function clearDraft(key){try{localStorage.removeItem("dp-draft-"+key);}catch{}}

// ─── Global toast (module-level, callable from anywhere) ───
let _toastHandler = null;
function toast(msg, ms=3000) {
  if(_toastHandler) _toastHandler(msg, ms);
  else console.log("[toast]", msg);
}

// ─── Notification helper (uses SECURITY DEFINER RPC to avoid RLS issues) ───
async function notify(params){
  try{await sb.rpc("create_notification",{
    p_user_id:params.user_id,p_type:params.type,
    p_from_username:params.from_username||null,p_from_avatar:params.from_avatar||null,
    p_text:params.text||"",p_diary_name:params.diary_name||null,
    p_report_number:params.report_number||null,p_thread_id:params.thread_id||null,
  });}catch{}
}
async function notifyBatch(arr){
  if(!arr||arr.length===0)return;
  try{await sb.rpc("create_notifications_batch",{p_data:JSON.stringify(arr)});}catch{}
}
    ...
    <div style={{position:"absolute",bottom:"100%",left:0,right:0,marginBottom:"4px",background:C.cardBg,border:`1px solid ${C.border}`,borderRadius:"10px",boxShadow:"0 4px 16px rgba(0,0,0,0.12)",overflow:"hidden",zIndex:50}}>
      {suggestions.map((u,i)=>(
        <div key={u.id} onClick={()=>insertMention(u)} style={{display:"flex",alignItems:"center",gap:"10px",padding:"8px 12px",cursor:"pointer",background:i===selIdx?C.accentBg:"transparent",borderBottom:i<suggestions.length-1?`1px solid ${C.border}`:"none"}}>
          <div style={{width:"24px",height:"24px",borderRadius:"50%",background:C.surface2,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"12px",overflow:"hidden",border:`1px solid ${C.border}`,flexShrink:0}}>{u.avatar_url?<img src={r(u.avatar_url)} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}} loading="lazy"/>:(u.avatar||"🌱")}</div>
...
function AuthScreen({ onLogin }) {
  ...
  const doLogin=async()=>{
    ...
    try{
      ...
      onLogin({...profile,avatarImg:r(profile.avatar_url),createdAt:profile.created_at,authId:data.user.id});
  ...
  return (
    <div style={{maxWidth:"600px",margin:"0 auto",padding:"32px 24px"}}>
      ...
      <div style={{background:C.surfaceLight,borderRadius:"12px",border:`1px solid ${C.border}`,padding:"16px 20px",marginBottom:"20px"}}>
        ...
        <div style={{fontFamily:F.sans,fontSize:"13px",color:C.muted,lineHeight:"2"}}><div>📧 {user.email}</div><div>📆 Membro desde {new Date(user.createdAt).toLocaleDateString("pt-BR",{month:"long",year:"numeric"})}</div>{(user.current_streak>0||user.best_streak>0)&&<div>🔥 Streak: {user.current_streak||0} semana{user.current_streak!==1?"s":""} consecutiva{user.current_streak!==1?"s":""}{user.best_streak>user.current_streak?` (recorde: ${user.best_streak})`:""}</div>}</div>
      </div>
      {(blockedUsers||[]).length>0&&<div style={{background:C.surfaceLight,borderRadius:"12px",border:`1px solid ${C.border}`,padding:"16px 20px",marginBottom:"20px"}}>
        ...
        {blockedUsers.map(b=>(
          <div key={b.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:`1px solid ${C.border22}`}}>
            <div style={{display:"flex",alignItems:"center",gap:"8px"}}>
              <div style={{width:"28px",height:"28px",borderRadius:"50%",background:C.surface2,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"14px",overflow:"hidden",border:`1px solid ${C.border}`}}>{b.avatarImg?<img src={b.avatarImg} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:b.avatar||"🌱"}</div>
              <span style={{fontFamily:F.sans,fontSize:"13px",color:C.text,fontWeight:"500"}}>@{b.username||b.id.substring(0,8)}</span>
            </div>
            <button onClick={()=>onUnblockUser?.(b.id)} style={{padding:"4px 12px",borderRadius:"8px",border:`1px solid ${C.accent44}`,background:C.accentBg,color:C.accent,cursor:"pointer",fontSize:"11px",fontFamily:F.sans,fontWeight:"600"}}>Desbloquear</button>
          </div>
        ))}
      </div>}
...
// ─── Diary Detail ───
// ─── Timeline Slideshow (grow evolution) ───
function TimelineSlideshow({frames,diaryName,strain,onViewImage,onShare}){
  // frames = [{weekNum, phase, photoUrl, height}]
  ...
  const f=frames[idx];
  if(!f)return null;
  const progress=frames.length>1?((idx/(frames.length-1))*100):100;

  return(
    <div style={{background:"#000",borderRadius:"16px",overflow:"hidden",position:"relative"}}>
      {/* Photo */}
      <div style={{position:"relative",minHeight:"280px",display:"flex",alignItems:"center",justifyContent:"center",background:"#0a0a0a"}}>
        <img src={f.photoUrl} alt={`Semana ${f.weekNum}`} onClick={()=>onViewImage?.(f.photoUrl)} style={{maxWidth:"100%",maxHeight:"420px",objectFit:"contain",cursor:"pointer",transition:"opacity 0.3s"}}/>
        
        {/* Week/Phase overlay top-left */}
        <div style={{position:"absolute",top:"12px",left:"12px",display:"flex",gap:"6px"}}>
          <span style={{padding:"4px 12px",borderRadius:"8px",background:"rgba(0,0,0,0.7)",color:"#fff",fontSize:"13px",fontFamily:"Inter,sans-serif",fontWeight:"700"}}>{PHASES[f.phase]===PHASES[0]?"Germinação":(PHASES[f.phase]===PHASES[3]?"Colheita":`Semana ${f.weekNum}`)}</span>
          <span style={{padding:"4px 10px",borderRadius:"8px",background:(PHASE_COLORS[f.phase]||"#1B9E42")+"cc",color:"#fff",fontSize:"11px",fontFamily:"Inter,sans-serif",fontWeight:"600"}}>{PHASES[f.phase]}</span>
        </div>

        {/* Height overlay top-right */}
        {f.height>0&&<span style={{position:"absolute",top:"12px",right:"12px",padding:"4px 10px",borderRadius:"8px",background:"rgba(0,0,0,0.7)",color:"#4ade80",fontSize:"12px",fontFamily:"Inter,sans-serif",fontWeight:"600"}}>📏 {f.height}cm</span>}
        
        {/* Counter */}
        <span style={{position:"absolute",bottom:"12px",right:"12px",padding:"3px 10px",borderRadius:"8px",background:"rgba(0,0,0,0.7)",color:"#fff",fontSize:"11px",fontFamily:"Inter,sans-serif"}}>{idx+1}/{frames.length}</span>
      </div>

      {/* Progress bar */}
      <div style={{height:"3px",background:"#333"}}>
        <div style={{height:"100%",width:`${progress}%`,background:"#1B9E42",transition:"width 0.3s"}}/>
      </div>

      {/* Controls */}
      <div style={{padding:"12px 16px",display:"flex",alignItems:"center",gap:"12px",background:"#111"}}>
        {/* Play/Pause */}
        <button onClick={()=>{if(!playing&&idx>=frames.length-1)setIdx(0);setPlaying(!playing);}} style={{width:"36px",height:"36px",borderRadius:"50%",border:"none",background:"#1B9E42",color:"#fff",cursor:"pointer",fontSize:"16px",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}} aria-label={playing?"Pausar":"Reproduzir"}>{playing?"⏸":"▶"}</button>
        
        {/* Scrubber */}
        <input type="range" min={0} max={frames.length-1} value={idx} onChange={e=>{setIdx(Number(e.target.value));setPlaying(false);}} style={{flex:1,accentColor:"#1B9E42"}} aria-label="Posição na timeline"/>
        
        {/* Speed */}
        <select value={speed} onChange={e=>setSpeed(Number(e.target.value))} style={{padding:"4px 8px",borderRadius:"6px",border:"1px solid #333",background:"#222",color:"#ccc",fontSize:"11px",fontFamily:"Inter,sans-serif",cursor:"pointer"}}>
        ...
        {/* Share */}
        {onShare&&<button onClick={onShare} style={{padding:"6px 12px",borderRadius:"8px",border:"1px solid #333",background:"#222",color:"#ccc",cursor:"pointer",fontSize:"12px",fontFamily:"Inter,sans-serif"}} aria-label="Compartilhar timeline">🔗</button>}
      </div>

      {/* Diary info bar */}
      <div style={{padding:"8px 16px",background:"#0a0a0a",borderTop:"1px solid #222",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <span style={{fontFamily:"Inter,sans-serif",fontSize:"13px",color:"#ccc",fontWeight:"600"}}>{diaryName}</span>
        {strain&&<span style={{fontFamily:"Inter,sans-serif",fontSize:"11px",color:"#4ade80",background:"rgba(74,222,128,0.1)",padding:"2px 8px",borderRadius:"6px"}}>🧬 {strain}</span>}
      </div>
    </div>
  );
}
  ...
  return (
    <div>
      <div style={{padding:"60px 24px 40px",textAlign:"center",position:"relative"}}>
        ...
        <div style={{position:"relative",zIndex:1}}>
          ...
          <div style={{display:"flex",justifyContent:"center",gap:"12px",marginTop:"16px",fontSize:"14px",fontFamily:F.sans,flexWrap:"wrap"}}>
            ...
            <button onClick={()=>{const url=`${window.location.origin}?diary=${diary.id}`;if(navigator.share){navigator.share({title:`🌱 ${diary.name}`,text:`Veja o cultivo de ${diary.strain||"planta"} no Diário da Planta!`,url}).catch(()=>{});}else{navigator.clipboard?.writeText(url).then(()=>toast("Link copiado!")).catch(()=>{});}}} style={{background:C.cardBg,border:`1px solid ${C.border}`,borderRadius:"20px",padding:"6px 12px",cursor:"pointer",display:"flex",alignItems:"center",gap:"4px",fontSize:"13px",fontFamily:F.sans,color:C.dim,fontWeight:"500",transition:"all 0.2s"}} aria-label="Compartilhar diário">🔗</button>
      ...
      <div style={{maxWidth:"700px",margin:"0 auto",padding:"0 24px"}}>
        ...
        {/* Timeline Slideshow — appears when 2+ weeks have photos */}
        {(()=>{
          const frames=(diary.weeks||[]).filter(w=>w.media?.length>0).map(w=>({
            weekNum:w.week,phase:w.phase,
            photoUrl:(w.media.find(m=>m.type!=="video")||w.media[0])?.data||"",
          ...
          return <div style={{marginBottom:"24px"}}>
            <TimelineSlideshow 
              ...
              strain={diary.strain||(diary.strains||[])[0]||""} 
              onViewImage={onViewImage}
              onShare={()=>{
                const url=`${window.location.origin}?diary=${diary.id}`;
                if(navigator.share){navigator.share({title:`🌱 ${diary.name} — Diário da Planta`,text:`Acompanhe a evolução do cultivo de ${diary.strain||"planta"}!`,url}).catch(()=>{});}
...
function EditWeekModal({ week, weekIdx, onClose, onSave, lang, diaryId, userId }) {
  ...
  useEffect(()=>{
    ...
    (async()=>{try{
      ...
      await signMediaUrls(rows,"media_url");
      setExistingMedia(rows.map(m=>({id:m.id,url:r(m.media_url),type:m.media_type})));
    }catch{}})();
  },[week.id]);
...
// ─── Messages Page ───
function MessagesPage({ msgs, user, onSend, onSendMedia, onMarkRead, onMarkUnread, onDeleteConv, onDeleteMessage, onForwardMsg, onLoadOlder, onCreateGroup, onNewDM, onViewImage, onViewVideo, onReportMessage, onBack, lang }) {
  const t=T[lang||"pt"];
  ...
  if(conv) return (
    <div style={{maxWidth:"700px",margin:"0 auto",display:"flex",flexDirection:"column",height:"calc(100vh - 140px)"}}>
      <div style={{padding:"14px 20px",borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center",gap:"12px"}}>
        <button onClick={()=>setActiveConv(null)} style={{padding:"6px 12px",borderRadius:"16px",border:`1px solid ${C.border}`,background:C.cardBg,color:C.muted,cursor:"pointer",fontSize:"13px",fontFamily:F.sans}} aria-label="Voltar">←</button>
        <div style={{width:"36px",height:"36px",borderRadius:"50%",background:C.surface2,display:"flex",alignItems:"center",justifyContent:"center",fontSize:conv.isGroup?"16px":"20px",border:`1px solid ${C.border}`,overflow:"hidden"}}>{conv.isGroup?"👥":(conv.avatarImg?<img src={conv.avatarImg} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:conv.avatar)}</div>
        <div style={{flex:1}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"16px 20px",display:"flex",flexDirection:"column",gap:"8px"}}>
        {conv.hasMore&&<div style={{textAlign:"center",marginBottom:"8px"}}><button onClick={()=>onLoadOlder?.(conv.id)} style={{padding:"6px 16px",borderRadius:"20px",border:`1px solid ${C.border}`,background:C.surface2,color:C.muted,cursor:"pointer",fontSize:"12px",fontFamily:F.sans}}>⬆ Carregar anteriores</button></div>}
          ...
          return (<div key={m.id}>
            ...
            <div style={{display:"flex",justifyContent:isMe?"flex-end":"flex-start",alignItems:"flex-end",gap:"6px"}}>
              <div style={{maxWidth:"75%",padding:m.media?"6px":"10px 14px",borderRadius:isMe?"16px 16px 4px 16px":"16px 16px 16px 4px",background:isMe?C.accent:C.msgBubble,color:isMe?C.onAccent:C.text,fontFamily:F.sans,fontSize:"14px",lineHeight:"1.5",position:"relative",overflow:"hidden"}}>
                ...
                <div style={{fontSize:"10px",color:isMe?"rgba(255,255,255,0.55)":C.dim,marginTop:"4px",textAlign:"right",padding:m.media?"0 8px 6px":"0"}}>{timeStr(m.time)}{isMe&&!conv.isGroup&&<span style={{marginLeft:"4px",color:conv.otherReadAt&&m.time<=conv.otherReadAt?"#4fc3f7":"rgba(255,255,255,0.35)"}}>{conv.otherReadAt&&m.time<=conv.otherReadAt?"✓✓":"✓"}</span>}</div>
              </div>
              <button onClick={()=>setForwardingMsg({text:m.text||"",media:m.media||null,fromConv:conv.id})} title="Encaminhar" aria-label="Encaminhar mensagem" style={{width:"24px",height:"24px",borderRadius:"50%",border:"none",background:"transparent",color:C.dim,cursor:"pointer",fontSize:"12px",flexShrink:0,opacity:0.5,display:"flex",alignItems:"center",justifyContent:"center"}} onMouseOver={e=>e.currentTarget.style.opacity="1"} onMouseOut={e=>e.currentTarget.style.opacity="0.5"}>↪</button>
              {!isMe&&!m.isSystem&&<button onClick={()=>onReportMessage?.({convId:conv.id,senderName:m.from,text:m.text,mediaUrl:m.media?.data})} title="Denunciar mensagem" aria-label="Denunciar mensagem" style={{width:"24px",height:"24px",borderRadius:"50%",border:"none",background:"transparent",color:C.dim,cursor:"pointer",fontSize:"12px",flexShrink:0,opacity:0.5,display:"flex",alignItems:"center",justifyContent:"center"}} onMouseOver={e=>{e.currentTarget.style.opacity="1";e.currentTarget.style.color="#f59e0b";}} onMouseOut={e=>{e.currentTarget.style.opacity="0.5";e.currentTarget.style.color=C.dim;}}>🚩</button>}
              {isMe&&<button onClick={()=>{if(window.confirm("Apagar esta mensagem?"))onDeleteMessage?.(conv.id,m.id);}} title="Apagar" aria-label="Apagar mensagem" style={{width:"24px",height:"24px",borderRadius:"50%",border:"none",background:"transparent",color:C.dim,cursor:"pointer",fontSize:"12px",flexShrink:0,opacity:0.5,display:"flex",alignItems:"center",justifyContent:"center"}} onMouseOver={e=>{e.currentTarget.style.opacity="1";e.currentTarget.style.color=C.error;}} onMouseOut={e=>{e.currentTarget.style.opacity="0.5";e.currentTarget.style.color=C.dim;}}>🗑</button>}
            </div>
          </div>);
        })}
        <div ref={endRef}/>
        ...
        return (
          <div key={c.id} style={{position:"relative",marginBottom:"10px"}}>
            <div onClick={()=>openConv(c.id)} style={{padding:"14px 16px",borderRadius:"12px",border:`1px solid ${C.border}`,background:C.cardBg,cursor:"pointer",display:"flex",alignItems:"center",gap:"14px",transition:"all 0.15s",boxShadow:"0 1px 3px rgba(0,0,0,0.04)"}} onMouseOver={e=>e.currentTarget.style.background=C.surface2} onMouseOut={e=>e.currentTarget.style.background=C.cardBg}>
              <div style={{width:"44px",height:"44px",borderRadius:"50%",background:C.surface2,display:"flex",alignItems:"center",justifyContent:"center",fontSize:c.isGroup?"18px":"22px",border:`1px solid ${C.border}`,flexShrink:0,overflow:"hidden"}}>{c.isGroup?"👥":(c.avatarImg?<img src={c.avatarImg} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:c.avatar)}</div>
              <div style={{flex:1,minWidth:0}}>
                ...
                {lastMsg&&<div style={{fontFamily:F.sans,fontSize:"13px",color:isUnread?C.text:C.muted,fontWeight:isUnread?"600":"400",marginTop:"2px",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{lastMsg.from===user.email?<><span style={{color:c.otherReadAt&&lastMsg.time<=c.otherReadAt?"#4fc3f7":C.dim,marginRight:"3px"}}>✓✓</span>Você: </>:""}{lastMsg.text||lastMsg.media?(lastMsg.text||(lastMsg.media?.type==="video"?"🎬 Vídeo":"📷 Imagem")):""}</div>}
              </div>
            ...
            {convMenu===c.id&&<div ref={menuRef} style={{position:"absolute",top:"50px",right:"8px",background:C.cardBg,borderRadius:"12px",border:`1px solid ${C.border}`,padding:"4px",minWidth:"200px",boxShadow:"0 8px 24px rgba(0,0,0,0.12)",zIndex:20}}>
              ...
              {convMenuItem("🗑️","Excluir conversa",()=>{if(window.confirm("Tem certeza que deseja excluir esta conversa?"))onDeleteConv?.(c.id);},C.error)}
            </div>}
          </div>
        );
              ...
              {groupSuggestions.length>0&&groupSearch.length>=2&&<div style={{position:"absolute",top:"100%",left:0,right:0,marginTop:"4px",background:C.cardBg,border:`1px solid ${C.border}`,borderRadius:"10px",boxShadow:"0 4px 16px rgba(0,0,0,0.12)",overflow:"hidden",zIndex:50,maxHeight:"200px",overflowY:"auto"}}>
                {groupSuggestions.map(u=>(
                  <div key={u.id} onClick={()=>{setGroupMemberChips(p=>[...p,u.username]);setGroupSearch("");setGroupSuggestions([]);}} style={{display:"flex",alignItems:"center",gap:"10px",padding:"10px 14px",cursor:"pointer"}} onMouseOver={e=>e.currentTarget.style.background=C.surface2} onMouseOut={e=>e.currentTarget.style.background="transparent"}>
                    <div style={{width:"28px",height:"28px",borderRadius:"50%",background:C.surface2,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"14px",overflow:"hidden",border:`1px solid ${C.border}`,flexShrink:0}}>{u.avatar_url?<img src={r(u.avatar_url)} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}} loading="lazy"/>:(u.avatar||"🌱")}</div>
                    <span style={{fontFamily:F.sans,fontSize:"13px",fontWeight:"600"}}>{u.username}</span>
                  </div>
                ))}
              </div>}
...
function AdminPanel({ user, onBack, onNewPost }) {
  ...
  const [contestForm,setContestForm]=useState({title:"",description:"",category:"geral",submission_end:"",voting_end:"",prize_description:"",status:"active"});
  const [editingContest,setEditingContest]=useState(null); // contest id being edited
  ...
  useEffect(()=>{(async()=>{
    try{
      ...
      const [profiles,diaries,reps,al,ac,bp,ct]=await Promise.all([
        ...
        sb.from("contests").select("*","&order=created_at.desc"),
      ]);

      const uMap={};(profiles||[]).forEach(p=>{uMap[p.id]={...p,email:p.email,username:p.username,avatar:p.avatar,avatarImg:r(p.avatar_url),bio:p.bio,city:p.city,role:p.role,banned:p.banned,createdAt:new Date(p.created_at).getTime()};});
      setAllUsers(uMap);
      ...
      setAllComments((ac||[]).map(c=>({id:c.id,text:c.text,username:c.profiles?.username,avatar:c.profiles?.avatar,avatarImg:r(c.profiles?.avatar_url),authorEmail:c.user_id,diaryId:c.diary_id,diaryName:c.diaries?.name,time:new Date(c.created_at).getTime()})));
  ...
  const doSendWarning=async()=>{
    ...
    try{
      // 1) Notification
      await notify({user_id:warnTarget,type:"warning",from_username:"Administração",from_avatar:"🛡️",text:cleanWarn,report_number:refNum||null});
  ...
  const doSendAnnouncement=async()=>{
    ...
    try{await notifyBatch(inserts);count=inserts.length;}catch{}
  ...
  const filteredUsers=searchUsers==="[banido]"?userList.filter(u=>u.banned):searchUsers==="[admin]"?userList.filter(u=>u.role==="admin"):searchUsers?userList.filter(u=>(u.username+" "+u.email).toLowerCase().includes(searchUsers.toLowerCase())):userList;
    ...
    <div style={{maxWidth:"1100px",margin:"0 auto",padding:"24px"}}>
      ...
      {tab==="dashboard"&&<div>
        ...
        {(()=>{
          ...
          const newUsersWeek=userList.filter(u=>u.createdAt>week).length;
          const commentsWeek=allComments.filter(c=>c.time>week).length;
          const activeContests=adminContests.filter(c=>c.status==="active").length;
          ...
          return(<>
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))",gap:"12px",marginBottom:"20px"}}>
              ...
              {stat("📅","Novos (7d)",newUsersWeek,"#2d8659")}
              {stat("📓","Diários Ativos",totalDiaries,"#805ad5")}
              {stat("💬","Comentários (7d)",commentsWeek,"#d69e2e")}
              {stat("📰","Posts Blog",totalBlogPosts,"#e53e3e")}
              {stat("🏆","Concursos Ativos",activeContests,"#f59e0b")}
              {stat("🚨","Denúncias",pendingReps,"#e53e3e")}
              {stat("🚫","Banidos",bannedUsers,"#718096")}
            </div>
            {/* Recent activity */}
            <div style={{fontFamily:F.sans,fontSize:"14px",fontWeight:"700",marginBottom:"10px"}}>🕐 Atividade Recente</div>
            <div style={{background:C.cardBg,borderRadius:"12px",border:`1px solid ${C.border}`,padding:"14px 16px"}}>
              {auditLog.slice(0,8).map(a=>(
                <div key={a.id} style={{fontFamily:F.sans,fontSize:"12px",color:C.muted,padding:"4px 0",borderBottom:`1px solid ${C.border22}`}}>
                  <span style={{fontWeight:"600",color:C.text}}>{a.admin}</span> {a.action} <span style={{color:C.accent}}>{a.target}</span> <span style={{color:C.dim,fontSize:"10px"}}>· {new Date(a.time).toLocaleString("pt-BR",{day:"2-digit",month:"2-digit",hour:"2-digit",minute:"2-digit"})}</span>
                </div>
              ))}
              {auditLog.length===0&&<div style={{fontFamily:F.sans,fontSize:"12px",color:C.dim,textAlign:"center",padding:"12px"}}>Nenhuma atividade registrada</div>}
            </div>
          </>);
        })()}
      </div>}
      ...
      {tab==="users"&&<div>
        ...
        <div style={{display:"flex",gap:"6px",marginBottom:"12px",flexWrap:"wrap"}}>
          {[["all","Todos",userList.length],["banned","🚫 Banidos",userList.filter(u=>u.banned).length],["admins","👑 Admins",userList.filter(u=>u.role==="admin").length]].map(([key,label,count])=>(
            <button key={key} onClick={()=>setSearchUsers(key==="banned"?"[banido]":(key==="admins"?"[admin]":""))} style={{padding:"4px 12px",borderRadius:"8px",border:`1px solid ${(searchUsers==="[banido]"&&key==="banned")||(searchUsers==="[admin]"&&key==="admins")||(searchUsers!==""&&searchUsers!=="[banido]"&&searchUsers!=="[admin]"&&key==="all")||(searchUsers===""&&key==="all")?C.accent:C.border}`,background:(searchUsers==="[banido]"&&key==="banned")||(searchUsers==="[admin]"&&key==="admins")||(searchUsers===""&&key==="all")?C.accentBg:"transparent",color:(searchUsers==="[banido]"&&key==="banned")||(searchUsers==="[admin]"&&key==="admins")||(searchUsers===""&&key==="all")?C.accent:C.muted,cursor:"pointer",fontSize:"11px",fontFamily:F.sans,fontWeight:"600"}}>{label} ({count})</button>
          ))}
        </div>
        {filteredUsers.map(u=>(
        ...
        <div style={{display:"flex",flexDirection:"column",gap:"8px"}}>
          {blogPosts.map(p=>(
            <div key={p.id} style={{background:C.cardBg,borderRadius:"12px",border:`1px solid ${C.border}`,padding:"14px 16px",display:"flex",alignItems:"center",gap:"12px"}}>
              {p.cover_url&&<img src={r(p.cover_url)} alt="" style={{width:"48px",height:"48px",borderRadius:"8px",objectFit:"cover",flexShrink:0}}/>}
              <div style={{flex:1,minWidth:0}}>
      ...
      {tab==="contests"&&<div>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"16px"}}>
          <div style={{fontFamily:F.sans,fontSize:"15px",fontWeight:"700"}}>🏆 Concursos ({adminContests.length})</div>
          <button onClick={()=>{setContestForm({title:"",description:"",category:"geral",submission_end:"",voting_end:"",prize_description:"",status:"active"});setEditingContest(null);setShowContestForm(true);}} style={{padding:"6px 14px",borderRadius:"10px",border:`1px solid ${C.accent}`,background:C.accentBg,color:C.accent,cursor:"pointer",fontSize:"12px",fontFamily:F.sans,fontWeight:"600"}}>+ Novo Concurso</button>
        </div>
        
        {adminContests.length===0?<div style={{textAlign:"center",padding:"40px",color:C.dim,fontFamily:F.sans}}>Nenhum concurso criado ainda.</div>:
        adminContests.map(c=>{
          const now=Date.now();
          const subEnd=new Date(c.submission_end).getTime();
          const voteEnd=new Date(c.voting_end).getTime();
          const phase=c.status==="draft"?"Rascunho":(c.status==="ended"?"Encerrado":(now<subEnd?"Recebendo fotos":(now<voteEnd?"Em votação":"Encerrado")));
          const phColor=phase==="Recebendo fotos"?"#38a169":(phase==="Em votação"?"#d69e2e":"#e53e3e");
          return(
            <div key={c.id} style={{background:C.cardBg,borderRadius:"12px",border:`1px solid ${C.border}`,padding:"14px 16px",marginBottom:"8px"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:"10px"}}>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{display:"flex",alignItems:"center",gap:"8px",marginBottom:"4px",flexWrap:"wrap"}}>
                    <span style={{fontFamily:F.sans,fontSize:"14px",fontWeight:"700"}}>{c.title}</span>
                    <span style={{padding:"2px 8px",borderRadius:"8px",background:phColor+"18",color:phColor,fontSize:"10px",fontFamily:F.sans,fontWeight:"600"}}>{phase}</span>
                    <span style={{padding:"2px 8px",borderRadius:"8px",background:C.surface2,fontSize:"10px",fontFamily:F.sans,color:C.dim}}>{c.category}</span>
                  </div>
                  {c.description&&<div style={{fontFamily:F.sans,fontSize:"12px",color:C.muted,marginBottom:"4px"}}>{c.description.substring(0,100)}</div>}
                  <div style={{fontFamily:F.sans,fontSize:"11px",color:C.dim}}>
                    Submissões até {new Date(c.submission_end).toLocaleDateString("pt-BR")} · Votação até {new Date(c.voting_end).toLocaleDateString("pt-BR")}
                    {c.prize_description&&` · 🎁 ${c.prize_description}`}
                  </div>
                </div>
                <div style={{display:"flex",gap:"6px",flexShrink:0}}>
                  <button onClick={()=>{setContestForm({title:c.title,description:c.description||"",category:c.category||"geral",submission_end:new Date(c.submission_end).toISOString().slice(0,16),voting_end:new Date(c.voting_end).toISOString().slice(0,16),prize_description:c.prize_description||"",status:c.status});setEditingContest(c.id);setShowContestForm(true);}} style={{padding:"4px 10px",borderRadius:"8px",border:`1px solid ${C.border}`,background:C.surface2,color:C.muted,cursor:"pointer",fontSize:"11px",fontFamily:F.sans}}>✏️</button>
                  {c.status!=="ended"&&<button onClick={async()=>{if(!window.confirm("Encerrar este concurso?"))return;try{await sb.from("contests").update({status:"ended"},`id=eq.${c.id}`);setAdminContests(p=>p.map(x=>x.id===c.id?{...x,status:"ended"}:x));showToast("Concurso encerrado.");await addAudit("Encerrou concurso",c.title,"");}catch{}}} style={{padding:"4px 10px",borderRadius:"8px",border:`1px solid #d69e2e33`,background:"#d69e2e08",color:"#d69e2e",cursor:"pointer",fontSize:"11px",fontFamily:F.sans}}>⏹️</button>}
                  <button onClick={async()=>{if(!window.confirm(`Excluir concurso "${c.title}"? Isso apaga todas as fotos e votos.`))return;try{await sb.from("contests").delete(`id=eq.${c.id}`);setAdminContests(p=>p.filter(x=>x.id!==c.id));showToast("Concurso excluído.");await addAudit("Excluiu concurso",c.title,"");}catch{}}} style={{padding:"4px 10px",borderRadius:"8px",border:`1px solid ${C.error}33`,background:C.error+"08",color:C.error,cursor:"pointer",fontSize:"11px",fontFamily:F.sans}}>🗑️</button>
                </div>
              </div>
            </div>
          );
        ...
        {showContestForm&&<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.4)",backdropFilter:"blur(8px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:300,padding:"20px"}} onClick={()=>setShowContestForm(false)}>
          <div style={{...cardBase,maxWidth:"480px"}} onClick={e=>e.stopPropagation()}>
            <h3 style={{fontFamily:F.sans,fontSize:"18px",fontWeight:"700",margin:"0 0 16px"}}>{editingContest?"✏️ Editar Concurso":"🏆 Novo Concurso"}</h3>
            <input placeholder="Título do concurso" aria-label="Título do concurso" value={contestForm.title} onChange={e=>setContestForm(p=>({...p,title:e.target.value}))} style={{...baseInput,marginBottom:"10px"}}/>
            <textarea placeholder="Descrição (opcional)" value={contestForm.description} onChange={e=>setContestForm(p=>({...p,description:e.target.value}))} style={{...baseInput,minHeight:"60px",resize:"vertical",marginBottom:"10px"}}/>
            <select value={contestForm.category} onChange={e=>setContestForm(p=>({...p,category:e.target.value}))} style={{...baseInput,marginBottom:"10px"}}>
              <option value="geral">Geral</option>
              <option value="indoor">Indoor</option>
              <option value="outdoor">Outdoor</option>
              <option value="melhor_flor">Melhor Flor</option>
              <option value="foto_semana">Foto da Semana</option>
              <option value="genetica">Melhor Genética</option>
            </select>
            {editingContest&&<select value={contestForm.status} onChange={e=>setContestForm(p=>({...p,status:e.target.value}))} style={{...baseInput,marginBottom:"10px"}}>
              <option value="draft">Rascunho</option>
              <option value="active">Ativo</option>
              <option value="ended">Encerrado</option>
            </select>}
            <label style={labelSt}>Fim das submissões *</label>
            <input type="datetime-local" value={contestForm.submission_end} onChange={e=>setContestForm(p=>({...p,submission_end:e.target.value}))} style={{...baseInput,marginBottom:"10px"}}/>
            <label style={labelSt}>Fim da votação *</label>
            <input type="datetime-local" value={contestForm.voting_end} onChange={e=>setContestForm(p=>({...p,voting_end:e.target.value}))} style={{...baseInput,marginBottom:"10px"}}/>
            <input placeholder="Prêmio (opcional)" value={contestForm.prize_description} onChange={e=>setContestForm(p=>({...p,prize_description:e.target.value}))} style={{...baseInput,marginBottom:"16px"}}/>
            <div style={{display:"flex",gap:"12px"}}>
              <button style={{...btnSecondary,width:"auto",padding:"10px 20px"}} onClick={()=>setShowContestForm(false)}>Cancelar</button>
              <button style={btnPrimary} disabled={!contestForm.title.trim()||!contestForm.submission_end||!contestForm.voting_end} onClick={async()=>{
                try{
                  const payload={title:sanitize(contestForm.title,100),description:sanitize(contestForm.description,500),category:contestForm.category,submission_end:new Date(contestForm.submission_end).toISOString(),voting_end:new Date(contestForm.voting_end).toISOString(),prize_description:sanitize(contestForm.prize_description,200),status:contestForm.status||"active"};
                  if(editingContest){
                    await sb.from("contests").update(payload,`id=eq.${editingContest}`);
                    setAdminContests(p=>p.map(x=>x.id===editingContest?{...x,...payload}:x));
                    showToast("Concurso atualizado!");
                    await addAudit("Editou concurso",payload.title,"");
                  }else{
                    payload.created_by=user.id;
                    const row=await sb.from("contests").insert(payload);
                    if(row)setAdminContests(p=>[row,...p]);
                    showToast("Concurso criado!");
                    await addAudit("Criou concurso",payload.title,"");
                  }
                  setShowContestForm(false);
                }catch(e){showToast("Erro: "+(e.message||"desconhecido"));}
              }}>{editingContest?"Salvar":"Criar"}</button>
            </div>
          </div>
        </div>}
      </div>}
...
// ─── Public Profile ───
function PublicProfile({ targetUser, diaries, onBack, onViewDiary, lang, allBadges, onReport, currentUserId, onBlock, onUnblock, isBlocked }) {
  ...
  return (
    <div style={{maxWidth:"700px",margin:"0 auto",padding:"32px 24px"}}>
      ...
      <div style={{background:C.surfaceLight,borderRadius:"16px",border:`1px solid ${C.border}`,padding:"32px",textAlign:"center",marginBottom:"20px"}}>
        ...
        {currentUserId&&targetUser.id!==currentUserId&&<div style={{marginTop:"16px",display:"flex",gap:"8px",justifyContent:"center",flexWrap:"wrap"}}>
          <button onClick={()=>onReport?.(targetUser.id,targetUser.username)} style={{padding:"8px 18px",borderRadius:"20px",border:`1px solid ${C.border}`,background:C.surface2,color:C.muted,cursor:"pointer",fontSize:"12px",fontFamily:F.sans,display:"inline-flex",alignItems:"center",gap:"6px"}}>🚩 Denunciar</button>
          {isBlocked?<button onClick={()=>onUnblock?.(targetUser.id)} style={{padding:"8px 18px",borderRadius:"20px",border:`1px solid ${C.accent}44`,background:C.accentBg,color:C.accent,cursor:"pointer",fontSize:"12px",fontFamily:F.sans,display:"inline-flex",alignItems:"center",gap:"6px"}}>✓ Desbloquear</button>
          :<button onClick={()=>{if(window.confirm(`Bloquear @${targetUser.username}? Você não verá mais o conteúdo dele.`))onBlock?.(targetUser.id);}} style={{padding:"8px 18px",borderRadius:"20px",border:`1px solid ${C.error}44`,background:C.errorBg,color:C.error,cursor:"pointer",fontSize:"12px",fontFamily:F.sans,display:"inline-flex",alignItems:"center",gap:"6px"}}>🚫 Bloquear</button>}
        </div>}
...
function BlogEditor({post,onSave,onClose,user}){
  ...
  const insertYT=()=>{
    ...
    if(!vid){toast("URL do YouTube inválida. Use youtube.com/watch?v=... ou youtu.be/...");return;}
    insertAtCursor(`\n[youtube:${vid}]\n`);
  ...
  const doSave=async(status)=>{
    if(!title.trim()){toast("Título é obrigatório");return;}
    setSaving(true);
    try{
  ...
  if(showPreview) return(
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:C.bg,zIndex:400,overflowY:"auto"}}>
      <div style={{maxWidth:"740px",margin:"0 auto",padding:"32px 24px"}}>
        ...
        <div style={{fontFamily:F.body,fontSize:"16px",color:C.text,lineHeight:"1.8"}} dangerouslySetInnerHTML={{__html:DOMPurify.sanitize(`<p style='margin:0 0 14px;line-height:1.8'>${renderPreview(content)}</p>`,{ADD_TAGS:["iframe"],ADD_ATTR:["target","allowfullscreen","playsinline","loading"]})}}/>
      </div>
    </div>
  );
...
function BlogPage({onBack,user,onOpenPost,onNewPost}){
  ...
  useEffect(()=>{(async()=>{
    try{
      ...
      await signMediaUrls(rows,"cover_url","profiles.avatar_url");
      setPosts(rows.map(p=>({...p,authorName:p.profiles?.username,authorAvatar:p.profiles?.avatar,authorAvatarImg:r(p.profiles?.avatar_url)})));
  ...
  return(
    <div style={{maxWidth:"800px",margin:"0 auto",padding:"24px"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:"24px"}}>
        {[...posts].sort((a,b)=>(b.is_featured?1:0)-(a.is_featured?1:0)).map(p=>{
          const ytId=getFirstYT(p.content);
          const firstImg=getFirstImg(p.content);
          const previewImg=r(p.cover_url)||firstImg||(ytId?`https://img.youtube.com/vi/${ytId}/hqdefault.jpg`:null);
          return(
          <div key={p.id} style={{position:"relative"}}>
            {p.is_featured&&<div style={{position:"absolute",top:"12px",left:"12px",zIndex:2,padding:"3px 10px",borderRadius:"8px",background:"#f59e0b",color:"#fff",fontSize:"11px",fontFamily:F.sans,fontWeight:"700"}}>📌 Destaque</div>}
            {user?.role==="admin"&&<button onClick={async(e)=>{e.stopPropagation();const nv=!p.is_featured;try{await sb.from("blog_posts").update({is_featured:nv},`id=eq.${p.id}`);setPosts(prev=>prev.map(x=>x.id===p.id?{...x,is_featured:nv}:x));toast(nv?"Post fixado como destaque!":"Destaque removido.");}catch{}}} style={{position:"absolute",top:"12px",right:"12px",zIndex:2,padding:"4px 10px",borderRadius:"8px",border:"none",background:"rgba(0,0,0,0.5)",color:"#fff",cursor:"pointer",fontSize:"11px",fontFamily:F.sans}}>{p.is_featured?"Remover destaque":"📌 Fixar"}</button>}
            <div onClick={()=>onOpenPost?.(p)} style={{background:C.cardBg,borderRadius:"16px",border:`1px solid ${p.is_featured?"#f59e0b44":C.border}`,overflow:"hidden",cursor:"pointer",transition:"all 0.2s",boxShadow:p.is_featured?"0 4px 16px rgba(245,158,11,0.15)":"0 2px 8px rgba(0,0,0,0.04)"}}>
...
function BlogPostView({post,onBack,user,onEdit,onViewImage}){
  ...
  return(
    <div style={{maxWidth:"740px",margin:"0 auto",padding:"32px 24px"}}>
      ...
      {post.cover_url&&<img src={r(post.cover_url)} alt="" onClick={()=>onViewImage?.(r(post.cover_url))} style={{width:"100%",maxHeight:"360px",objectFit:"cover",borderRadius:"16px",marginBottom:"24px",cursor:"pointer"}}/>}
      <div style={{display:"flex",gap:"8px",alignItems:"center",marginBottom:"12px"}}>
      ...
      <div ref={contentRef} style={{fontFamily:F.body,fontSize:"16px",color:C.text,lineHeight:"1.8"}} dangerouslySetInnerHTML={{__html:DOMPurify.sanitize(`<p style='margin:0 0 14px;line-height:1.8'>${renderMarkdown(post.content)}</p>`,{ADD_TAGS:["iframe"],ADD_ATTR:["target","allowfullscreen","playsinline","loading"]})}}/>
    </div>
  );
}
...
function GrowersPage({user,onBack,onViewProfile,follows,onFollow,onUnfollow,onReport}){
  ...
  useEffect(()=>{(async()=>{
    try{
      ...
      await signMediaUrls(profiles,"avatar_url");
        ...
        {filtered.map((g,idx)=>(
          <div key={g.id} onClick={()=>onViewProfile?.(g)} style={{background:C.cardBg,borderRadius:"16px",border:`1px solid ${C.border}`,padding:"20px 16px",textAlign:"center",cursor:"pointer",transition:"all 0.2s",boxShadow:"0 2px 8px rgba(0,0,0,0.04)",position:"relative"}}>
            ...
            <div style={{width:"64px",height:"64px",borderRadius:"50%",margin:"0 auto 12px",overflow:"hidden",border:`3px solid ${g.level?.color||C.accent}`,display:"flex",alignItems:"center",justifyContent:"center",background:C.surface2,fontSize:"28px"}}>
              {g.avatar_url?<img src={r(g.avatar_url)} alt="" style={{width:"100%",height:"100%",objectFit:"cover",loading:"lazy"}}/>:g.avatar||"🌱"}
            </div>
...
function ContestsPage({user,onBack,onViewImage,pendingContestId}){
  const [contests,setContests]=useState([]);
  const [loading,setLoading]=useState(true);
  const [selectedContest,setSelectedContest]=useState(null);
  const [entries,setEntries]=useState([]);
  const [myVote,setMyVote]=useState(null); // entry_id I voted for
  const [carouselIdx,setCarouselIdx]=useState(0);
  const [showSubmit,setShowSubmit]=useState(false);
  const [submitCaption,setSubmitCaption]=useState("");
  const [submitStrain,setSubmitStrain]=useState("");
  const [submitPhoto,setSubmitPhoto]=useState(null); // base64
  ...
  useEffect(()=>{
    (async()=>{
      try{
        const rows=await sb.from("contests").select("*,profiles(username)",`&order=created_at.desc`);
        setContests(rows||[]);
        // Auto-open shared contest link
        if(pendingContestId&&rows){
          const target=rows.find(c=>c.id===pendingContestId);
          if(target)setSelectedContest(target);
        }
      }catch{}
      setLoading(false);
    })();
  ...
  // Load entries + my vote when contest selected
  useEffect(()=>{
    if(!selectedContest)return;
    (async()=>{
      try{
        const e=await sb.from("contest_entries").select("*,profiles(username,avatar,avatar_url)",`&contest_id=eq.${selectedContest.id}&order=votes_count.desc`);
        await signMediaUrls(e||[],"photo_url","profiles.avatar_url");
        setEntries(e||[]);
        setCarouselIdx(0);
        if(user){
          const v=await sb.from("contest_votes").select("entry_id",`&contest_id=eq.${selectedContest.id}&user_id=eq.${user.id}&limit=1`);
          setMyVote(v?.[0]?.entry_id||null);
        }
      }catch{}
    })();
  },[selectedContest?.id,user?.id]);
  ...
  const getPhase=(c)=>{
    ...
    const subEnd=new Date(c.submission_end).getTime();
    const voteEnd=new Date(c.voting_end).getTime();
    if(now<subEnd)return"submitting";
    if(now<voteEnd)return"voting";
    return"ended";
  };
  const phaseLabel=(p)=>({draft:"Rascunho",submitting:"Recebendo fotos",voting:"Em votação",ended:"Encerrado"}[p]||p);
  const phaseColor=(p)=>({draft:C.dim,submitting:"#38a169",voting:"#d69e2e",ended:"#e53e3e"}[p]||C.dim);
  const timeLeft=(dateStr)=>{
    const d=new Date(dateStr).getTime()-now;
  ...
  const handleSubmit=async()=>{
    if(!user||!selectedContest||!submitPhoto)return;
    setSubmitting(true);
    try{
      const ext="jpg";
      const path=`${user.id}/contest-${Date.now()}.${ext}`;
      const ok=await sbStorage.uploadBase64(path,submitPhoto,"image/jpeg");
      if(!ok){toast("Erro no upload da foto.");setSubmitting(false);return;}
      const photoUrl=sbStorage.getUrl(path);
      await sb.from("contest_entries").insert({contest_id:selectedContest.id,user_id:user.id,photo_url:photoUrl,caption:sanitize(submitCaption,200),strain:sanitize(submitStrain,50)});
      toast("Foto enviada ao concurso!");
      setShowSubmit(false);setSubmitPhoto(null);setSubmitCaption("");setSubmitStrain("");
      // Refresh entries
      const e=await sb.from("contest_entries").select("*,profiles(username,avatar,avatar_url)",`&contest_id=eq.${selectedContest.id}&order=votes_count.desc`);
      await signMediaUrls(e||[],"photo_url","profiles.avatar_url");
      setEntries(e||[]);
      ...
      const msg=e.message||"";
      if(msg.includes("unique")||msg.includes("duplicate"))toast("Você já enviou uma foto para este concurso.");
      else{toast("Erro ao enviar foto.");reportError(e,{feature:"contests",op:"submit"});}
    }
    setSubmitting(false);
  };
  ...
  const handleVote=async(entryId)=>{
    ...
    try{
      await sb.from("contest_votes").insert({contest_id:selectedContest.id,entry_id:entryId,user_id:user.id});
      setMyVote(entryId);
      setEntries(p=>p.map(e=>e.id===entryId?{...e,votes_count:(e.votes_count||0)+1}:e));
      toast("Voto registrado!");
    }catch(e){
      if(e.message?.includes("unique")||e.message?.includes("duplicate"))toast("Você já votou neste concurso.");
      else{toast("Erro ao votar.");reportError(e,{feature:"contests",op:"vote"});}
    }
  };

  // Create contest (admin)
  // Share link
  const shareContest=(c)=>{
    const url=`${window.location.origin}?contest=${c.id}`;
    if(navigator.share){navigator.share({title:`🏆 ${c.title} — Diário da Planta`,text:c.description,url}).catch(()=>{});}
    else{navigator.clipboard?.writeText(url).then(()=>toast("Link copiado!")).catch(()=>{});}
  };
  ...
  const currentEntry=entries[carouselIdx];
  const totalVotes=entries.reduce((s,e)=>s+(e.votes_count||0),0);

  // ─── Contest detail view ───
  if(selectedContest){
    const phase=getPhase(selectedContest);
    const canSubmit=phase==="submitting"&&!myEntry;
    const canVote=phase==="voting"&&!myVote;
    
    return(
      <div style={{maxWidth:"700px",margin:"0 auto",padding:"24px"}}>
        <button onClick={()=>{setSelectedContest(null);setEntries([]);setMyVote(null);}} style={{padding:"6px 14px",borderRadius:"16px",border:`1px solid ${C.border}`,background:C.cardBg,color:C.muted,cursor:"pointer",fontSize:"13px",fontFamily:F.sans,marginBottom:"20px"}}>← Voltar</button>
        
        {/* Header */}
        <div style={{background:C.cardBg,borderRadius:"16px",border:`1px solid ${C.border}`,padding:"24px",marginBottom:"20px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:"12px",marginBottom:"12px"}}>
            <h1 style={{fontFamily:F.sans,fontSize:"24px",fontWeight:"800",margin:0}}>🏆 {selectedContest.title}</h1>
            <span style={{padding:"4px 12px",borderRadius:"20px",background:phaseColor(phase)+"18",color:phaseColor(phase),fontSize:"12px",fontFamily:F.sans,fontWeight:"600",whiteSpace:"nowrap"}}>{phaseLabel(phase)}</span>
          </div>
          {selectedContest.description&&<p style={{fontFamily:F.body,fontSize:"14px",color:C.muted,margin:"0 0 12px",lineHeight:"1.5"}}>{selectedContest.description}</p>}
          {selectedContest.prize_description&&<div style={{padding:"8px 14px",borderRadius:"10px",background:C.accentBg,marginBottom:"12px"}}><span style={{fontFamily:F.sans,fontSize:"13px",color:C.accent,fontWeight:"600"}}>🎁 {selectedContest.prize_description}</span></div>}
          <div style={{display:"flex",gap:"16px",flexWrap:"wrap",fontFamily:F.sans,fontSize:"12px",color:C.dim}}>
            <span>📷 {entries.length} {entries.length===1?"foto":"fotos"}</span>
            <span>🗳️ {totalVotes} {totalVotes===1?"voto":"votos"}</span>
            {phase==="submitting"&&<span>⏰ Submissões: {timeLeft(selectedContest.submission_end)}</span>}
            {phase==="voting"&&<span>⏰ Votação: {timeLeft(selectedContest.voting_end)}</span>}
          </div>
          <div style={{display:"flex",gap:"8px",marginTop:"14px"}}>
            {canSubmit&&<button onClick={()=>setShowSubmit(true)} style={{...btnPrimary,width:"auto",padding:"8px 18px",fontSize:"13px"}}>📷 Enviar Foto</button>}
            <button onClick={()=>shareContest(selectedContest)} style={{padding:"8px 18px",borderRadius:"10px",border:`1px solid ${C.border}`,background:C.surface2,color:C.text,cursor:"pointer",fontSize:"13px",fontFamily:F.sans}}>🔗 Compartilhar</button>
          </div>
        </div>

        {/* Carousel */}
        {entries.length>0?(
          <div style={{background:C.cardBg,borderRadius:"16px",border:`1px solid ${C.border}`,overflow:"hidden",marginBottom:"20px"}}>
            {/* Photo */}
            <div style={{position:"relative",background:"#000",minHeight:"300px",display:"flex",alignItems:"center",justifyContent:"center"}}>
              <img src={r(currentEntry?.photo_url)} alt="" onClick={()=>onViewImage?.(r(currentEntry?.photo_url))} style={{maxWidth:"100%",maxHeight:"500px",objectFit:"contain",cursor:"pointer"}}/>
              {/* Arrows */}
              {entries.length>1&&<>
                <button onClick={()=>setCarouselIdx(p=>(p-1+entries.length)%entries.length)} style={{position:"absolute",left:"8px",top:"50%",transform:"translateY(-50%)",width:"36px",height:"36px",borderRadius:"50%",background:"rgba(0,0,0,0.5)",border:"none",color:"#fff",cursor:"pointer",fontSize:"18px"}}>‹</button>
                <button onClick={()=>setCarouselIdx(p=>(p+1)%entries.length)} style={{position:"absolute",right:"8px",top:"50%",transform:"translateY(-50%)",width:"36px",height:"36px",borderRadius:"50%",background:"rgba(0,0,0,0.5)",border:"none",color:"#fff",cursor:"pointer",fontSize:"18px"}}>›</button>
              </>}
              {/* Counter */}
              <div style={{position:"absolute",bottom:"8px",right:"12px",padding:"2px 10px",borderRadius:"12px",background:"rgba(0,0,0,0.6)",color:"#fff",fontSize:"11px",fontFamily:F.sans}}>{carouselIdx+1}/{entries.length}</div>
              {/* Rank badge */}
              {phase==="ended"&&carouselIdx<3&&<div style={{position:"absolute",top:"12px",left:"12px",padding:"4px 12px",borderRadius:"12px",background:["#FFD700","#C0C0C0","#CD7F32"][carouselIdx],color:"#000",fontSize:"13px",fontFamily:F.sans,fontWeight:"700"}}>{["🥇 1º","🥈 2º","🥉 3º"][carouselIdx]}</div>}
            </div>
            {/* Info bar */}
            <div style={{padding:"16px 20px"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"8px"}}>
                <div style={{display:"flex",alignItems:"center",gap:"8px"}}>
                  <div style={{width:"28px",height:"28px",borderRadius:"50%",background:C.surface2,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"14px",overflow:"hidden",border:`1px solid ${C.border}`}}>{currentEntry?.profiles?.avatar_url?<img src={r(currentEntry.profiles.avatar_url)} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:(currentEntry?.profiles?.avatar||"🌱")}</div>
                  <span style={{fontFamily:F.sans,fontSize:"14px",fontWeight:"600"}}>@{currentEntry?.profiles?.username||"anônimo"}</span>
                  {currentEntry?.strain&&<span style={{fontFamily:F.sans,fontSize:"11px",color:C.accent,background:C.accentBg,padding:"2px 8px",borderRadius:"8px"}}>🧬 {currentEntry.strain}</span>}
                </div>
                <div style={{display:"flex",alignItems:"center",gap:"6px"}}>
                  <span style={{fontFamily:F.sans,fontSize:"14px",fontWeight:"700",color:C.accent}}>❤️ {currentEntry?.votes_count||0}</span>
                </div>
              </div>
              {currentEntry?.caption&&<p style={{fontFamily:F.body,fontSize:"13px",color:C.muted,margin:"0 0 10px"}}>{currentEntry.caption}</p>}
              {canVote&&<button onClick={()=>handleVote(currentEntry?.id)} style={{...btnPrimary,width:"100%",fontSize:"14px",padding:"10px"}}>{myVote?"✓ Você já votou":"❤️ Votar nesta foto"}</button>}
              {myVote===currentEntry?.id&&<div style={{textAlign:"center",fontFamily:F.sans,fontSize:"12px",color:C.accent,fontWeight:"600",marginTop:"8px"}}>✓ Seu voto está nesta foto</div>}
            </div>
            {/* Dots */}
            {entries.length>1&&entries.length<=20&&<div style={{display:"flex",justifyContent:"center",gap:"4px",paddingBottom:"12px"}}>
              {entries.map((_,i)=><button key={i} onClick={()=>setCarouselIdx(i)} style={{width:i===carouselIdx?"20px":"8px",height:"8px",borderRadius:"4px",border:"none",background:i===carouselIdx?C.accent:C.border,cursor:"pointer",transition:"all .2s"}}/>)}
            </div>}
          </div>
        ):(
          <div style={{textAlign:"center",padding:"48px 24px",background:C.surface2,borderRadius:"16px"}}>
            <div style={{fontSize:"48px",marginBottom:"12px"}}>📷</div>
            <p style={{fontFamily:F.sans,fontSize:"15px",color:C.muted}}>Nenhuma foto enviada ainda. Seja o primeiro!</p>
            {canSubmit&&<button onClick={()=>setShowSubmit(true)} style={{...btnPrimary,width:"auto",padding:"10px 24px",marginTop:"16px"}}>📷 Enviar Minha Foto</button>}
          </div>
        )}

        {/* Ranking table (when ended or voting) */}
        {(phase==="voting"||phase==="ended")&&entries.length>0&&<div style={{background:C.cardBg,borderRadius:"16px",border:`1px solid ${C.border}`,padding:"16px 20px"}}>
          <h3 style={{fontFamily:F.sans,fontSize:"15px",fontWeight:"700",margin:"0 0 12px"}}>🏆 Ranking</h3>
          {entries.sort((a,b)=>(b.votes_count||0)-(a.votes_count||0)).map((e,i)=>(
            <div key={e.id} onClick={()=>setCarouselIdx(entries.indexOf(e)>=0?entries.indexOf(e):i)} style={{display:"flex",alignItems:"center",gap:"10px",padding:"8px 0",borderBottom:i<entries.length-1?`1px solid ${C.border22}`:"none",cursor:"pointer"}}>
              <span style={{fontFamily:F.sans,fontSize:"14px",fontWeight:"700",color:i<3?["#FFD700","#888","#CD7F32"][i]:C.dim,width:"24px",textAlign:"center"}}>{i+1}</span>
              <img src={r(e.photo_url)} alt="" style={{width:"40px",height:"40px",borderRadius:"8px",objectFit:"cover"}}/>
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontFamily:F.sans,fontSize:"13px",fontWeight:"600",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>@{e.profiles?.username||"anônimo"}</div>
                {e.strain&&<div style={{fontFamily:F.sans,fontSize:"11px",color:C.dim}}>🧬 {e.strain}</div>}
              </div>
              <span style={{fontFamily:F.sans,fontSize:"13px",fontWeight:"700",color:C.accent}}>❤️ {e.votes_count||0}</span>
              {myVote===e.id&&<span style={{fontSize:"10px"}}>✓</span>}
            </div>
          ))}
        </div>}

        {/* Submit modal */}
        {showSubmit&&<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.5)",backdropFilter:"blur(6px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:300,padding:"24px"}} onClick={()=>setShowSubmit(false)}>
          <div onClick={e=>e.stopPropagation()} style={{background:C.cardBg,borderRadius:"16px",padding:"24px",maxWidth:"440px",width:"100%",maxHeight:"90vh",overflowY:"auto"}}>
            <h3 style={{fontFamily:F.sans,fontSize:"18px",fontWeight:"700",margin:"0 0 16px"}}>📷 Enviar Foto ao Concurso</h3>
            <input ref={fileRef} type="file" accept="image/*" style={{display:"none"}} onChange={pickPhoto}/>
            {submitPhoto?<div style={{position:"relative",marginBottom:"14px"}}>
              <img src={submitPhoto} alt="" style={{width:"100%",maxHeight:"300px",objectFit:"cover",borderRadius:"12px"}}/>
              <button onClick={()=>setSubmitPhoto(null)} style={{position:"absolute",top:"8px",right:"8px",width:"28px",height:"28px",borderRadius:"50%",background:"rgba(0,0,0,0.6)",border:"none",color:"#fff",cursor:"pointer",fontSize:"14px"}}>✕</button>
            </div>:<button onClick={()=>fileRef.current?.click()} style={{width:"100%",padding:"32px",borderRadius:"12px",border:`2px dashed ${C.border}`,background:C.surface2,cursor:"pointer",fontFamily:F.sans,fontSize:"14px",color:C.muted,marginBottom:"14px"}}>📷 Clique para selecionar uma foto</button>}
            <input placeholder="Legenda (opcional)" aria-label="Legenda" value={submitCaption} onChange={e=>setSubmitCaption(e.target.value)} style={{...baseInput,marginBottom:"10px"}}/>
            <input placeholder="Genética / Strain (opcional)" value={submitStrain} onChange={e=>setSubmitStrain(e.target.value)} style={{...baseInput,marginBottom:"16px"}}/>
            <div style={{display:"flex",gap:"10px"}}>
              <button onClick={()=>setShowSubmit(false)} style={{...btnSecondary,flex:1}}>Cancelar</button>
              <button onClick={handleSubmit} disabled={!submitPhoto||submitting} style={{...btnPrimary,flex:1,opacity:(!submitPhoto||submitting)?0.5:1}}>{submitting?"Enviando...":"Enviar"}</button>
            </div>
          </div>
        </div>}
      </div>
    ...
    <div style={{maxWidth:"800px",margin:"0 auto",padding:"24px"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"24px"}}>
        <div>
          ...
          <p style={{fontFamily:F.sans,fontSize:"14px",color:C.muted,margin:0}}>Participe dos concursos da comunidade e ganhe destaque!</p>
        </div>
      </div>

      {loading?<div style={{textAlign:"center",padding:"40px",color:C.dim}}>Carregando...</div>:
      contests.length===0?<div style={{textAlign:"center",padding:"48px",background:C.surface2,borderRadius:"16px"}}>
        <div style={{fontSize:"48px",marginBottom:"12px"}}>🏆</div>
        <p style={{fontFamily:F.sans,fontSize:"15px",color:C.muted}}>Nenhum concurso disponível no momento.</p>
      </div>:
      <div style={{display:"flex",flexDirection:"column",gap:"16px"}}>
        {contests.map(c=>{
          const phase=getPhase(c);
          return(
            <div key={c.id} onClick={()=>setSelectedContest(c)} style={{background:C.cardBg,borderRadius:"16px",border:`1px solid ${C.border}`,padding:"20px",cursor:"pointer",display:"flex",gap:"16px",alignItems:"center",boxShadow:"0 2px 8px rgba(0,0,0,0.04)",transition:"all .15s"}} onMouseOver={e=>e.currentTarget.style.transform="translateY(-2px)"} onMouseOut={e=>e.currentTarget.style.transform=""}>
              <div style={{width:"64px",height:"64px",borderRadius:"14px",background:phaseColor(phase)+"14",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"32px",flexShrink:0}}>🏆</div>
              <div style={{flex:1,minWidth:0}}>
                <div style={{display:"flex",alignItems:"center",gap:"8px",marginBottom:"4px",flexWrap:"wrap"}}>
                  <h3 style={{fontFamily:F.sans,fontSize:"16px",fontWeight:"700",margin:0}}>{c.title}</h3>
                  <span style={{padding:"2px 10px",borderRadius:"20px",background:phaseColor(phase)+"18",color:phaseColor(phase),fontSize:"11px",fontFamily:F.sans,fontWeight:"600"}}>{phaseLabel(phase)}</span>
                </div>
                {c.description&&<p style={{fontFamily:F.body,fontSize:"13px",color:C.muted,margin:"0 0 6px",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{c.description}</p>}
                <div style={{fontFamily:F.sans,fontSize:"11px",color:C.dim}}>
                  {phase==="submitting"&&`⏰ Submissões: ${timeLeft(c.submission_end)}`}
                  {phase==="voting"&&`🗳️ Votação: ${timeLeft(c.voting_end)}`}
                  {phase==="ended"&&"✅ Encerrado"}
                </div>
              </div>
              <div style={{fontSize:"18px",color:C.dim}}>→</div>
            </div>
          );
        })}
      </div>}
...
function ForumPage({user,onBack,onReport,pendingThreadId,onThreadOpened}){
  ...
  const [threadPoll,setThreadPoll]=useState(null);
  const [threadPollOpts,setThreadPollOpts]=useState([]);
  const [threadPollVote,setThreadPollVote]=useState(null);
  const [forumSearchQ,setForumSearchQ]=useState("");
  const [forumSearchResults,setForumSearchResults]=useState([]);
  const [forumSearching,setForumSearching]=useState(false);
  useEffect(()=>{
    if(forumSearchQ.length<2){setForumSearchResults([]);return;}
    const t=setTimeout(async()=>{
      setForumSearching(true);
      try{
        const r=await sb.rpc("search_forum",{q:forumSearchQ,lim:15});
        setForumSearchResults(r||[]);
      }catch{}
      setForumSearching(false);
    },400);
    ...
    if(liked){
      ...
      try{
        ...
        if(authorId&&authorId!==user.id){
          await notify({user_id:authorId,type:"forum_like",from_username:user.username,from_avatar:user.avatar,text:"curtiu seu tópico",thread_id:threadId});
        }
      }catch{}
  ...
  const [newTitle,setNewTitle]=useState(()=>loadDraft("forum-title"));
  const [newContent,setNewContent]=useState(()=>loadDraft("forum-content"));
  ...
  // Auto-save drafts
  useEffect(()=>{const t=setTimeout(()=>{saveDraft("forum-title",newTitle);saveDraft("forum-content",newContent);},1000);return()=>clearTimeout(t);},[newTitle,newContent]);
  // Poll creation state
  ...
  const loadThreads=async(tid)=>{try{const rows=await sb.from("forum_threads").select("*,profiles(username,avatar,avatar_url)",`&topic_id=eq.${tid}&order=pinned.desc,updated_at.desc`);await signMediaUrls(rows||[],"profiles.avatar_url");setThreads(rows||[]);}catch{setThreads([]);}};
  const loadReplies=async(tid)=>{
    try{const rows=await sb.from("forum_replies").select("*,profiles(username,avatar,avatar_url)",`&thread_id=eq.${tid}&order=created_at.asc`);await signMediaUrls(rows||[],"profiles.avatar_url");setReplies(rows||[]);}catch{setReplies([]);}
    // Load poll for this thread
    try{
      const p=await sb.from("forum_polls").select("*",`&thread_id=eq.${tid}&limit=1`);
      if(p?.[0]){
        const poll=p[0];
        poll.closed=poll.closes_at&&new Date(poll.closes_at).getTime()<Date.now();
        setThreadPoll(poll);
        const opts=await sb.from("forum_poll_options").select("*",`&poll_id=eq.${poll.id}&order=position.asc`);
        setThreadPollOpts(opts||[]);
        if(user){
          const v=await sb.from("forum_poll_votes").select("option_id",`&poll_id=eq.${poll.id}&user_id=eq.${user.id}&limit=1`);
          setThreadPollVote(v?.[0]?.option_id||null);
        }
      }else{setThreadPoll(null);setThreadPollOpts([]);setThreadPollVote(null);}
    }catch{setThreadPoll(null);}
  };
  ...
  const renderContent=(text)=>{
    ...
    try{
      ...
      h=h
        ...
        .replace(/\n/g,"<br/>");
      return{__html:DOMPurify.sanitize(h,{ADD_TAGS:["iframe"],ADD_ATTR:["target","allowfullscreen","playsinline","loading","controls","rel"]})};
    }catch{return{__html:DOMPurify.sanitize((text||"").replace(/</g,"&lt;").replace(/\n/g,"<br/>"))};}
  };
  ...
  const createThread=async()=>{
    ...
    setPosting(true);
    try{
      const row=await sb.from("forum_threads").insert({topic_id:selTopic.id,author_id:user.id,title:newTitle.trim().substring(0,200),content:newContent.substring(0,5000)});
      // Create poll if enabled
      if(pollEnabled&&pollQuestion.trim()&&row?.id){
        const validOpts=pollOptions.filter(o=>o.trim());
        if(validOpts.length>=2){
          const pollRow=await sb.from("forum_polls").insert({thread_id:row.id,question:pollQuestion.trim().substring(0,200)});
          if(pollRow?.id){
            for(let i=0;i<validOpts.length;i++){
              await sb.from("forum_poll_options").insert({poll_id:pollRow.id,label:validOpts[i].trim().substring(0,100),position:i});
            }
          }
        }
      }
      ...
      clearDraft("forum-title");clearDraft("forum-content");
    ...
    try{
      ...
      try{
        ...
        if(mentions.length>0){
          ...
          if(mentionNotifs.length>0)await notifyBatch(mentionNotifs);
        }
  ...
  const av=(p)=><div style={{width:"32px",height:"32px",borderRadius:"50%",background:C.surface2,overflow:"hidden",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"16px",border:"1px solid "+C.border,flexShrink:0}}>{p?.avatar_url?<img src={r(p.avatar_url)} alt="" style={{width:"100%",height:"100%",objectFit:"cover",loading:"lazy"}}/>:(p?.avatar||"🌱")}</div>;
  ...
  if(view==="thread"&&selThread) return(
    <div style={{maxWidth:"800px",margin:"0 auto",padding:"24px"}}>
      ...
      {/* Poll if exists */}
      {threadPoll&&<div style={{background:C.cardBg,borderRadius:"14px",border:`1px solid ${C.accent}44`,padding:"18px 20px",marginBottom:"20px"}}>
        <div style={{fontFamily:F.sans,fontSize:"15px",fontWeight:"700",marginBottom:"12px",display:"flex",alignItems:"center",gap:"8px"}}>📊 {threadPoll.question}{threadPoll.closed&&<span style={{fontSize:"11px",color:C.dim,fontWeight:"400"}}>(encerrada)</span>}</div>
        {threadPollOpts.map(o=>{
          const totalVotes=threadPollOpts.reduce((s,x)=>s+(x.votes_count||0),0);
          const pct=totalVotes>0?Math.round((o.votes_count||0)/totalVotes*100):0;
          const isMyVote=threadPollVote===o.id;
          const showResults=!!threadPollVote||threadPoll.closed;
          return <div key={o.id} style={{marginBottom:"6px"}}>
            {showResults?<div style={{position:"relative",borderRadius:"8px",overflow:"hidden",border:`1px solid ${isMyVote?C.accent:C.border}`,background:C.surface2}}>
              <div style={{position:"absolute",top:0,left:0,bottom:0,width:`${pct}%`,background:isMyVote?C.accent+"22":C.border+"44",transition:"width .3s"}}/>
              <div style={{position:"relative",padding:"8px 12px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <span style={{fontFamily:F.sans,fontSize:"13px",fontWeight:isMyVote?"600":"400"}}>{isMyVote?"✓ ":""}{o.label}</span>
                <span style={{fontFamily:F.sans,fontSize:"12px",color:C.dim,fontWeight:"600"}}>{pct}%</span>
              </div>
            </div>:
            <button onClick={async()=>{
              if(threadPollVote||threadPoll.closed||!user)return;
              try{
                await sb.from("forum_poll_votes").insert({poll_id:threadPoll.id,option_id:o.id,user_id:user.id});
                setThreadPollVote(o.id);
                setThreadPollOpts(p=>p.map(x=>x.id===o.id?{...x,votes_count:(x.votes_count||0)+1}:x));
                toast("Voto registrado!");
              }catch(e){if(e.message?.includes("unique"))toast("Você já votou nesta enquete.");}
            }} style={{width:"100%",padding:"10px 14px",borderRadius:"8px",border:`1px solid ${C.border}`,background:C.cardBg,cursor:"pointer",fontFamily:F.sans,fontSize:"13px",color:C.text,textAlign:"left",transition:"all .15s"}} onMouseOver={e=>e.currentTarget.style.background=C.surface2} onMouseOut={e=>e.currentTarget.style.background=C.cardBg}>{o.label}</button>}
          </div>;
        })}
        <div style={{fontFamily:F.sans,fontSize:"11px",color:C.dim,marginTop:"6px"}}>{threadPollOpts.reduce((s,x)=>s+(x.votes_count||0),0)} voto{threadPollOpts.reduce((s,x)=>s+(x.votes_count||0),0)!==1?"s":""}</div>
      </div>}
      ...
      <div style={{marginTop:"16px",background:C.cardBg,borderRadius:"12px",border:"1px solid "+C.border,padding:"16px"}}>
        ...
        <div style={{position:"relative",marginBottom:"10px"}}>
          <textarea ref={replyTextRef} style={{...baseInput,minHeight:"100px",resize:"vertical",width:"100%"}} value={replyText} onChange={e=>setReplyText(e.target.value)} placeholder="Escreva sua resposta" aria-label="Escreva sua resposta... **negrito** *itálico* > citação @username"/>
    ...
    <div style={{maxWidth:"800px",margin:"0 auto",padding:"24px"}}>
      ...
      {showNewThread&&<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.4)",backdropFilter:"blur(8px)",display:"flex",alignItems:"flex-start",justifyContent:"center",zIndex:300,padding:"40px 20px",overflowY:"auto"}} onClick={()=>setShowNewThread(false)}>
        <div onClick={e=>e.stopPropagation()} style={{background:C.surface,borderRadius:"16px",border:"1px solid "+C.border,padding:"24px",width:"100%",maxWidth:"600px"}}>
          ...
          {(newTitle||newContent)&&!posting&&<div style={{padding:"6px 12px",borderRadius:"8px",background:C.accentBg,fontFamily:F.sans,fontSize:"11px",color:C.accent,marginBottom:"12px"}}>💾 Rascunho recuperado</div>}
          <div style={{marginBottom:"12px"}}><label style={labelSt}>Assunto *</label><input style={baseInput} value={newTitle} onChange={e=>setNewTitle(e.target.value)} placeholder="Título do tópico" aria-label="Título do tópico"/></div>
          ...
          {/* Poll toggle */}
          <div style={{marginBottom:"16px"}}>
            <label style={{display:"flex",alignItems:"center",gap:"8px",cursor:"pointer",fontFamily:F.sans,fontSize:"13px",color:C.muted}}>
              <input type="checkbox" checked={pollEnabled} onChange={e=>setPollEnabled(e.target.checked)} style={{accentColor:C.accent}}/>
              📊 Adicionar enquete
            </label>
            {pollEnabled&&<div style={{marginTop:"10px",padding:"14px",borderRadius:"10px",border:`1px solid ${C.border}`,background:C.surface2}}>
              <input placeholder="Pergunta da enquete" value={pollQuestion} onChange={e=>setPollQuestion(e.target.value)} style={{...baseInput,marginBottom:"8px",fontSize:"13px"}}/>
              {pollOptions.map((opt,i)=>(
                <div key={i} style={{display:"flex",gap:"6px",marginBottom:"6px"}}>
                  <input placeholder={`Opção ${i+1}`} value={opt} onChange={e=>{const n=[...pollOptions];n[i]=e.target.value;setPollOptions(n);}} style={{...baseInput,flex:1,fontSize:"12px"}}/>
                  {pollOptions.length>2&&<button onClick={()=>setPollOptions(p=>p.filter((_,j)=>j!==i))} style={{border:"none",background:"none",color:C.error,cursor:"pointer",fontSize:"14px"}}>✕</button>}
                </div>
              ))}
              {pollOptions.length<6&&<button onClick={()=>setPollOptions(p=>[...p,""])} style={{border:`1px dashed ${C.border}`,background:"none",color:C.accent,cursor:"pointer",fontSize:"12px",fontFamily:F.sans,padding:"6px 12px",borderRadius:"6px",width:"100%"}}>+ Opção</button>}
            </div>}
          </div>
  ...
  return(
    <div style={{maxWidth:"800px",margin:"0 auto",padding:"24px"}}>
      ...
      <p style={{fontFamily:F.sans,fontSize:"14px",color:C.muted,margin:"0 0 16px"}}>Fóruns de discussão da comunidade</p>
      
      {/* Search bar */}
      <div style={{position:"relative",marginBottom:"20px"}}>
        <input value={forumSearchQ} onChange={e=>setForumSearchQ(e.target.value)} placeholder="Buscar tópicos no fórum..." aria-label="Buscar no fórum" style={{...baseInput,paddingLeft:"14px",paddingRight:"36px"}}/>
        {forumSearchQ&&<button onClick={()=>{setForumSearchQ("");setForumSearchResults([]);}} style={{position:"absolute",right:"10px",top:"50%",transform:"translateY(-50%)",background:"none",border:"none",color:C.dim,cursor:"pointer",fontSize:"14px"}}>✕</button>}
      </div>
      
      {/* Search results */}
      {forumSearchQ.length>=2&&(
        forumSearching?<div style={{textAlign:"center",padding:"20px",color:C.dim,fontSize:"13px",fontFamily:F.sans}}>Buscando...</div>:
        forumSearchResults.length>0?<div style={{display:"flex",flexDirection:"column",gap:"8px",marginBottom:"24px"}}>
          <div style={{fontFamily:F.sans,fontSize:"12px",color:C.dim,marginBottom:"4px"}}>{forumSearchResults.length} resultado{forumSearchResults.length!==1?"s":""} para "{forumSearchQ}"</div>
          {forumSearchResults.map(t=>(
            <div key={t.id} onClick={async()=>{
              try{
                const full=await sb.from("forum_threads").select("*,profiles(username,avatar,avatar_url)",`&id=eq.${t.id}&limit=1`);
                if(full?.[0]){setSelThread(full[0]);setView("thread");
                  const r=await sb.from("forum_replies").select("*,profiles(username,avatar,avatar_url)",`&thread_id=eq.${t.id}&order=created_at.asc`);
                  setReplies(r||[]);
                }
              }catch{}
              setForumSearchQ("");setForumSearchResults([]);
            }} style={{padding:"12px 16px",borderRadius:"12px",border:`1px solid ${C.border}`,background:C.cardBg,cursor:"pointer"}} onMouseOver={e=>e.currentTarget.style.background=C.surface2} onMouseOut={e=>e.currentTarget.style.background=C.cardBg}>
              <div style={{fontFamily:F.sans,fontSize:"14px",fontWeight:"600",marginBottom:"2px"}}>{t.title}</div>
              <div style={{fontFamily:F.sans,fontSize:"12px",color:C.dim}}>{(t.content||"").substring(0,100)}{(t.content||"").length>100?"...":""} · ❤️ {t.likes_count||0} · 💬 {t.reply_count||0}</div>
            </div>
          ))}
        </div>:<div style={{textAlign:"center",padding:"16px",color:C.dim,fontSize:"13px",fontFamily:F.sans,marginBottom:"16px"}}>Nenhum resultado para "{forumSearchQ}"</div>
      )}
...
// ─── Comparação de Cepas ───
function StrainsPage({onBack}){
  const [strains,setStrains]=useState([]);
  const [loading,setLoading]=useState(true);
  const [selected,setSelected]=useState([]); // up to 3 strain names
  const [searchQ,setSearchQ]=useState("");

  useEffect(()=>{
    (async()=>{
      try{
        const r=await sb.rpc("strain_stats");
        setStrains(r||[]);
      }catch{}
      setLoading(false);
    })();
  ...
  const filtered=searchQ.length>=1?strains.filter(s=>s.strain.toLowerCase().includes(searchQ.toLowerCase())):strains;
  const toggleSelect=(name)=>{
    setSelected(p=>p.includes(name)?p.filter(n=>n!==name):(p.length<3?[...p,name]:p));
  };
  const compareData=selected.map(name=>strains.find(s=>s.strain===name)).filter(Boolean);

  return(
    <div style={{maxWidth:"800px",margin:"0 auto",padding:"24px"}}>
      <h1 style={{fontFamily:F.sans,fontSize:"28px",fontWeight:"800",margin:"0 0 4px"}}>🧬 Genéticas</h1>
      <p style={{fontFamily:F.sans,fontSize:"14px",color:C.muted,margin:"0 0 20px"}}>Compare genéticas cultivadas pela comunidade. Selecione até 3 pra comparar.</p>

      {/* Search */}
      <input value={searchQ} onChange={e=>setSearchQ(e.target.value)} placeholder="Buscar genética..." aria-label="Buscar genética" style={{...baseInput,marginBottom:"16px"}}/>

      {/* Comparison panel */}
      {compareData.length>=2&&<div style={{background:C.cardBg,borderRadius:"16px",border:`1px solid ${C.accent}44`,padding:"20px",marginBottom:"24px",boxShadow:`0 4px 16px ${C.accent}11`}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"16px"}}>
          <h3 style={{fontFamily:F.sans,fontSize:"16px",fontWeight:"700",margin:0}}>📊 Comparação</h3>
          <button onClick={()=>setSelected([])} style={{padding:"4px 12px",borderRadius:"8px",border:`1px solid ${C.border}`,background:C.surface2,color:C.muted,cursor:"pointer",fontSize:"11px",fontFamily:F.sans}}>Limpar</button>
        </div>
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontFamily:F.sans,fontSize:"13px"}}>
            <thead><tr style={{borderBottom:`2px solid ${C.border}`}}>
              <th style={{padding:"8px",textAlign:"left",fontWeight:"600",color:C.dim}}>Métrica</th>
              {compareData.map(s=><th key={s.strain} style={{padding:"8px",textAlign:"center",fontWeight:"700"}}>{s.strain}</th>)}
            </tr></thead>
            <tbody>
              <tr style={{borderBottom:`1px solid ${C.border22}`}}>
                <td style={{padding:"8px",color:C.dim}}>📓 Diários</td>
                {compareData.map(s=><td key={s.strain} style={{padding:"8px",textAlign:"center",fontWeight:"600"}}>{s.total_diaries}</td>)}
              </tr>
              <tr style={{borderBottom:`1px solid ${C.border22}`}}>
                <td style={{padding:"8px",color:C.dim}}>📅 Semanas médias</td>
                {compareData.map(s=><td key={s.strain} style={{padding:"8px",textAlign:"center",fontWeight:"600"}}>{s.avg_weeks||"—"}</td>)}
              </tr>
              <tr style={{borderBottom:`1px solid ${C.border22}`}}>
                <td style={{padding:"8px",color:C.dim}}>❤️ Total curtidas</td>
                {compareData.map(s=><td key={s.strain} style={{padding:"8px",textAlign:"center",fontWeight:"600"}}>{s.total_likes}</td>)}
              </tr>
              <tr style={{borderBottom:`1px solid ${C.border22}`}}>
                <td style={{padding:"8px",color:C.dim}}>🏠 Ambientes</td>
                {compareData.map(s=><td key={s.strain} style={{padding:"8px",textAlign:"center",fontSize:"11px"}}>{(s.environments||[]).join(", ")||"—"}</td>)}
              </tr>
              <tr>
                <td style={{padding:"8px",color:C.dim}}>💡 Iluminação</td>
                {compareData.map(s=><td key={s.strain} style={{padding:"8px",textAlign:"center",fontSize:"11px"}}>{(s.lightings||[]).join(", ")||"—"}</td>)}
              </tr>
            </tbody>
          </table>
        </div>
      ...
      {/* Strain list */}
      {loading?<div style={{textAlign:"center",padding:"40px",color:C.dim}}>Carregando...</div>:
      filtered.length===0?<div style={{textAlign:"center",padding:"40px",color:C.dim,fontFamily:F.sans}}>Nenhuma genética encontrada.</div>:
      <div style={{display:"flex",flexDirection:"column",gap:"8px"}}>
        {filtered.map(s=>{
          const isSel=selected.includes(s.strain);
          return(
            <div key={s.strain} onClick={()=>toggleSelect(s.strain)} style={{display:"flex",alignItems:"center",gap:"12px",padding:"12px 16px",borderRadius:"12px",border:`1px solid ${isSel?C.accent:C.border}`,background:isSel?C.accentBg:C.cardBg,cursor:"pointer",transition:"all .15s"}} onMouseOver={e=>{if(!isSel)e.currentTarget.style.background=C.surface2;}} onMouseOut={e=>{if(!isSel)e.currentTarget.style.background=C.cardBg;}}>
              <div style={{width:"32px",height:"32px",borderRadius:"8px",background:isSel?C.accent:C.surface2,display:"flex",alignItems:"center",justifyContent:"center",color:isSel?"#fff":C.dim,fontSize:"14px",fontWeight:"700",flexShrink:0}}>{isSel?"✓":"🧬"}</div>
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontFamily:F.sans,fontSize:"14px",fontWeight:"600",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{s.strain}</div>
                <div style={{fontFamily:F.sans,fontSize:"11px",color:C.dim}}>{s.total_diaries} {s.total_diaries===1?"diário":"diários"} · ❤️ {s.total_likes} · ~{s.avg_weeks} sem</div>
              </div>
            </div>
          );
        })}
...
function CalculatorsPage({onBack}){
  const [active,setActive]=useState(null);
  // VPD calc state
  const [leafTemp,setLeafTemp]=useState(25);
  const [airTemp,setAirTemp]=useState(26);
  const [humidity,setHumidity]=useState(55);
  // DLI calc state
  const [ppfd,setPpfd]=useState(600);
  const [photoperiod,setPhotoperiod]=useState(18);
  // pH/EC state
  const [phIn,setPhIn]=useState(6.5);
  const [ecIn,setEcIn]=useState(1.2);
  // Nutrient dilution
  const [stockConc,setStockConc]=useState(100);
  const [targetConc,setTargetConc]=useState(2);
  const [volume,setVolume]=useState(10);

  // VPD formula
  const svp=(t)=>0.6108*Math.exp((17.27*t)/(t+237.3));
  const vpd_val=svp(leafTemp)-((humidity/100)*svp(airTemp));
  const vpdZone=vpd_val<0.4?"Muito baixo — risco de mofo":(vpd_val<0.8?"Propagação / Clones (ideal)":(vpd_val<1.2?"Vegetativo (ideal)":(vpd_val<1.6?"Floração (ideal)":"Muito alto — stress")));
  const vpdColor=vpd_val<0.4?"#3b82f6":(vpd_val<0.8?"#10b981":(vpd_val<1.2?"#22c55e":(vpd_val<1.6?"#f59e0b":"#ef4444")));

  // DLI formula
  const dli_val=(ppfd*photoperiod*3600)/1000000;
  const dliZone=dli_val<15?"Baixo — ideal pra clones/mudas":(dli_val<25?"Médio — vegetativo":(dli_val<40?"Alto — floração (ideal)":(dli_val<50?"Muito alto — máximo rendimento":"Excessivo — pode causar stress")));

  // Nutrient dilution
  const mlNeeded=(targetConc*volume)/stockConc*1000;

  const numInput=(val,set,min,max,step,unit)=>(
    <div style={{display:"flex",alignItems:"center",gap:"8px"}}>
      <input type="range" min={min} max={max} step={step} value={val} onChange={e=>set(Number(e.target.value))} style={{flex:1,accentColor:C.accent}}/>
      <span style={{fontFamily:F.sans,fontSize:"14px",fontWeight:"600",minWidth:"60px",textAlign:"right"}}>{val}{unit}</span>
    </div>
  );

  const card=(emoji,title,desc,id)=>(
    <div key={id} onClick={()=>setActive(id)} style={{background:C.cardBg,borderRadius:"14px",border:`1px solid ${active===id?C.accent:C.border}`,padding:"18px",cursor:"pointer",transition:"all .15s"}} onMouseOver={e=>e.currentTarget.style.transform="translateY(-2px)"} onMouseOut={e=>e.currentTarget.style.transform=""}>
      <div style={{fontSize:"28px",marginBottom:"8px"}}>{emoji}</div>
      <h3 style={{fontFamily:F.sans,fontSize:"15px",fontWeight:"700",margin:"0 0 4px"}}>{title}</h3>
      <p style={{fontFamily:F.sans,fontSize:"12px",color:C.muted,margin:0,lineHeight:"1.4"}}>{desc}</p>
    </div>
  );

  const sectionStyle={background:C.cardBg,borderRadius:"16px",border:`1px solid ${C.border}`,padding:"24px",marginBottom:"20px"};
  const labelStyle={fontFamily:F.sans,fontSize:"13px",fontWeight:"600",marginBottom:"6px",display:"block"};
  const resultStyle={padding:"16px",borderRadius:"12px",background:C.surface2,marginTop:"16px",textAlign:"center"};

  return(
    <div style={{maxWidth:"700px",margin:"0 auto",padding:"24px"}}>
      <h1 style={{fontFamily:F.sans,fontSize:"28px",fontWeight:"800",margin:"0 0 4px"}}>🧮 Calculadoras</h1>
      <p style={{fontFamily:F.sans,fontSize:"14px",color:C.muted,margin:"0 0 24px"}}>Ferramentas essenciais para otimizar seu cultivo</p>

      {/* Calculator cards */}
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:"12px",marginBottom:"24px"}}>
        {card("🌡️","VPD","Pressão de Déficit de Vapor — clima ideal para cada fase","vpd")}
        {card("☀️","DLI","Integral de Luz Diária — quantidade total de luz por dia","dli")}
        {card("💧","pH / EC","Referência rápida de pH e EC por fase de cultivo","phec")}
        {card("🧪","Diluição","Calculadora de diluição de nutrientes concentrados","dilution")}
      </div>

      {/* VPD Calculator */}
      {active==="vpd"&&<div style={sectionStyle}>
        <h2 style={{fontFamily:F.sans,fontSize:"18px",fontWeight:"700",margin:"0 0 16px"}}>🌡️ Calculadora de VPD</h2>
        <label style={labelStyle}>Temperatura do ar (°C)</label>
        {numInput(airTemp,setAirTemp,15,40,0.5,"°C")}
        <label style={{...labelStyle,marginTop:"12px"}}>Temperatura da folha (°C)</label>
        {numInput(leafTemp,setLeafTemp,15,40,0.5,"°C")}
        <label style={{...labelStyle,marginTop:"12px"}}>Umidade relativa (%)</label>
        {numInput(humidity,setHumidity,20,95,1,"%")}
        <div style={resultStyle}>
          <div style={{fontFamily:F.sans,fontSize:"36px",fontWeight:"800",color:vpdColor}}>{vpd_val.toFixed(2)} kPa</div>
          <div style={{fontFamily:F.sans,fontSize:"14px",color:vpdColor,fontWeight:"600",marginTop:"4px"}}>{vpdZone}</div>
          <div style={{display:"flex",justifyContent:"center",gap:"6px",marginTop:"12px",flexWrap:"wrap"}}>
            {[{l:"<0.4",c:"#3b82f6",t:"Mofo"},{l:"0.4-0.8",c:"#10b981",t:"Clones"},{l:"0.8-1.2",c:"#22c55e",t:"Veg"},{l:"1.2-1.6",c:"#f59e0b",t:"Flor"},{l:">1.6",c:"#ef4444",t:"Stress"}].map(z=>
              <span key={z.l} style={{padding:"3px 10px",borderRadius:"8px",background:z.c+"18",color:z.c,fontSize:"10px",fontFamily:F.sans,fontWeight:"600"}}>{z.l} {z.t}</span>
            )}
          </div>
        </div>
      </div>}
      ...
      {/* DLI Calculator */}
      {active==="dli"&&<div style={sectionStyle}>
        <h2 style={{fontFamily:F.sans,fontSize:"18px",fontWeight:"700",margin:"0 0 16px"}}>☀️ Calculadora de DLI</h2>
        <label style={labelStyle}>PPFD (µmol/m²/s)</label>
        {numInput(ppfd,setPpfd,50,2000,10,"µmol")}
        <label style={{...labelStyle,marginTop:"12px"}}>Fotoperíodo (horas/dia)</label>
        {numInput(photoperiod,setPhotoperiod,6,24,1,"h")}
        <div style={resultStyle}>
          <div style={{fontFamily:F.sans,fontSize:"36px",fontWeight:"800",color:C.accent}}>{dli_val.toFixed(1)} mol/m²/d</div>
          <div style={{fontFamily:F.sans,fontSize:"14px",color:C.muted,fontWeight:"600",marginTop:"4px"}}>{dliZone}</div>
          <div style={{marginTop:"12px",fontFamily:F.sans,fontSize:"12px",color:C.dim}}>
            <div>Mudas/Clones: 12-18 mol · Vegetativo: 18-30 mol</div>
            <div>Floração: 30-45 mol · Máximo: 45-65 mol</div>
          </div>
        </div>
      </div>}

      {/* pH/EC Reference */}
      {active==="phec"&&<div style={sectionStyle}>
        <h2 style={{fontFamily:F.sans,fontSize:"18px",fontWeight:"700",margin:"0 0 16px"}}>💧 Referência pH / EC</h2>
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontFamily:F.sans,fontSize:"13px"}}>
            <thead><tr style={{borderBottom:`2px solid ${C.border}`}}>
              <th style={{padding:"8px",textAlign:"left",fontWeight:"700"}}>Fase</th>
              <th style={{padding:"8px",textAlign:"center",fontWeight:"700"}}>pH (Solo)</th>
              <th style={{padding:"8px",textAlign:"center",fontWeight:"700"}}>pH (Hidro)</th>
              <th style={{padding:"8px",textAlign:"center",fontWeight:"700"}}>EC (mS/cm)</th>
            </tr></thead>
            <tbody>
              {[
                {phase:"Germinação",phSoil:"6.0 - 6.5",phHydro:"5.5 - 6.0",ec:"0.2 - 0.4"},
                {phase:"Muda/Clone",phSoil:"6.0 - 6.5",phHydro:"5.5 - 6.0",ec:"0.4 - 0.8"},
                {phase:"Vegetativo (início)",phSoil:"6.0 - 6.8",phHydro:"5.5 - 6.0",ec:"0.8 - 1.2"},
                {phase:"Vegetativo (pleno)",phSoil:"6.0 - 7.0",phHydro:"5.5 - 6.5",ec:"1.2 - 1.6"},
                {phase:"Pré-Floração",phSoil:"6.0 - 7.0",phHydro:"5.5 - 6.5",ec:"1.2 - 1.8"},
                {phase:"Floração (início)",phSoil:"6.0 - 7.0",phHydro:"5.5 - 6.5",ec:"1.4 - 2.0"},
                {phase:"Floração (plena)",phSoil:"6.0 - 7.0",phHydro:"6.0 - 6.5",ec:"1.6 - 2.2"},
                {phase:"Flush final",phSoil:"6.0 - 6.5",phHydro:"5.5 - 6.0",ec:"0.0 - 0.4"},
              ].map((r,i)=><tr key={i} style={{borderBottom:`1px solid ${C.border22}`}}>
                <td style={{padding:"8px",fontWeight:"600"}}>{r.phase}</td>
                <td style={{padding:"8px",textAlign:"center"}}>{r.phSoil}</td>
                <td style={{padding:"8px",textAlign:"center"}}>{r.phHydro}</td>
                <td style={{padding:"8px",textAlign:"center"}}>{r.ec}</td>
              </tr>)}
            </tbody>
          </table>
        </div>
        <div style={{marginTop:"12px",fontFamily:F.sans,fontSize:"11px",color:C.dim,lineHeight:"1.5"}}>
          💡 Em solo, o pH ideal fica entre 6.0-7.0. Em hidroponia/coco, entre 5.5-6.5. EC varia conforme genética e tamanho da planta — comece baixo e aumente gradualmente.
        </div>
      </div>}

      {/* Dilution Calculator */}
      {active==="dilution"&&<div style={sectionStyle}>
        <h2 style={{fontFamily:F.sans,fontSize:"18px",fontWeight:"700",margin:"0 0 16px"}}>🧪 Calculadora de Diluição</h2>
        <label style={labelStyle}>Concentração do produto (ml/L ou g/L)</label>
        {numInput(stockConc,setStockConc,1,500,1,"")}
        <label style={{...labelStyle,marginTop:"12px"}}>Concentração desejada (ml/L)</label>
        {numInput(targetConc,setTargetConc,0.1,20,0.1,"")}
        <label style={{...labelStyle,marginTop:"12px"}}>Volume de água (litros)</label>
        {numInput(volume,setVolume,1,100,1,"L")}
        <div style={resultStyle}>
          <div style={{fontFamily:F.sans,fontSize:"14px",color:C.muted,marginBottom:"4px"}}>Adicione à água:</div>
          <div style={{fontFamily:F.sans,fontSize:"36px",fontWeight:"800",color:C.accent}}>{mlNeeded.toFixed(1)} ml</div>
          <div style={{fontFamily:F.sans,fontSize:"12px",color:C.dim,marginTop:"8px"}}>({targetConc} ml/L × {volume} L) ÷ {stockConc} ml/L × 1000</div>
        </div>
      </div>}
    </div>
  );
...
function NavBar({ user, page, setPage, setShowCreate, myDiaries, onLogout, onNavigate, lang, setLang, unreadNotifs, unreadMsgs, notifs, onMarkNotifsRead, dark, onToggleDark }) {
  ...
  const sidebarItems=[
    ...
    {icon:"🧬",label:"Genéticas",action:()=>nav("geneticas"),active:page==="geneticas"},
    {icon:"🧮",label:"Calculadoras",action:()=>nav("calculadoras"),active:page==="calculadoras"},
  ...
  return (
    <>
      ...
      `}</style>
      <nav role="navigation" aria-label="Menu principal" style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 16px",borderBottom:`1px solid ${C.border}`,backdropFilter:"blur(20px)",background:"var(--dp-overlay85)",position:"sticky",top:0,zIndex:100,gap:"8px"}}>
        {/* Left: hamburger + logo */}
  ...
  const doReset=async()=>{
    ...
    try{
      ...
      if(res.ok){setDone(true);setMsg("Senha alterada com sucesso!");}
        ...
        if(d.error_code==="same_password"||/different from the old/i.test(d.msg||"")){
          setMsg("A nova senha precisa ser diferente da anterior.");
        }else if(d.error_code==="weak_password"||/weak/i.test(d.msg||"")){
          setMsg("Senha muito fraca. Use letras, números e símbolos.");
        }else if(res.status===401||res.status===403){
          setMsg("O link expirou. Solicite um novo email de recuperação.");
        }else{
          setMsg(d.msg||d.error_description||"Erro ao alterar senha. Tente novamente.");
        }
      }
...
function AppInner() {
  ...
  useEffect(()=>{
    _toastHandler=(msg,ms=3000)=>{setGlobalToast(msg);setTimeout(()=>setGlobalToast(""),ms);};
  ...
  const [dark,setDark]=useState(()=>{
    try{
      ...
      // No user preference stored — follow OS
      return window.matchMedia?.("(prefers-color-scheme: dark)")?.matches||false;
    }catch{return false;}
  });
  // Listen for OS theme changes (only when user hasn't manually toggled)
  useEffect(()=>{
    const mq=window.matchMedia?.("(prefers-color-scheme: dark)");
    if(!mq)return;
    const handler=(e)=>{
      try{if(localStorage.getItem("dp-dark")===null)setDark(e.matches);}catch{}
    };
  ...
  useEffect(()=>{
    ...
    (async()=>{
      try{
        const res=await fetch(`${SB_URL}/rest/v1/rpc/global_search`,{method:"POST",headers:{...sbHeaders(),"Content-Type":"application/json"},body:JSON.stringify({q:debouncedSearch,lim:5})});
        if(!res.ok)return;
        const data=await res.json();
  ...
  useEffect(()=>{
    ...
    const poll=async()=>{
      try{
        ...
        const mapped=n.map(x=>({id:x.id,type:x.type,from:x.from_username,avatar:x.from_avatar,text:x.text,diary:x.diary_name,threadId:x.thread_id,time:new Date(x.created_at).getTime(),read:x.read}));
    ...
    // Smart polling: 10s when tab is active, paused when hidden, immediate refresh on focus
    let interval=null;
    const startPolling=()=>{if(!interval)interval=setInterval(poll,10000);};
    ...
    return()=>{stopPolling();document.removeEventListener("visibilitychange",onVisibility);window.removeEventListener("focus",poll);};
  },[user?.id]);
  ...
  useEffect(()=>{if(!user)return;(async()=>{try{
    ...
    if(convIds.length>0){
      ...
      for(const cr of convRows){
        ...
        const members=await sb.from("conversation_members").select("user_id,read_at,profiles(username,avatar,avatar_url)",`&conversation_id=eq.${c.id}`);
        const msgRows=await sb.from("messages").select("id,text,media_url,media_type,forwarded,created_at,sender_id,is_system,report_number,profiles(username)",`&conversation_id=eq.${c.id}&order=created_at.desc&limit=50`);
        const sortedMsgs=msgRows.reverse();
        const other=members.find(m=>m.user_id!==user.id);
        const otherReadAt=!c.is_group&&other?.read_at?new Date(other.read_at).getTime():null;
        // Collect paths for batch signing
        if(other?.profiles?.avatar_url) { const p=extractStoragePath(other.profiles.avatar_url); if(p) allPathsToSign.push(p); }
        sortedMsgs.forEach(m=>{ if(m.media_url) { const p=extractStoragePath(m.media_url); if(p) allPathsToSign.push(p); } });
        convList.push({cr,c,members,sortedMsgs,other,otherReadAt});
      }
      // Batch sign all paths at once
      if(allPathsToSign.length>0) await sbStorage.signBatch([...new Set(allPathsToSign)]);
      // Now map with r() — cache is populated
      const finalList=convList.map(({cr,c,sortedMsgs,other,otherReadAt,members})=>({
        id:c.id,
        with:c.is_group?c.group_name:(other?.profiles?.username||"Usuário"),
        avatar:c.is_group?"👥":(other?.profiles?.avatar||"🌱"),
        avatarImg:!c.is_group&&other?.profiles?.avatar_url?r(other.profiles.avatar_url):null,
        isGroup:c.is_group,
        members:c.is_group?members.map(m=>m.profiles?.username).filter(Boolean):[],
        readAt:cr.read_at?new Date(cr.read_at).getTime():null,
        otherReadAt,
        hasMore:sortedMsgs.length>=50,
        messages:sortedMsgs.map(m=>({id:m.id,from:m.sender_id===user.id?user.email:(m.profiles?.username||m.sender_id),text:m.text||"",media:m.media_url?{type:m.media_type||"image",data:r(m.media_url)}:null,time:new Date(m.created_at).getTime(),forwarded:m.forwarded,isSystem:!!m.is_system,reportNumber:m.report_number||null})),
      }));
      setMsgs(finalList);
    }
  }catch{}})();},[user]);
  ...
  const loadOlderMessages=async(convId)=>{
    ...
    if(!oldest)return;
    try{
      const olderRows=await sb.from("messages").select("id,text,media_url,media_type,forwarded,created_at,sender_id,is_system,report_number,profiles(username)",`&conversation_id=eq.${convId}&created_at=lt.${new Date(oldest.time).toISOString()}&order=created_at.desc&limit=50`);
      if(!olderRows||olderRows.length===0){
        setMsgs(p=>p.map(c=>c.id===convId?{...c,hasMore:false}:c));
        return;
      }
      await signMediaUrls(olderRows,"media_url");
      const mapped=olderRows.reverse().map(m=>({id:m.id,from:m.sender_id===user.id?user.email:(m.profiles?.username||m.sender_id),text:m.text||"",media:m.media_url?{type:m.media_type||"image",data:r(m.media_url)}:null,time:new Date(m.created_at).getTime(),forwarded:m.forwarded,isSystem:!!m.is_system,reportNumber:m.report_number||null}));
      setMsgs(p=>p.map(c=>c.id===convId?{...c,messages:[...mapped,...c.messages],hasMore:olderRows.length>=50}:c));
    }catch(e){reportError(e,{feature:"messages",op:"loadOlder"});}
  };
  ...
  const createGroup=async(name,members)=>{
    ...
    try{
      ...
      toast("Erro ao criar grupo: "+(e.message||"desconhecido"));
    }
  };
  const newDM=async(username,firstMsg,media)=>{
    if(!user)return;
    const cleanMsg=sanitize(firstMsg||"",2000);
    if(!cleanMsg&&!media){toast("Mensagem vazia.");return;}
    const cleanUser=sanitize(username,30).trim();
    if(!cleanUser){toast("Username inválido.");return;}
    ...
    try{
      ...
      if(!target){toast(`Usuário "${cleanUser}" não encontrado.`);return;}
      if(target.id===user.id){toast("Você não pode enviar mensagem para si mesmo.");return;}
      
      // Upload media first if provided (before the RPC, so we can pass the URL)
      ...
      if(media&&media.data&&media.data.startsWith("data:")){
        ...
        if(ok){mediaUrl=sbStorage.getUrl(path);mediaType=media.type;}
      }
      ...
      if(!convId){toast("Erro ao criar conversa. Tente novamente.");return;}
      ...
      if(mediaUrl)optimisticMsgs.push({id:"m"+(Date.now()+1),from:user.email,text:"",media:{type:mediaType,data:mediaUrl},time:Date.now()+1});
      setMsgs(p=>[{
        id:convId,
        with:target.username,
        avatar:target.avatar||"🌱",
        avatarImg:r(target.avatar_url)||null,
        isGroup:false,
        members:[],
      ...
      toast("Erro ao enviar mensagem: "+(e.message||"desconhecido"));
    }
  };
  ...
  useEffect(()=>{
    // Parse ?diary=<id> or ?contest=<id> or share_target params from URL at mount
    try{
      const qs=new URLSearchParams(window.location.search||"");
      const did=qs.get("diary");
      const cid=qs.get("contest");
      // Share target API (PWA receives shared content)
      const shareText=qs.get("share_text")||qs.get("share_title")||"";
      const shareUrl=qs.get("share_url")||"";
      if(did) setPendingDiaryId(did);
      if(cid) setPendingContestId(cid);
      if(shareText||shareUrl) setSharedContent((shareText+(shareUrl?" "+shareUrl:"")).trim());
      if(did||cid||shareText||shareUrl){
  ...
  // When shared content arrives, navigate to messages so user can forward it
  useEffect(()=>{
    if(!sharedContent||!user)return;
    setPage("mensagens");
    toast("Conteúdo compartilhado! Escolha uma conversa pra enviar.",4000);
  },[sharedContent,user]);

  // Open pending contest once user is logged in
  useEffect(()=>{
    if(!pendingContestId||!user)return;
    setPage("concursos");
    setPendingContestId(null);
  },[pendingContestId,user]);
  ...
  useEffect(()=>{
    const handler=(e)=>{
      ...
      if(e.key==="/"&&!e.ctrlKey&&!e.metaKey){e.preventDefault();setShowGlobalSearch(true);}
      if(e.key==="Escape"){setShowGlobalSearch(false);setViewImage(null);setViewVideo(null);}
    };
  ...
  useEffect(()=>{
    const handler=()=>{
      ...
      setTimeout(()=>{toast("Sua sessão expirou. Por favor, faça login novamente.",5000);},100);
    };
      ...
      try{
        ...
        if(d){
          setSelectedDiary({
            ...
            avatar:d.profiles?.avatar,avatarImg:r(d.profiles?.avatar_url),
            ...
            cover:0,coverImage:r(d.cover_url),hidden:d.hidden,isOwn:false,weeks:[],
          });
        }else{
          toast("Diário não encontrado ou foi removido.");
        }
    ...
    try{
      ...
      if(authUser?.id){
        ...
        if(profile&&!profile.banned){
          await preSign(profile.avatar_url);
          setUser({...profile,avatarImg:r(profile.avatar_url),createdAt:profile.created_at,authId:authUser.id});
        }
      }
    }catch{}
    setAuthLoading(false);
  ...
  useEffect(()=>{if(!user)return;(async()=>{try{
    ...
    // Pre-sign all media URLs before mapping to state
    await signMediaUrls(rows,"cover_url");
    const mapped=rows.map(d=>({
      ...
      likes:d.likes_count,comments:d.comments_count,cover:0,coverImage:r(d.cover_url),hidden:d.hidden,isOwn:true,
      weeks:[], // loaded on demand
    }));
    setMyDiaries(mapped);
  }catch{}setDataLoaded(true);})();},[user]);

  // ─── Load public diaries from all users ───
  useEffect(()=>{if(!user)return;(async()=>{
    try{
      const rows=await sb.from("diaries").select("*,profiles(username,avatar,avatar_url)",`&hidden=eq.false&user_id=neq.${user.id}&order=created_at.desc&limit=100`);
      // Pre-sign all media URLs
      await signMediaUrls(rows||[],"cover_url","profiles.avatar_url");
      const mapped=(rows||[]).map(d=>({
        id:d.id,name:d.name,strain:d.strain,strains:d.strains||[],
        author:d.profiles?.username||"",authorId:d.user_id,
        avatar:d.profiles?.avatar||"🌱",avatarImg:r(d.profiles?.avatar_url)||null,
        phase:d.phase,week:d.current_week,env:d.environment,light:d.lighting,watts:d.watts,substrate:d.substrate,
        watering:d.watering,germination:d.germination,techniques:d.techniques||[],numPlants:d.num_plants,tags:d.tags||[],
        likes:d.likes_count,comments:d.comments_count,cover:0,coverImage:r(d.cover_url),hidden:false,isOwn:false,
        weeks:[],
      }));
      setPublicDiaries(mapped);
  ...
  const doLogout=async()=>{
    ...
    setUser(null);setMyDiaries([]);setPublicDiaries([]);setNotifs([]);setMsgs([]);setLikes({});setFavs({});setCommentsMap({});setBlockedUsers([]);setFollows([]);setDataLoaded(false);setSelectedDiary(null);setPublicProfile(null);setPage("home");
  };
  ...
  const doFollow=async(targetId)=>{
    ...
    if(!actionRateLimit("follow",20,60000)){toast("Aguarde um momento antes de seguir mais pessoas.");return;}
    try{await sb.from("follows").insert({follower_id:user.id,following_id:targetId});setFollows(p=>[...p,targetId]);
      await notify({user_id:targetId,type:"follow",from_username:user.username,from_avatar:user.avatar,text:`${user.username} começou a te seguir`});
        ...
        if(isUUID(w.id)){
          ...
          try{
            ...
            if(row?.id){
              ...
              try{await sb.rpc("update_user_streak",{p_user_id:user.id});}catch{}
            }
  ...
  const doLike=async(id)=>{
    if(!user)return;
    if(!(await serverRateLimit("like",30,60))){toast("Aguarde um momento antes de curtir novamente.");return;}
    ...
    setMyDiaries(p=>p.map(d=>d.id===id?{...d,likes:Math.max((d.likes||0)+delta,0)}:d));
    setPublicDiaries(p=>p.map(d=>d.id===id?{...d,likes:Math.max((d.likes||0)+delta,0)}:d));
    if(selectedDiary?.id===id)setSelectedDiary(sd=>({...sd,likes:Math.max((sd.likes||0)+delta,0)}));
  };
  ...
  const doReportGeneric=async({targetType,targetId,reason,targetName})=>{
    if(!user)return null;
    if(!(await serverRateLimit("report",5,300))){toast("Muitas denúncias recentes. Aguarde alguns minutos.");return null;}
    ...
    try{
      ...
      toast(num?`Denúncia #${num} registrada. Nossa equipe vai revisar.`:"Denúncia registrada. Nossa equipe vai revisar.",4000);
      return num||null;
    }catch(e){reportError(e,{feature:"report",op:"create"});toast("Erro ao registrar denúncia. Tente novamente.");return null;}
  };
  ...
  useEffect(()=>{if(!user)return;(async()=>{
    try{const bk=await sb.from("blocked_users").select("blocked_id,profiles:blocked_id(username,avatar,avatar_url)",`&blocker_id=eq.${user.id}`);setBlockedUsers(bk.map(b=>({id:b.blocked_id,username:b.profiles?.username||"",avatar:b.profiles?.avatar||"🌱",avatarImg:r(b.profiles?.avatar_url)||null})));}catch{}
  })();},[user]);
  ...
  const loadComments=async(diaryId)=>{
    try{
      ...
      const mapped=rows.map(c=>({id:c.id,text:c.text,username:c.profiles?.username,avatar:c.profiles?.avatar,avatarImg:r(c.profiles?.avatar_url),authorEmail:c.user_id,time:new Date(c.created_at).getTime(),editedAt:c.edited_at?new Date(c.edited_at).getTime():null,parentId:c.parent_id}));
  ...
  const doAddComment=async(diaryId,text,parentId)=>{
    ...
    if(!(await serverRateLimit("comment",10,60))){toast("Aguarde um momento antes de comentar novamente.");return;}
    ...
    try{
      ...
      if(diary?.authorId&&diary.authorId!==user.id){
        await notify({user_id:diary.authorId,type:"comment",from_username:user.username,from_avatar:user.avatar,text:parentId?"respondeu um comentário":"comentou no seu diário",diary_name:diary.name});
      }
      ...
      if(mentions.length>0){
        try{
          ...
          for(const u of(mentioned||[])){
            if(u.id!==user.id) await notify({user_id:u.id,type:"comment",from_username:user.username,from_avatar:user.avatar,text:`mencionou você: ${clean.substring(0,80)}`,diary_name:diary?.name||""});
          }
        }catch{}
      }
      try{await sb.rpc("check_and_grant_badges",{uid:user.id});}catch{}
  ...
  const doBlockUser=async(blockedId)=>{
    if(!user||blockedUsers.some(b=>b.id===blockedId))return;
    try{
      await sb.from("blocked_users").insert({blocker_id:user.id,blocked_id:blockedId});
      // Fetch profile for display
      const p=await sb.from("profiles").select("username,avatar,avatar_url",`&id=eq.${blockedId}&limit=1`);
      setBlockedUsers(prev=>[...prev,{id:blockedId,username:p?.[0]?.username||"",avatar:p?.[0]?.avatar||"🌱",avatarImg:r(p?.[0]?.avatar_url)||null}]);
      toast("Usuário bloqueado.");
    }catch{}
  };
  const doUnblockUser=async(blockedId)=>{
    if(!user)return;
    try{await sb.from("blocked_users").delete(`blocker_id=eq.${user.id}&blocked_id=eq.${blockedId}`);setBlockedUsers(p=>p.filter(b=>b.id!==blockedId));toast("Usuário desbloqueado.");}catch{}
  };
  ...
  const doLikeWithNotif=async(id)=>{
    ...
    if(!wasLiked){
      ...
      if(diary?.authorId&&diary.authorId!==user?.id){
        try{await notify({user_id:diary.authorId,type:"like",from_username:user.username,from_avatar:user.avatar,text:"curtiu seu diário",diary_name:diary.name});}catch{}
      }
    }
  };
  ...
  const openPublicProfile=async(targetId)=>{
    ...
    try{
      ...
      const mapped=diaries.map(d=>({id:d.id,name:d.name,strain:d.strain,strains:d.strains||[],author:target.username,authorId:target.id,avatar:target.avatar,avatarImg:r(target.avatar_url),phase:d.phase,week:d.current_week,env:d.environment,light:d.lighting,likes:d.likes_count,comments:d.comments_count,cover:0,coverImage:r(d.cover_url),hidden:false,techniques:d.techniques||[],weeks:[]}));
      setPublicProfile({user:{...target,avatarImg:r(target.avatar_url)},diaries:mapped});
    }catch{}
  };
      ...
      try{
        ...
        if(weekRows.length>0){
          ...
          if(weekIds.length>0){
            ...
            // Pre-sign all media URLs
            await signMediaUrls(mediaRows,"media_url");
            mediaRows.forEach(m=>{if(!mediaMap[m.week_id])mediaMap[m.week_id]=[];mediaMap[m.week_id].push({id:m.id,name:"",type:m.media_type==="video"?"video":"photo",data:r(m.media_url)});});
          }
  ...
  if(recoveryMode){
    return <RecoveryForm dark={dark} onDone={()=>{setRecoveryMode(false);setTokens(null,null);setUser(null);}}/>;
  }
  ...
  const renderPage=()=>{
    if(publicProfile) return <PublicProfile targetUser={publicProfile.user} diaries={publicProfile.diaries} onBack={()=>setPublicProfile(null)} onViewDiary={d=>{setPublicProfile(null);setPage("home");setSelectedDiary(d);}} lang={lang} allBadges={allBadges} onReport={(uid,name)=>openReport("user",uid,name)} currentUserId={user?.id} onBlock={doBlockUser} onUnblock={doUnblockUser} isBlocked={blockedUsers.some(b=>b.id===publicProfile.user?.id)}/>;
    ...
    if(page==="cultivadores") return <GrowersPage user={user} onBack={()=>setPage("home")} follows={follows} onFollow={doFollow} onUnfollow={doUnfollow} onReport={(uid,name)=>openReport("user",uid,name)} onViewProfile={async(g)=>{try{const diaries=await sb.from("diaries").select("*",`&user_id=eq.${g.id}&hidden=eq.false&order=created_at.desc`);setPublicProfile({user:{...g,username:g.username,avatar:g.avatar,avatarImg:r(g.avatar_url),bio:g.bio,city:g.city,createdAt:g.created_at},diaries:diaries.map(d=>({id:d.id,name:d.name,strain:d.strain,strains:d.strains||[],author:g.username,authorId:d.user_id,avatar:g.avatar,avatarImg:r(g.avatar_url),phase:d.phase,week:d.current_week,env:d.environment,light:d.lighting,watts:d.watts,substrate:d.substrate,watering:d.watering,germination:d.germination,techniques:d.techniques||[],numPlants:d.num_plants,tags:d.tags||[],likes:d.likes_count,comments:d.comments_count,cover:0,coverImage:r(d.cover_url),hidden:false,isOwn:d.user_id===user?.id,weeks:[]}))});}catch{}}}/>;
    if(page==="concursos") return <ContestsPage user={user} onBack={()=>setPage("home")} onViewImage={setViewImage} pendingContestId={pendingContestId}/>;
    if(page==="geneticas") return <StrainsPage onBack={()=>setPage("home")}/>;
    if(page==="calculadoras") return <CalculatorsPage onBack={()=>setPage("home")}/>;
    ...
    if(page==="mensagens") return <MessagesPage msgs={msgs} user={user} onSend={sendMsg} onSendMedia={sendMedia} onMarkRead={markMsgRead} onMarkUnread={markMsgUnread} onDeleteConv={deleteConv} onDeleteMessage={deleteMessage} onForwardMsg={forwardMsg} onLoadOlder={loadOlderMessages} onCreateGroup={createGroup} onNewDM={newDM} onViewImage={setViewImage} onViewVideo={setViewVideo} onReportMessage={async({convId,senderName,text})=>{
      // Resolve sender profile ID from username
      try{
        const p=await sb.from("profiles").select("id",`&username=ilike.${encodeURIComponent(senderName)}&limit=1`);
        const targetId=p?.[0]?.id;
        if(targetId){setReportModal({targetType:"user",targetId,targetLabel:`Mensagem de @${senderName}: "${(text||"[mídia]").substring(0,80)}"`});}
        else{toast("Não foi possível identificar o remetente.");}
      }catch{toast("Erro ao preparar denúncia.");}
    }} onBack={()=>setPage("home")} lang={lang}/>;
    ...
    if(selectedDiary){
      ...
      return <DiaryDetail diary={selectedDiary} onBack={()=>setSelectedDiary(null)} onUpdate={doUpdateDiary} onRemove={doRemoveDiary} onHide={doHideDiary} lang={lang} onLike={doLikeWithNotif} onFav={doFav} isLiked={!!likes[selectedDiary.id]} isFaved={!!favs[selectedDiary.id]} onViewImage={setViewImage} onViewVideo={setViewVideo} onReport={doReport} comments={diaryComments} onAddComment={doAddComment} onDeleteComment={doDeleteComment} onEditComment={doEditComment} blockedByOwner={!isOwnerViewing&&blockedUsers.some(b=>b.id===user.id)} onBlockUser={doBlockUser} onUnblockUser={doUnblockUser} onReportUser={doReportUser} currentUserEmail={user.id} onAuthorClick={openPublicProfile}/>;
    }
    return null;
  };
  ...
  return (
    <div style={shellStyle}>
      ...
      {blogEditor&&<BlogEditor post={blogEditor.id?blogEditor:null} user={user} onClose={()=>setBlogEditor(null)} onSave={()=>{setBlogEditor(null);if(page==="blog")setPage("_");setTimeout(()=>setPage("blog"),50);}}/>}
      {!cookieConsent&&<CookieBanner onAccept={acceptCookies} onReject={rejectCookies}/>}
      {globalToast&&<div style={{position:"fixed",bottom:"24px",left:"50%",transform:"translateX(-50%)",background:"#1a1a1a",color:"#fff",padding:"12px 24px",borderRadius:"12px",fontFamily:F.sans,fontSize:"14px",fontWeight:"500",zIndex:99999,maxWidth:"90vw",textAlign:"center",boxShadow:"0 8px 32px rgba(0,0,0,0.3)",animation:"fadeInUp .3s ease"}}>{globalToast}</div>}
