      ...
      <div style={{background:C.surface,borderRadius:"16px",border:`1px solid ${C.border}`,width:"100%",maxWidth:"600px",overflow:"hidden"}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{padding:"20px 24px",borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          ...
          <div style={{fontFamily:F.sans,fontSize:"20px",fontWeight:"800",color:C.text,display:"flex",alignItems:"center",gap:"8px"}}>
            <span style={{fontSize:"22px"}}>📝</span> {phase===0?t.germination:`${t.week} ${wn}`}
          </div>
      ...
      <div style={{padding:"60px 24px 40px",textAlign:"center",position:"relative"}}>
        ...
        <div style={{position:"relative",zIndex:1}}>
          ...
          <div style={{display:"flex",justifyContent:"center",gap:"12px",marginTop:"16px",fontSize:"14px",fontFamily:F.sans,flexWrap:"wrap"}}>
            ...
            <span style={{display:"flex",alignItems:"center",color:C.dim,fontSize:"13px"}}>{diary.phase===0?t.germination:`${t.week} ${diary.week}`}</span>
          </div>
        </div>
      </div>
            ...
            <div key={i} style={{background:C.cardBg,borderRadius:"14px",border:`1px solid ${C.border}`,marginBottom:"16px",overflow:"hidden",boxShadow:"0 1px 3px rgba(0,0,0,0.04)"}}>
              ...
              <div onClick={()=>setExpandedWeek(isOpen?null:i)} style={{padding:"16px 20px",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",background:isOpen?C.surface2:C.cardBg,transition:"background 0.2s"}}>
                <div style={{display:"flex",alignItems:"center",gap:"12px"}}>
                  <div style={{fontFamily:F.sans,fontSize:"16px",fontWeight:"700",color:C.accent}}>{w.phase===0?t.germination:`${t.week} ${w.week}`}</div>
...
function BlogEditor({post,onSave,onClose,user}){
  ...
  const doSave=async(status)=>{
    ...
    setSaving(true);
    try{
      const data={title:sanitize(title,200),content:sanitize(content,50000),excerpt:sanitize(excerpt,500),category,cover_url:coverUrl,status,updated_at:new Date().toISOString()};
  ...
  const tbtn=(icon,tag,tip)=><button key={tag} title={tip} onClick={()=>insertFormat(tag)} style={{width:"32px",height:"32px",borderRadius:"6px",border:`1px solid ${C.border}`,background:C.surface2,color:C.text,cursor:"pointer",fontSize:"14px",display:"flex",alignItems:"center",justifyContent:"center"}}>{icon}</button>;
      ...
      <div style={{maxWidth:"900px",margin:"0 auto",padding:"20px 24px"}}>
        ...
        <div style={{display:"flex",gap:"24px",flexWrap:"wrap"}}>
          ...
          <div style={{flex:1,minWidth:"300px"}}>
            <input style={{...baseInput,fontSize:"20px",fontWeight:"700",marginBottom:"16px"}} value={title} onChange={e=>setTitle(e.target.value)} placeholder="Título do post"/>
            ...
            <div style={{display:"flex",gap:"4px",flexWrap:"wrap",marginBottom:"8px",padding:"8px",background:C.surface2,borderRadius:"8px",border:`1px solid ${C.border}`}}>
              {tbtn("B","b","Negrito")}{tbtn("I","i","Itálico")}{tbtn("H2","h2","Subtítulo")}{tbtn("H3","h3","Subtítulo menor")}{tbtn("•","ul","Lista")}{tbtn("1.","ol","Lista numerada")}{tbtn("❝","quote","Citação")}{tbtn("—","hr","Linha")}{tbtn("🔗","link","Link")}
            </div>

            <textarea ref={contentRef} style={{...baseInput,minHeight:"400px",resize:"vertical",fontFamily:"monospace",fontSize:"14px",lineHeight:"1.8"}} value={content} onChange={e=>setContent(e.target.value)} placeholder="Escreva o conteúdo do post aqui...&#10;&#10;Use **negrito**, *itálico*, ## subtítulos&#10;- listas com traço&#10;> citações com >"/>

            <div style={{fontFamily:F.sans,fontSize:"11px",color:C.dim,marginTop:"6px"}}>Contagem de palavras: {wordCount}</div>
          </div>
          ...
          <div style={{width:"260px",flexShrink:0}}>
            ...
            <div style={{background:C.cardBg,borderRadius:"12px",border:`1px solid ${C.border}`,padding:"16px",marginBottom:"16px"}}>
              ...
              <div style={{fontFamily:F.sans,fontSize:"12px",color:C.muted,marginBottom:"8px"}}>Status: {post?.status==="published"?"Publicado":"Rascunho"}</div>
              <div style={{display:"flex",gap:"8px"}}>
                <button onClick={()=>doSave("draft")} disabled={saving} style={{...btnSecondary,fontSize:"12px",padding:"8px 14px"}}>💾 Salvar Rascunho</button>
                <button onClick={()=>doSave("published")} disabled={saving} style={{...btnPrimary,fontSize:"12px",padding:"8px 14px"}}>{saving?"...":"🚀 Publicar"}</button>
              </div>
            </div>

            {/* Category */}
            <div style={{background:C.cardBg,borderRadius:"12px",border:`1px solid ${C.border}`,padding:"16px",marginBottom:"16px"}}>
              <div style={{fontFamily:F.sans,fontSize:"13px",fontWeight:"700",marginBottom:"10px"}}>📁 Categoria</div>
              {BLOG_CATEGORIES.map(c=>(
                <label key={c} style={{display:"flex",alignItems:"center",gap:"8px",cursor:"pointer",padding:"4px 0",fontFamily:F.sans,fontSize:"13px",color:category===c?C.accent:C.muted}}>
                  <input type="radio" name="cat" checked={category===c} onChange={()=>setCategory(c)} style={{accentColor:C.accent}}/>{c}
                </label>
              ))}
            </div>
            ...
            <div style={{background:C.cardBg,borderRadius:"12px",border:`1px solid ${C.border}`,padding:"16px",marginBottom:"16px"}}>
              ...
              <button onClick={()=>coverRef.current?.click()} style={{...linkBtn,fontSize:"12px"}}>{coverUrl?"Trocar imagem":"+ Adicionar imagem"}</button>
            ...
            <div style={{background:C.cardBg,borderRadius:"12px",border:`1px solid ${C.border}`,padding:"16px"}}>
              ...
              <textarea style={{...baseInput,minHeight:"60px",resize:"vertical",fontSize:"12px"}} value={excerpt} onChange={e=>setExcerpt(e.target.value)} placeholder="Resumo curto do post..." maxLength={500}/>
            </div>
          </div>
        </div>
      </div>
...
function BlogPage({onBack,user,onOpenPost,onNewPost}){
  ...
  const renderMarkdown=(text)=>{
    ...
    return text
      ...
      .replace(/^### (.+)$/gm,"<h4>$1</h4>")
      .replace(/^## (.+)$/gm,"<h3>$1</h3>")
      .replace(/^> (.+)$/gm,"<blockquote style='border-left:3px solid #1B9E42;padding-left:12px;color:#666;margin:8px 0'>$1</blockquote>")
  ...
  return(
    <div style={{maxWidth:"800px",margin:"0 auto",padding:"24px"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"32px"}}>
        ...
        {user?.role==="admin"&&<button onClick={onNewPost} style={{...btnPrimary,width:"auto",padding:"10px 20px",fontSize:"13px"}}>✏️ Novo Post</button>}
      </div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:"24px"}}>
        {posts.map(p=>(
          <div key={p.id} onClick={()=>onOpenPost?.(p)} style={{background:C.cardBg,borderRadius:"16px",border:`1px solid ${C.border}`,overflow:"hidden",cursor:"pointer",transition:"all 0.2s",boxShadow:"0 2px 8px rgba(0,0,0,0.04)"}}>
            {p.cover_url&&<img src={p.cover_url} alt="" style={{width:"100%",height:"200px",objectFit:"cover"}}/>}
            <div style={{padding:"20px 24px"}}>
              ...
              <p style={{fontFamily:F.body,fontSize:"14px",color:C.muted,lineHeight:"1.6",margin:"0 0 12px"}}>{p.excerpt||p.content?.substring(0,150)+"..."}</p>
...
// ─── Blog Post View ───
function BlogPostView({post,onBack,user,onEdit}){
  const renderMarkdown=(text)=>{
    if(!text)return"";
    return text
      .replace(/\*\*(.+?)\*\*/g,"<strong>$1</strong>")
      .replace(/\*(.+?)\*/g,"<em>$1</em>")
      .replace(/^### (.+)$/gm,"<h4 style='margin:20px 0 8px;font-size:16px'>$1</h4>")
      .replace(/^## (.+)$/gm,"<h3 style='margin:24px 0 10px;font-size:20px'>$1</h3>")
      .replace(/^> (.+)$/gm,"<blockquote style='border-left:3px solid #1B9E42;padding-left:16px;color:#888;margin:12px 0;font-style:italic'>$1</blockquote>")
  ...
  return(
    <div style={{maxWidth:"740px",margin:"0 auto",padding:"32px 24px"}}>
      ...
      {post.cover_url&&<img src={post.cover_url} alt="" style={{width:"100%",maxHeight:"360px",objectFit:"cover",borderRadius:"16px",marginBottom:"24px"}}/>}
      ...
      <div style={{fontFamily:F.body,fontSize:"16px",color:C.text,lineHeight:"1.8"}} dangerouslySetInnerHTML={{__html:`<p style='margin:0 0 14px;line-height:1.8'>${renderMarkdown(post.content)}</p>`}}/>
    </div>
  );
}
...
function AppInner() {
  ...
  const renderPage=()=>{
    ...
    if(blogPost) return <BlogPostView post={blogPost} onBack={()=>setBlogPost(null)} user={user} onEdit={p=>{setBlogPost(null);setBlogEditor(p);}}/>;
