...
function AppInner() {
  ...
  useEffect(()=>{if(inApp&&user&&pendingDiaryId){const id=pendingDiaryId;setPendingDiaryId(null);setPage("explorar");openDiaryById(id);}/* eslint-disable-next-line */},[inApp,user,pendingDiaryId]);
  const openDiaryById=async(id)=>{try{const rows=await sb.from("diaries").select("*,profiles(username,avatar,avatar_url)",`&id=eq.${id}&hidden=eq.false&limit=1`);const dd=rows?.[0];if(dd)setSelectedDiary({id:dd.id,name:dd.name,strain:dd.strain,strains:dd.strains||[],author:dd.profiles?.username||"",authorId:dd.user_id,avatar:dd.profiles?.avatar||"🌱",avatarImg:dd.profiles?.avatar_url||null,phase:dd.phase,week:dd.current_week,env:dd.environment,light:dd.lighting,watts:dd.watts,substrate:dd.substrate,watering:dd.watering,germination:dd.germination,techniques:dd.techniques||[],numPlants:dd.num_plants,tags:dd.tags||[],likes:dd.likes_count,comments:dd.comments_count,cover:0,coverImage:dd.cover_url,hidden:false,isOwn:dd.user_id===user?.id,weeks:[]});}catch{}};
