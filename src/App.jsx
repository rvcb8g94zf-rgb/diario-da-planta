...
function AppInner() {
  ...
  const sendMedia=async(convId,media)=>{
    ...
    setMsgs(p=>p.map(c=>c.id===convId?{...c,messages:[...c.messages,{id:"m"+Date.now(),from:user.email,text:"",media:{type:media.type,data:mediaLocal,path:dbMedia},time:Date.now()}],readAt:Date.now()}:c));
  };
