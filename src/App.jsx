...
const sbStorage = {
  upload: async (path, file) => {
    ...
    if(res.ok) _signedUrlCache.delete(path); // invalidate cache
    return res.ok;
  },
  ...
  getSignedUrl: async (path) => {
    ...
    return `${SB_URL}/storage/v1/object/authenticated/media/${path}`;
  },
  // Synchronous URL for immediate use (returns cached or placeholder)
  getUrl: (path) => {
    if(!path) return "";
    const cached = _signedUrlCache.get(path);
    if(cached && cached.expiresAt > Date.now()) return cached.url;
    // Trigger async signing in background
    sbStorage.getSignedUrl(path).catch(()=>{});
    // Return authenticated endpoint as immediate fallback
    return `${SB_URL}/storage/v1/object/authenticated/media/${path}`;
  },
      ...
      if(res.ok) {
        ...
        (data||[]).forEach(item => {
          if(item.signedURL) {
            ...
            _signedUrlCache.set(item.path||unsigned[data.indexOf(item)], {url, expiresAt: Date.now() + 82800000});
          }
        });
      }
    } catch {}
    const result = {};
    paths.forEach(p => { result[p] = _signedUrlCache.get(p)?.url || `${SB_URL}/storage/v1/object/authenticated/media/${p}`; });
    return result;
  },
  delete: async (paths) => {
...
async function signMediaUrls(items, ...fields) {
  ...
  const paths = [];
  const pathMap = []; // [{itemIdx, field}]
  items.forEach((item, idx) => {
    fields.forEach(f => {
      const url = item[f];
      if(url && typeof url === "string") {
        const path = extractStoragePath(url);
        if(path) { paths.push(path); pathMap.push({idx, f, path}); }
      }
    });
  });
  if(paths.length === 0) return items;
  const signed = await sbStorage.signBatch([...new Set(paths)]);
  const result = items.map(i => ({...i}));
  pathMap.forEach(({idx, f, path}) => {
    if(signed[path]) result[idx][f] = signed[path];
  });
  return result;
}
...
function ForumPage({user,onBack,onReport,pendingThreadId,onThreadOpened}){
  ...
  const loadReplies=async(tid)=>{
    try{const r=await sb.from("forum_replies").select("*,profiles(username,avatar,avatar_url)",`&thread_id=eq.${tid}&order=created_at.asc`);setReplies(r||[]);}catch{setReplies([]);}
  ...
  useEffect(()=>{(async()=>{
    ...
    try{
      ...
      if(authUser?.id){
        ...
        if(profile&&!profile.banned) setUser({...profile,avatarImg:r(profile.avatar_url),createdAt:profile.created_at,authId:authUser.id});
      }
    }catch{}
    setAuthLoading(false);
  })();},[]);
