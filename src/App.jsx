...
// ─── Storage shim (same API as before) ───
const sbStorage = {
...
function NPListItem({ article, showImage = true, showSummary = false, compact = false }) {
  const cat = NP_CAT_LABEL[article.category] || "Geral";
  return (
    <a href={article.url} target="_blank" rel="noopener noreferrer"
      style={{ display: "flex", gap: compact ? "10px" : "13px", alignItems: "flex-start", textDecoration: "none", color: "inherit", paddingBottom: compact ? "11px" : "14px", marginBottom: compact ? "11px" : "14px", borderBottom: `1px solid ${NP.border}` }}>
      {showImage && (
        <div style={{ width: compact ? "70px" : "100px", height: compact ? "52px" : "72px", flexShrink: 0, borderRadius: "3px", background: article.image_url ? `url(${article.image_url}) center/cover` : `linear-gradient(135deg, ${NP.ciano}, ${NP.indigo})` }} />
      )}
      <div style={{ flex: 1 }}>
        <span style={{ display: "inline-block", fontFamily: NP.sans, fontSize: "10px", fontWeight: "700", color: NP.ciano, textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: "4px" }}>{cat}</span>
        <h3 style={{ fontFamily: NP.serif, fontSize: compact ? "14px" : "16px", fontWeight: "700", color: NP.text, margin: 0, lineHeight: "1.25" }}>{article.title}</h3>
...
function NPGridCard({ article }) {
  const cat = NP_CAT_LABEL[article.category] || "Geral";
  return (
    <a href={article.url} target="_blank" rel="noopener noreferrer" style={{ display: "block", textDecoration: "none", color: "inherit" }}>
      <div style={{ width: "100%", paddingTop: "58%", borderRadius: "3px", background: article.image_url ? `url(${article.image_url}) center/cover` : `linear-gradient(135deg, ${NP.ciano}, ${NP.indigo})`, marginBottom: "9px", position: "relative" }}>
        <span style={{ position: "absolute", top: "8px", left: "8px", fontFamily: NP.sans, fontSize: "10px", fontWeight: "700", color: NP.white, background: "rgba(26,43,74,0.85)", padding: "3px 9px", borderRadius: "2px", textTransform: "uppercase", letterSpacing: "0.4px" }}>{cat}</span>
      </div>
      <h3 style={{ fontFamily: NP.serif, fontSize: "16px", fontWeight: "700", color: NP.text, margin: "0 0 6px", lineHeight: "1.25" }}>{article.title}</h3>
...
function NewsPortal({ onEnterApp, dark, onToggleDark, embedded = false }) {
  const [articles, setArticles] = useState([]);
  const [loading, setLoading] = useState(true);
  const [catFilter, setCatFilter] = useState("todas");
  const [langFilter, setLangFilter] = useState("todas");
  ...
  // Distribuir artigos pelas seções do layout
  const featured = articles.find((a) => a.featured) || articles[0];
  const afterFeatured = articles.filter((a) => a.id !== featured?.id);
  const heroSide = afterFeatured.slice(0, 4);
  const poolAfterHero = afterFeatured.slice(4);
  // Duas colunas: Últimas Atualizações (todas) vs Artigos (medicinal/ciência)
  const colLatest = poolAfterHero.slice(0, 5);
  const colArticles = poolAfterHero.slice(5, 10);
  // Blocos por categoria
  const cultivoArticles = articles.filter((a) => a.category === "cultivo").slice(0, 3);
  const medicinalArticles = articles.filter((a) => a.category === "medicinal").slice(0, 3);
  // Sidebar — mais lidas (simplesmente as mais recentes por ora)
  ...
  const handleCat = (id) => setCatFilter(id);

  return (
    <div style={{ minHeight: "100vh", background: NP.white, color: NP.text, fontFamily: NP.serif }}>
      {!embedded && (
        <>
          {/* ─── Top bar ─── */}
          <div style={{ background: NP.indigo, color: NP.white, fontFamily: NP.sans, fontSize: "12px", padding: "8px 20px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "8px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
              <span style={{ color: "rgba(255,255,255,0.85)" }}>📍 São Paulo</span>
              <span style={{ color: "rgba(255,255,255,0.4)" }}>|</span>
              <span style={{ color: "rgba(255,255,255,0.7)", textTransform: "capitalize" }}>{isToday}</span>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
              ...
              <button onClick={onEnterApp} style={{ background: "none", border: "none", color: NP.cianoLight, cursor: "pointer", fontFamily: NP.sans, fontSize: "12px", fontWeight: "700" }}>Entrar / Cadastrar</button>
            </div>
          </div>
          ...
          <div style={{ textAlign: "center", padding: "22px 16px 16px", background: NP.white }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: "12px" }}>
              <img src={LOGO_SRC} alt="Diário da Planta" style={{ height: "48px", width: "48px", borderRadius: "8px", objectFit: "cover", border: `2px solid ${NP.ciano}` }} />
              <div style={{ textAlign: "left" }}>
                <div style={{ fontFamily: NP.serif, fontSize: "32px", fontWeight: "700", color: NP.indigo, lineHeight: "1", letterSpacing: "-0.5px" }}>Diário da Planta</div>
      ...
      <div style={{ background: NP.indigo, display: "flex", alignItems: "center", justifyContent: "center", flexWrap: "wrap", padding: "0 8px", position: embedded ? "static" : "sticky", top: 0, zIndex: 50 }}>
        {NP_CATS.map((c) => (
          <button key={c.id} onClick={() => handleCat(c.id)}
            style={{ background: catFilter === c.id ? NP.ciano : "transparent", color: NP.white, fontFamily: NP.sans, fontSize: "13px", fontWeight: "700", padding: "12px 16px", textTransform: "uppercase", letterSpacing: "0.4px", border: "none", cursor: "pointer", borderBottom: catFilter === c.id ? `3px solid ${NP.cianoLight}` : "3px solid transparent" }}>{c.label}</button>
        ))}
        <div style={{ marginLeft: "auto", display: "flex", gap: "4px", padding: "6px 0" }}>
      ...
      {loading ? (
        ...
        <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "20px 16px" }}>
          {/* ─── Mosaico de destaque ─── */}
          <div style={{ display: "grid", gridTemplateColumns: "1.7fr 1fr", gap: "16px", marginBottom: "8px" }}>
            {featured && (
              <a href={featured.url} target="_blank" rel="noopener noreferrer" style={{ position: "relative", borderRadius: "3px", overflow: "hidden", minHeight: "340px", textDecoration: "none", display: "block" }}>
                <div style={{ position: "absolute", inset: 0, background: featured.image_url ? `url(${featured.image_url}) center/cover` : `linear-gradient(135deg, ${NP.ciano}, ${NP.indigo})` }} />
                ...
                <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "24px" }}>
                  <span style={{ display: "inline-block", fontFamily: NP.sans, fontSize: "11px", fontWeight: "700", color: NP.white, background: NP.amber, padding: "4px 12px", textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: "12px" }}>{NP_CAT_LABEL[featured.category] || "Destaque"}</span>
                  <h2 style={{ fontFamily: NP.serif, fontSize: "28px", fontWeight: "700", color: NP.white, margin: "0 0 8px", lineHeight: "1.18" }}>{featured.title}</h2>
                  {featured.summary && <p style={{ fontFamily: NP.serif, fontSize: "14px", color: "rgba(255,255,255,0.9)", margin: "0 0 8px", lineHeight: "1.45", maxWidth: "600px" }}>{featured.summary}</p>}
                  <div style={{ fontFamily: NP.sans, fontSize: "12px", color: "rgba(255,255,255,0.8)" }}>{featured.source_name} · {npTimeAgo(featured.published_at)}</div>
                </div>
              </a>
            )}
            <div>{heroSide.map((a) => <NPListItem key={a.id} article={a} />)}</div>
          </div>

          {/* ─── Duas colunas: Últimas Atualizações + Artigos ─── */}
          {(colLatest.length > 0 || colArticles.length > 0) && (
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0", borderTop: `1px solid ${NP.border}`, marginTop: "12px" }}>
              <div style={{ padding: "16px 20px 0 0", borderRight: `1px solid ${NP.border}` }}>
                ...
                {colLatest.map((a) => <NPListItem key={a.id} article={a} compact />)}
              </div>
              <div style={{ padding: "16px 0 0 20px" }}>
                ...
                {colArticles.map((a) => <NPListItem key={a.id} article={a} compact />)}
              </div>
            </div>
          )}

          {/* ─── Conteúdo + Sidebar ─── */}
          <div style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: "28px", marginTop: "8px" }}>
            <div>
              {cultivoArticles.length > 0 && (
                <>
                  <NPSectionHead title="Cultivo Medicinal" onMore={() => handleCat("cultivo")} />
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "16px", padding: "14px 0 8px" }}>
                    {cultivoArticles.map((a) => <NPGridCard key={a.id} article={a} />)}
                  </div>
                </>
              )}
              {medicinalArticles.length > 0 && (
                <>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "16px", padding: "14px 0 8px" }}>
                    {medicinalArticles.map((a) => <NPGridCard key={a.id} article={a} />)}
                  </div>
                </>
              )}
              {/* Espaço publicitário */}
              <div style={{ background: NP.bgLight, border: `1px dashed ${NP.border}`, textAlign: "center", padding: "18px", fontFamily: NP.sans, fontSize: "11px", color: NP.dim, letterSpacing: "1px", margin: "16px 0", borderRadius: "3px" }}>— ESPAÇO PUBLICITÁRIO —</div>

              {hasMore && (
                <div style={{ textAlign: "center", marginTop: "8px" }}>
                  <button onClick={() => loadNews(articles.length, true)} disabled={loadingMore} style={{ fontFamily: NP.sans, fontSize: "13px", fontWeight: "700", color: NP.indigo, background: "none", border: `1px solid ${NP.indigo}`, padding: "10px 32px", cursor: "pointer", textTransform: "uppercase", letterSpacing: "0.5px" }}>{loadingMore ? "Carregando..." : "Carregar mais"}</button>
                </div>
              )}
            </div>
            ...
            <aside>
              {!embedded && (
                <>
                  <div style={{ fontFamily: NP.sans, fontSize: "13px", fontWeight: "700", textTransform: "uppercase", letterSpacing: "0.5px", borderBottom: `2px solid ${NP.indigo}`, paddingBottom: "6px", marginBottom: "13px", color: NP.indigo }}>Conecte-se</div>
                  {[["24.856", "Fãs", "CURTIR"], ["3.913", "Seguidores", "SEGUIR"], ["22.800", "Inscritos", "INSCREVER"]].map(([n, lbl, btn], i) => (
                    <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "9px 0", borderBottom: `1px solid ${NP.border}`, fontFamily: NP.sans }}>
                      <span style={{ fontSize: "12px" }}><b>{n}</b> <span style={{ color: NP.dim }}>{lbl}</span></span>
                      <span style={{ color: NP.ciano, fontSize: "11px", fontWeight: "700" }}>{btn}</span>
                    </div>
                  ))}
                  <div style={{ background: NP.indigo, color: NP.white, textAlign: "center", padding: "22px 14px", margin: "16px 0", borderRadius: "3px", borderTop: `3px solid ${NP.ciano}` }}>
                    ...
                    <button onClick={onEnterApp} style={{ background: NP.ciano, color: NP.white, fontFamily: NP.sans, fontSize: "12px", fontWeight: "700", padding: "9px 22px", borderRadius: "20px", border: "none", cursor: "pointer" }}>Criar conta grátis</button>
                  </div>
                </>
              )}
              ...
              {mostRead.map((a, i) => (
                <a key={a.id} href={a.url} target="_blank" rel="noopener noreferrer" style={{ display: "flex", gap: "11px", marginBottom: "13px", alignItems: "flex-start", textDecoration: "none", color: "inherit" }}>
                  <span style={{ fontFamily: NP.serif, fontSize: "30px", fontWeight: "700", color: NP.amber, opacity: 0.5, lineHeight: "0.9" }}>{i + 1}</span>
                  <div>
                    <h4 style={{ fontFamily: NP.serif, fontSize: "14px", fontWeight: "700", color: NP.text, margin: 0, lineHeight: "1.25" }}>{a.title}</h4>
...
function NavBar({ user, page, setPage, setShowCreate, myDiaries, onLogout, onNavigate, lang, setLang, unreadNotifs, unreadMsgs, notifs, onMarkNotifsRead, dark, onToggleDark }) {
  ...
  const sidebarItems=[
    ...
    {icon:"📰",label:"Notícias",action:()=>nav("noticias"),active:page==="noticias"},
    ...
    <>
      ...
      <nav style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 16px",borderBottom:`1px solid ${C.border}`,backdropFilter:"blur(20px)",background:"var(--dp-overlay85)",position:"sticky",top:0,zIndex:100,gap:"8px"}}>
        ...
        <div className="dp-nav-links">
          {[["home",t.home],["noticias","Notícias"],["explorar",t.explore],["meus",t.myDiaries]].map(([p,label])=>(
          ...
          <div ref={ref} style={{position:"relative"}}>
            ...
            {showMenu&&<div style={{position:"absolute",top:"46px",right:0,background:C.cardBg,borderRadius:"14px",border:`1px solid ${C.border}`,padding:"8px",minWidth:"240px",boxShadow:"0 8px 30px rgba(0,0,0,0.12)",zIndex:110}}>
              ...
              <div style={{borderTop:`1px solid ${C.border}`,marginTop:"6px",paddingTop:"6px"}}>
                {menuItem("📰","Voltar ao Portal",()=>nav("noticias"))}
...
function AppInner() {
    ...
    if(convIds.length>0){
      ...
      for(const cr of convRows){
        ...
        convList.push({
          ...
          messages:msgRows.map(m=>({id:m.id,from:m.sender_id===user.id?user.email:(m.profiles?.username||m.sender_id),text:m.text||"",media:m.media_url?{type:m.media_type||"image",data:m.media_url}:null,time:new Date(m.created_at).getTime(),forwarded:m.forwarded,isSystem:!!m.is_system,reportNumber:m.report_number||null})),
        });
      }
      setMsgs(convList);
    }
  ...
  const doLogout=async()=>{
    ...
    try{
      ...
      // Se a revogação no servidor falhar (offline, token já inválido), limpa a sessão local mesmo assim
      console.warn("[logout] signOut falhou, limpando sessão local:",e);
    }
    setUser(null);setMyDiaries([]);setPublicDiaries([]);setNotifs([]);setMsgs([]);setLikes({});setFavs({});setCommentsMap({});setBlockedUsers([]);setFollows([]);setDataLoaded(false);setSelectedDiary(null);setPublicProfile(null);setShowAuth(false);setPage("home");
    setTimeout(()=>{loggingOutRef.current=false;},800);
  };
  ...
  if(!user) {
    ...
    return <><ThemeCSS dark={dark}/><NewsPortal onEnterApp={()=>setShowAuth(true)} dark={dark} onToggleDark={toggleDark}/></>;
  }
  ...
  const renderPage=()=>{
    ...
    if(page==="noticias") return <NewsPortal embedded onEnterApp={()=>{}} dark={dark} onToggleDark={toggleDark}/>;
