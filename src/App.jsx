...
function ForumPage({user,onBack}){
  ...
  // Toolbar render helper (NOT a component - avoids re-mount)
  const toolbarBtnSt={width:"32px",height:"32px",borderRadius:"6px",border:`1px solid ${C.border}`,background:C.cardBg,color:C.text,cursor:"pointer",fontSize:"13px",display:"flex",alignItems:"center",justifyContent:"center"};
  const renderToolbar=(cRef,content,setContent,fileRef)=>(
    <div style={{display:"flex",gap:"4px",flexWrap:"wrap",marginBottom:"8px",padding:"8px",background:C.surface2,borderRadius:"8px",border:`1px solid ${C.border}`}}>
      <button type="button" onClick={()=>insertFmt(cRef,setContent,content,"b")} style={{...toolbarBtnSt,fontWeight:"800"}}>B</button>
      <button type="button" onClick={()=>insertFmt(cRef,setContent,content,"i")} style={toolbarBtnSt}>I</button>
      <button type="button" onClick={()=>insertFmt(cRef,setContent,content,"quote")} style={toolbarBtnSt}>❝</button>
      <button type="button" onClick={()=>insertFmt(cRef,setContent,content,"link")} style={toolbarBtnSt}>🔗</button>
      <div style={{width:"1px",height:"24px",background:C.border,margin:"4px 2px"}}/>
      <button type="button" onClick={()=>fileRef.current?.click()} title="Inserir foto/vídeo" style={{...toolbarBtnSt,border:`1px solid ${C.accent}`,background:C.accentBg,color:C.accent}}>📷</button>
      <button type="button" onClick={()=>insertYT(setContent,content,cRef)} title="YouTube" style={{...toolbarBtnSt,border:"1px solid #e53e3e",background:"rgba(229,62,62,0.06)",color:"#e53e3e"}}>▶️</button>
      <input ref={fileRef} type="file" accept="image/*,video/*" style={{display:"none"}} onChange={()=>handleForumMedia(fileRef,setContent,content,cRef)}/>
      {uploading&&<span style={{fontSize:"11px",color:C.accent,fontFamily:F.sans,display:"flex",alignItems:"center"}}>⏳</span>}
    </div>
  );
  ...
  if(view==="thread"&&selThread) return(
    <div style={{maxWidth:"800px",margin:"0 auto",padding:"24px"}}>
      ...
      {replies.map(r=>(
        <div key={r.id} style={{background:C.cardBg,borderRadius:"12px",border:`1px solid ${C.border}`,padding:"14px 18px",marginBottom:"8px"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"8px"}}>
            ...
            <button onClick={()=>{setReplyText(prev=>prev+`\n> ${r.profiles?.username||""} disse: ${(r.content||"").substring(0,100)}${(r.content||"").length>100?"...":""}\n\n`);}} type="button" style={{padding:"4px 10px",borderRadius:"6px",border:`1px solid ${C.border}`,background:C.surface2,color:C.dim,cursor:"pointer",fontSize:"11px",fontFamily:F.sans}}>❝ Citar</button>
          </div>
      ...
      <div style={{marginTop:"16px",background:C.cardBg,borderRadius:"12px",border:`1px solid ${C.border}`,padding:"16px"}}>
        ...
        {renderToolbar(replyContentRef,replyText,setReplyText,replyMediaRef)}
      ...
      {showNewThread&&<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.4)",backdropFilter:"blur(8px)",display:"flex",alignItems:"flex-start",justifyContent:"center",zIndex:300,padding:"40px 20px",overflowY:"auto"}} onClick={()=>setShowNewThread(false)}>
        <div style={{...cardBase,maxWidth:"600px"}} onClick={e=>e.stopPropagation()}>
          ...
          <div style={{marginBottom:"16px"}}><label style={labelSt}>Mensagem *</label>
            {renderToolbar(threadContentRef,newContent,setNewContent,mediaRef)}
