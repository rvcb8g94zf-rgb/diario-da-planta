...
function BlogEditor({post,onSave,onClose,user}){
  ...
  const renderPreview=(text)=>{
    ...
    return text
      // YouTube embeds
      .replace(/\[youtube:([a-zA-Z0-9_-]{11})\]/g,'<div style="position:relative;padding-bottom:56.25%;height:0;margin:16px 0;border-radius:12px;overflow:hidden"><iframe src="https://www.youtube.com/embed/$1" style="position:absolute;top:0;left:0;width:100%;height:100%;border:none" allowfullscreen></iframe></div>')
      // Images
    ...
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:C.bg,zIndex:400,overflowY:"auto"}}>
      ...
      {showYTModal&&<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.5)",backdropFilter:"blur(6px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:500,padding:"24px"}} onClick={()=>setShowYTModal(false)}>
        <div style={{...cardBase,maxWidth:"480px"}} onClick={e=>e.stopPropagation()}>
          ...
          {ytUrl&&extractYTId(ytUrl)&&<div style={{margin:"16px 0",borderRadius:"12px",overflow:"hidden",position:"relative",paddingBottom:"56.25%",background:"#000"}}><iframe src={`https://www.youtube.com/embed/${extractYTId(ytUrl)}`} style={{position:"absolute",top:0,left:0,width:"100%",height:"100%",border:"none"}} allowFullScreen/></div>}
...
function BlogPostView({post,onBack,user,onEdit}){
  const renderMarkdown=(text)=>{
    ...
    return text
      // YouTube embeds
      .replace(/\[youtube:([a-zA-Z0-9_-]{11})\]/g,'<div style="position:relative;padding-bottom:56.25%;height:0;margin:20px 0;border-radius:12px;overflow:hidden"><iframe src="https://www.youtube.com/embed/$1" style="position:absolute;top:0;left:0;width:100%;height:100%;border:none" allowfullscreen></iframe></div>')
      // Images
      .replace(/!\[([^\]]*)\]\(([^)]+)\)/g,'<div style="margin:20px 0"><img src="$2" alt="$1" style="max-width:100%;border-radius:12px;box-shadow:0 2px 12px rgba(0,0,0,0.08)"/></div>')
      // Headers
      .replace(/^### (.+)$/gm,'<h4 style="margin:20px 0 8px;font-size:16px;font-weight:700;font-family:Inter,sans-serif">$1</h4>')
      .replace(/^## (.+)$/gm,'<h3 style="margin:24px 0 10px;font-size:20px;font-weight:700;font-family:Inter,sans-serif">$1</h3>')
      // Bold, italic
