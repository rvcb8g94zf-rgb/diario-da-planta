...
// ─── Botão de fixar (admin/editor) — grava na tabela certa via __post ───
function PinButton({ item, onPin, size = "sm" }) {
  const [busy, setBusy] = useState(false);
  const pinned = !!item.pinned;
  const toggle = async (e) => {
    e.preventDefault(); e.stopPropagation();
    if (busy) return; setBusy(true);
    const table = item.__post ? "portal_posts" : "news_articles";
    const realId = item.__post ? item.postData?.id : item.id;
    const patch = pinned ? { pinned: false, pinned_at: null } : { pinned: true, pinned_at: new Date().toISOString() };
    try { await sb.from(table).update(patch, `&id=eq.${realId}`); if (onPin) onPin(item, !pinned); } catch (err) { console.error("[pin]", err); }
    setBusy(false);
  };
  const lg = size === "lg";
  return (
    <button onClick={toggle} title={pinned ? "Desafixar do carrossel" : "Fixar no carrossel"} aria-label={pinned ? "Desafixar" : "Fixar"}
      style={{ position: "absolute", top: lg ? "12px" : "6px", right: lg ? "12px" : "6px", zIndex: 4, width: lg ? "36px" : "28px", height: lg ? "36px" : "28px", borderRadius: "50%", border: pinned ? "none" : "1px solid rgba(255,255,255,0.5)", cursor: "pointer", fontSize: lg ? "16px" : "13px", lineHeight: 1, display: "flex", alignItems: "center", justifyContent: "center", background: pinned ? NP.amber : "rgba(0,0,0,0.5)", color: "#fff", boxShadow: "0 1px 5px rgba(0,0,0,0.35)", opacity: busy ? 0.6 : 1 }}>
      {busy ? "…" : "📌"}
    </button>
  );
}
        ...
        {items.map((a) => (
          <div key={a.id} style={{ minWidth: "100%", position: "relative" }}>
            <a href={a.__post ? "#" : a.url} target={a.__post ? undefined : "_blank"} rel={a.__post ? undefined : "noopener noreferrer"} onClick={a.__post ? (e) => { e.preventDefault(); onOpen && onOpen(a); } : undefined} style={{ display: "block", position: "relative", height: H, textDecoration: "none" }}>
              ...
              {a.pinned && <span style={{ position: "absolute", top: "12px", left: "12px", background: NP.amber, color: "#fff", fontFamily: NP.sans, fontSize: "11px", fontWeight: "800", letterSpacing: "0.5px", padding: "6px 10px", borderRadius: "4px", zIndex: 2 }}>📌 FIXADO</span>}
            ...
            {canEdit && onPin && <PinButton item={a} onPin={onPin} size="lg" />}
          </div>
        ))}
      </div>
...
function NPListItem({ article, showImage = true, showSummary = false, compact = false, onOpen, canEdit, onPin }) {
  ...
  return (
    <div style={{ position: "relative", paddingBottom: compact ? "11px" : "14px", marginBottom: compact ? "11px" : "14px", borderBottom: `1px solid ${NP.border}` }}>
      {canEdit && onPin && <PinButton item={article} onPin={onPin} />}
