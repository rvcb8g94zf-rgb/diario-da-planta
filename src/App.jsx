...
function AdminPanel({ user, onBack, onNewPost }) {
  ...
  const doSendWarning=async()=>{
    ...
    try{
      // 1) Notification
      await sb.from("notifications").insert({user_id:warnTarget,type:"warning",from_username:"Administração",from_avatar:"🛡️",text:cleanWarn,report_number:refNum||null});
...
function ContestsPage({user,onBack,onViewImage}){
  ...
  const [newVoteEnd,setNewVoteEnd]=useState("");
  ...
  const handleCreate=async()=>{
    if(!isAdmin||!newTitle.trim()||!newSubEnd||!newVoteEnd)return;
    try{
      await sb.from("contests").insert({title:sanitize(newTitle,100),description:sanitize(newDesc,500),category:newCategory,submission_end:new Date(newSubEnd).toISOString(),voting_end:new Date(newVoteEnd).toISOString(),prize_description:sanitize(newPrize,200),created_by:user.id,status:"active"});
      toast("Concurso criado!");
      setShowCreate(false);setNewTitle("");setNewDesc("");setNewPrize("");
      const rows=await sb.from("contests").select("*,profiles(username)",`&order=created_at.desc`);
      setContests(rows||[]);
    }catch(e){toast("Erro ao criar concurso.");reportError(e,{feature:"contests",op:"create"});}
  };
  ...
  return(
    <div style={{maxWidth:"800px",margin:"0 auto",padding:"24px"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"24px"}}>
        ...
        {isAdmin&&<button onClick={()=>setShowCreate(true)} style={{...btnPrimary,width:"auto",padding:"8px 18px",fontSize:"13px"}}>+ Criar</button>}
      </div>
      ...
      contests.length===0?<div style={{textAlign:"center",padding:"48px",background:C.surface2,borderRadius:"16px"}}>
        ...
        {isAdmin&&<button onClick={()=>setShowCreate(true)} style={{...btnPrimary,width:"auto",padding:"10px 24px",marginTop:"16px"}}>+ Criar Primeiro Concurso</button>}
      </div>:
      ...
      {/* Create modal (admin) */}
      {showCreate&&<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.5)",backdropFilter:"blur(6px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:300,padding:"24px"}} onClick={()=>setShowCreate(false)}>
        <div onClick={e=>e.stopPropagation()} style={{background:C.cardBg,borderRadius:"16px",padding:"24px",maxWidth:"480px",width:"100%",maxHeight:"90vh",overflowY:"auto"}}>
          <h3 style={{fontFamily:F.sans,fontSize:"18px",fontWeight:"700",margin:"0 0 16px"}}>🏆 Criar Concurso</h3>
          <input placeholder="Título do concurso" value={newTitle} onChange={e=>setNewTitle(e.target.value)} style={{...baseInput,marginBottom:"10px"}}/>
          <textarea placeholder="Descrição (opcional)" value={newDesc} onChange={e=>setNewDesc(e.target.value)} style={{...baseInput,minHeight:"60px",resize:"vertical",marginBottom:"10px"}}/>
          <select value={newCategory} onChange={e=>setNewCategory(e.target.value)} style={{...baseInput,marginBottom:"10px"}}>
          ...
          <label style={{fontFamily:F.sans,fontSize:"12px",color:C.dim,display:"block",marginBottom:"4px"}}>Fim das submissões</label>
          <input type="datetime-local" value={newSubEnd} onChange={e=>setNewSubEnd(e.target.value)} style={{...baseInput,marginBottom:"10px"}}/>
          <label style={{fontFamily:F.sans,fontSize:"12px",color:C.dim,display:"block",marginBottom:"4px"}}>Fim da votação</label>
          <input type="datetime-local" value={newVoteEnd} onChange={e=>setNewVoteEnd(e.target.value)} style={{...baseInput,marginBottom:"10px"}}/>
          <input placeholder="Prêmio (opcional, ex: Badge exclusiva)" value={newPrize} onChange={e=>setNewPrize(e.target.value)} style={{...baseInput,marginBottom:"16px"}}/>
          <div style={{display:"flex",gap:"10px"}}>
            <button onClick={()=>setShowCreate(false)} style={{...btnSecondary,flex:1}}>Cancelar</button>
            <button onClick={handleCreate} disabled={!newTitle.trim()||!newSubEnd||!newVoteEnd} style={{...btnPrimary,flex:1,opacity:(!newTitle.trim()||!newSubEnd||!newVoteEnd)?0.5:1}}>Criar</button>
          </div>
        </div>
      </div>}
    </div>
    ...
    if(liked){
      ...
      try{
        ...
        if(authorId&&authorId!==user.id){
          await sb.from("notifications").insert({user_id:authorId,type:"forum_like",from_username:user.username,from_avatar:user.avatar,text:"curtiu seu tópico",thread_id:threadId});
        }
      }catch{}
    ...
    try{
      ...
      try{
        ...
        if(mentions.length>0){
          ...
          if(mentionNotifs.length>0)await sb.from("notifications").insert(mentionNotifs);
        }
...
function AppInner() {
      ...
      if(mentions.length>0){
        try{
          ...
          for(const u of(mentioned||[])){
            if(u.id!==user.id) await sb.from("notifications").insert({user_id:u.id,type:"comment",from_username:user.username,from_avatar:user.avatar,text:`mencionou você: ${clean.substring(0,80)}`,diary_name:diary?.name||""});
          }
        }catch{}
      }
      try{await sb.rpc("check_and_grant_badges",{uid:user.id});}catch{}
  ...
  const renderPage=()=>{
    ...
    if(page==="concursos") return <ContestsPage user={user} onBack={()=>setPage("home")} onViewImage={setViewImage}/>;
