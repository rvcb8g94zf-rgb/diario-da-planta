...
function NPListItem({ article, showImage = true, showSummary = false, compact = false, onOpen }) {
...
function NPGridCard({ article, onOpen }) {
  const cat = NP_CAT_LABEL[article.category] || "Geral";
  return (
...
const newBlock = (type = "paragraph") => ({ id: "b" + Date.now().toString(36) + Math.random().toString(36).slice(2, 6), type, html: "", level: 2, url: "", caption: "", videoId: "", settings: {} });
...
function PortalPostView({ post, onBack, backLabel = "← Voltar ao portal" }) {
  ...
  return (
    <div style={{ maxWidth: "760px", margin: "0 auto" }}>
      ...
      <div style={{ fontFamily: NP.sans, fontSize: "12px", color: NP.dim, marginBottom: "20px" }}>
...
function PortalPostEditor({ post, user, onBack, onSaved }) {
  ...
  const save = async (mode) => { // 'draft' | 'publish'
    ...
    const payload = { title: cleanTitle, content_blocks: commit(), excerpt: excerpt.trim() || null, cover_url: coverUrl || null, category, tags, status, featured, published_at: pubAt, updated_at: new Date().toISOString() };
    try {
  ...
  const slashMenu = (b) => (
    <div style={{ position: "absolute", zIndex: 10, marginTop: "2px", background: "#fff", border: `1px solid ${NP.border}`, borderRadius: "6px", boxShadow: "0 4px 14px rgba(0,0,0,0.14)", width: "220px", overflow: "hidden" }}>
      {[["🖼", "Imagem", "image"], ["🅷", "Título", "heading"], ["≡", "Lista", "list"], ["❝", "Citação", "quote"], ["▬", "Separador", "divider"], ["▶", "Vídeo do YouTube", "youtube"]].map(([ic, lb, tp]) => (
            ...
            {sideTab === "doc" ? (
              <div>
                <div style={{ borderBottom: `1px solid ${NP.bgLight}`, padding: "12px 14px" }}>
                  ...
                  <div style={{ fontFamily: NP.sans, fontSize: "11px", color: NP.dim, marginBottom: "10px" }}>Visibilidade: <b style={{ color: NP.ciano }}>Público</b> (posts do portal são abertos a todos)</div>
              ...
              return (
                <div>
                  <div style={{ borderBottom: `1px solid ${NP.bgLight}`, padding: "12px 14px" }}>
                    <div style={{ fontFamily: NP.sans, fontSize: "12px", fontWeight: "700", color: NP.text }}>{sel.type === "paragraph" ? "¶ Parágrafo" : sel.type === "heading" ? "🅷 Título" : sel.type === "quote" ? "❝ Citação" : sel.type === "list" ? "≡ Lista" : sel.type === "image" ? "🖼 Imagem" : sel.type === "youtube" ? "▶ Vídeo" : "▬ Separador"}</div>
                  </div>
                  {sel.type === "heading" && (
                    <div style={{ borderBottom: `1px solid ${NP.bgLight}`, padding: "12px 14px" }}>
...
function NewsPortal({ onEnterApp, dark, onToggleDark, embedded = false, loggedUser = null, onLogout = null }) {
  ...
  const openPost = (a) => setView({ name: "post", post: a });
  ...
  const loadNews = async (offset = 0, append = false) => {
    ...
    try {
      ...
      if (append) setArticles((prev) => [...prev, ...page]);
        ...
        const mappedPosts = posts.map((p) => ({ id: "pp-" + p.id, __post: true, postData: p, title: p.title, summary: p.excerpt, url: "#", image_url: p.cover_url, source_name: "Diário da Planta", source_lang: "pt", category: p.category || "geral", published_at: p.published_at, featured: !!p.featured }));
  ...
  const featured = articles.find((a) => a.__post && a.featured) || articles.find((a) => a.featured) || articles[0];
    ...
    <div style={{ minHeight: "100vh", background: NP.white, color: NP.text, fontFamily: NP.serif, overflowX: "hidden" }}>
        ...
        <>
          ...
          <div style={{ background: NP.indigo, color: NP.white, fontFamily: NP.sans, fontSize: "12px", padding: "8px 20px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "8px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
              ...
              {!isMobile && <><span style={{ color: "rgba(255,255,255,0.4)" }}>|</span>
      ...
      {view.name !== "feed" ? (
        <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "20px 16px" }}>
          {view.name === "post" ? (
            <PortalPostView post={view.post} onBack={() => setView({ name: "feed" })} />
          ) : view.name === "manage" ? (
        ...
        <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "20px 16px" }}>
          {/* ─── Mosaico de destaque ─── */}
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1.7fr 1fr", gap: "16px", marginBottom: "8px" }}>
            {featured && (
              <a href={featured.__post ? "#" : featured.url} target={featured.__post ? undefined : "_blank"} rel={featured.__post ? undefined : "noopener noreferrer"} onClick={featured.__post ? (e) => { e.preventDefault(); openPost(featured); } : undefined} style={{ position: "relative", borderRadius: "3px", overflow: "hidden", minHeight: isMobile ? "260px" : "340px", textDecoration: "none", display: "block" }}>
                <div style={{ position: "absolute", inset: 0, background: featured.image_url ? `url(${featured.image_url}) center/cover no-repeat, linear-gradient(135deg, ${NP.ciano}, ${NP.indigo})` : `linear-gradient(135deg, ${NP.ciano}, ${NP.indigo})` }} />
                ...
                <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "24px" }}>
                  <span style={{ display: "inline-block", fontFamily: NP.sans, fontSize: "11px", fontWeight: "700", color: NP.white, background: NP.amber, padding: "4px 12px", textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: "12px" }}>{(featured.__post ? "Original · " : "") + (NP_CAT_LABEL[featured.category] || "Destaque")}</span>
                  <h2 style={{ fontFamily: NP.serif, fontSize: isMobile ? "22px" : "28px", fontWeight: "700", color: NP.white, margin: "0 0 8px", lineHeight: "1.18" }}>{featured.title}</h2>
            ...
            <div>{heroSide.map((a) => <NPListItem key={a.id} article={a} onOpen={openPost} />)}</div>
          </div>
          ...
          {(colLatest.length > 0 || colArticles.length > 0) && (
            <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: "0", borderTop: `1px solid ${NP.border}`, marginTop: "12px" }}>
              <div style={{ padding: isMobile ? "16px 0 0" : "16px 20px 0 0", borderRight: isMobile ? "none" : `1px solid ${NP.border}` }}>
                ...
                {colLatest.map((a) => <NPListItem key={a.id} article={a} compact onOpen={openPost} />)}
              </div>
              <div style={{ padding: isMobile ? "8px 0 0" : "16px 0 0 20px" }}>
                ...
                {colArticles.map((a) => <NPListItem key={a.id} article={a} compact onOpen={openPost} />)}
              </div>
            </div>
          )}
            ...
            <div>
              {cultivoArticles.length > 0 && (
                <>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(230px, 1fr))", gap: "16px", padding: "14px 0 8px" }}>
                    {cultivoArticles.map((a) => <NPGridCard key={a.id} article={a} onOpen={openPost} />)}
                  </div>
                </>
              )}
              {medicinalArticles.length > 0 && (
                <>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(230px, 1fr))", gap: "16px", padding: "14px 0 8px" }}>
                    {medicinalArticles.map((a) => <NPGridCard key={a.id} article={a} onOpen={openPost} />)}
                  </div>
                </>
              )}
              {/* Espaço publicitário */}
...
function AppInner() {
  ...
  const renderPage=()=>{
    ...
    if(page==="noticias") return <NewsPortal embedded onEnterApp={()=>{}} dark={dark} onToggleDark={toggleDark} loggedUser={user}/>;
