          ...
          {step===1?(<>
            ...
            <div style={{marginBottom:"20px",textAlign:"center"}}>
              ...
              <div onClick={()=>signupPhotoRef.current?.click()} style={{width:"80px",height:"80px",borderRadius:"50%",border:`3px dashed ${signupPhoto?C.accent:C.borderLight}`,background:signupPhoto?"transparent":C.surface2,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",margin:"8px auto",overflow:"hidden",transition:"all 0.2s"}}>
                {signupPhoto?<img src={signupPhoto} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:<span style={{fontSize:"28px",opacity:0.5}}>📷</span>}
              </div>
  ...
  return (
    <div style={{borderRadius:"14px",overflow:"hidden",border:`1px solid ${C.border}`,background:C.cardBg,transition:"all 0.3s",cursor:"pointer",transform:h?"translateY(-4px)":"none",boxShadow:h?"0 6px 24px rgba(0,0,0,0.1)":"0 1px 4px rgba(0,0,0,0.05)"}} onMouseEnter={()=>setH(true)} onMouseLeave={()=>setH(false)} onClick={onClick}>
      <div style={{height:"140px",background:COVER_GRADIENTS[(diary.cover||0)%6],display:"flex",alignItems:"center",justifyContent:"center",position:"relative",overflow:"hidden"}}>
        {diary.coverImage?<img src={diary.coverImage} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:<div dangerouslySetInnerHTML={{__html:generatePlantArt(diary.id.charCodeAt(1)*7+(diary.id.charCodeAt(0)||1),100)}} style={{opacity:0.8}}/>}
        ...
        <div style={{position:"relative",zIndex:1}}>
          ...
          <div style={{position:"relative",display:"inline-block",margin:"0 auto 16px"}}>
            ...
            <div style={{position:"absolute",bottom:diary.coverImage?"-6px":"-8px",right:diary.coverImage?"-6px":"-20px",width:"36px",height:"36px",borderRadius:"50%",background:C.cardBg,border:"2px solid #fff",boxShadow:"0 2px 6px rgba(0,0,0,0.15)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"18px",overflow:"hidden"}}>
              {diary.avatarImg?<img src={diary.avatarImg} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:diary.avatar||"🌱"}
            </div>
    ...
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.4)",backdropFilter:"blur(8px)",display:"flex",alignItems:"flex-start",justifyContent:"center",zIndex:300,padding:"20px",overflowY:"auto"}} onClick={onClose}>
      <div style={{...cardBase,maxWidth:"560px"}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{marginBottom:"16px"}}>
          ...
          {uploading&&<div style={{padding:"8px 12px",background:C.accentBg,borderRadius:"8px",marginBottom:"8px",fontFamily:F.sans,fontSize:"12px",color:C.accent}}>⏳ Enviando...</div>}
          {allMedia.length>0&&<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill, minmax(70px,1fr))",gap:"6px",marginBottom:"10px"}}>
            {allMedia.map(m=>(
              <div key={m.id} style={{position:"relative",borderRadius:"8px",overflow:"hidden",aspectRatio:"1",background:C.surface2,border:`1px solid ${C.border}`}}>
                {m.type==="video"?<div style={{width:"100%",height:"100%",display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{fontSize:"20px"}}>🎬</span></div>:<img src={m.url} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>}
...
function AdminPanel({ user, onBack, onNewPost }) {
  ...
  useEffect(()=>{(async()=>{
    try{
      ...
      // Load blog posts
      const bp=await sb.from("blog_posts").select("*,profiles(username)","&order=created_at.desc");
            ...
            {warnSearch&&!warnTarget&&<div style={{maxHeight:"150px",overflowY:"auto",border:`1px solid ${C.border}`,borderRadius:"8px",background:C.cardBg}}>
              {userList.filter(u=>(u.username+" "+u.email).toLowerCase().includes(warnSearch.toLowerCase())).slice(0,10).map(u=>(
                <div key={u.id} onClick={()=>{setWarnTarget(u.id);setWarnSearch("");}} style={{padding:"10px 14px",cursor:"pointer",display:"flex",alignItems:"center",gap:"10px",borderBottom:`1px solid ${C.border22}`,fontFamily:F.sans,fontSize:"13px"}} onMouseOver={e=>e.currentTarget.style.background=C.surface2} onMouseOut={e=>e.currentTarget.style.background="transparent"}>
                  <div style={{width:"28px",height:"28px",borderRadius:"50%",background:C.surface2,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"14px",overflow:"hidden",border:`1px solid ${C.border}`}}>{u.avatarImg?<img src={u.avatarImg} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:u.avatar}</div>
    ...
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:C.bg,zIndex:400,overflowY:"auto"}}>
      <div style={{maxWidth:"740px",margin:"0 auto",padding:"32px 24px"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:"10px",marginBottom:"32px",paddingBottom:"20px",borderBottom:`1px solid ${C.border}`}}>
          <div style={{width:"36px",height:"36px",borderRadius:"50%",background:C.surface2,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"20px",overflow:"hidden",border:`1px solid ${C.border}`}}>{user?.avatarImg?<img src={user.avatarImg} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:user?.avatar||"🌱"}</div>
        ...
        <div style={{display:"flex",gap:"24px",flexWrap:"wrap"}}>
          ...
          <div style={{width:"260px",flexShrink:0}}>
            ...
            <div style={{background:C.cardBg,borderRadius:"12px",border:`1px solid ${C.border}`,padding:"16px",marginBottom:"16px"}}>
              ...
              {coverUrl?<div style={{position:"relative",marginBottom:"8px"}}><img src={coverUrl} alt="" style={{width:"100%",borderRadius:"8px",maxHeight:"150px",objectFit:"cover"}}/><button onClick={()=>setCoverUrl(null)} style={{position:"absolute",top:"4px",right:"4px",width:"24px",height:"24px",borderRadius:"50%",border:"none",background:"rgba(0,0,0,0.5)",color:"#fff",cursor:"pointer",fontSize:"12px"}}>✕</button></div>:<div onClick={()=>coverRef.current?.click()} style={{border:`2px dashed ${C.borderLight}`,borderRadius:"8px",padding:"20px",textAlign:"center",cursor:"pointer"}}><div style={{fontSize:"24px",marginBottom:"4px"}}>🖼️</div><div style={{fontFamily:F.sans,fontSize:"11px",color:C.dim}}>Definir imagem destacada</div></div>}
        ...
        {posts.map(p=>{
          ...
          <div key={p.id} onClick={()=>onOpenPost?.(p)} style={{background:C.cardBg,borderRadius:"16px",border:`1px solid ${C.border}`,overflow:"hidden",cursor:"pointer",transition:"all 0.2s",boxShadow:"0 2px 8px rgba(0,0,0,0.04)"}}>
            {previewImg&&<div style={{position:"relative"}}>
              <img src={previewImg} alt="" style={{width:"100%",height:"200px",objectFit:"cover"}}/>
            ...
            <div style={{padding:"20px 24px"}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:"8px"}}>
                <div style={{width:"24px",height:"24px",borderRadius:"50%",background:C.surface2,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"14px",overflow:"hidden",border:`1px solid ${C.border}`}}>{p.authorAvatarImg?<img src={p.authorAvatarImg} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:p.authorAvatar||"🌱"}</div>
...
function BlogPostView({post,onBack,user,onEdit,onViewImage}){
  ...
  return(
    <div style={{maxWidth:"740px",margin:"0 auto",padding:"32px 24px"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:"10px",marginBottom:"32px",paddingBottom:"20px",borderBottom:`1px solid ${C.border}`}}>
        <div style={{width:"36px",height:"36px",borderRadius:"50%",background:C.surface2,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"20px",overflow:"hidden",border:`1px solid ${C.border}`}}>{post.authorAvatarImg?<img src={post.authorAvatarImg} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:post.authorAvatar||"🌱"}</div>
        ...
        {filtered.map((g,idx)=>(
          <div key={g.id} onClick={()=>onViewProfile?.(g)} style={{background:C.cardBg,borderRadius:"16px",border:`1px solid ${C.border}`,padding:"20px 16px",textAlign:"center",cursor:"pointer",transition:"all 0.2s",boxShadow:"0 2px 8px rgba(0,0,0,0.04)",position:"relative"}}>
            ...
            <div style={{width:"64px",height:"64px",borderRadius:"50%",margin:"0 auto 12px",overflow:"hidden",border:`3px solid ${g.level?.color||C.accent}`,display:"flex",alignItems:"center",justifyContent:"center",background:C.surface2,fontSize:"28px"}}>
              {g.avatar_url?<img src={g.avatar_url} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:g.avatar||"🌱"}
            </div>
...
function ForumPage({user,onBack}){
  const [categories,setCategories]=useState([]);const [topics,setTopics]=useState([]);
  const [threads,setThreads]=useState([]);const [replies,setReplies]=useState([]);
  const [loading,setLoading]=useState(true);const [latestThreads,setLatestThreads]=useState([]);
  const [view,setView]=useState("categories");
  const [selCat,setSelCat]=useState(null);const [selTopic,setSelTopic]=useState(null);const [selThread,setSelThread]=useState(null);
  const [newTitle,setNewTitle]=useState("");const [newContent,setNewContent]=useState("");
  const [replyText,setReplyText]=useState("");const [showNewThread,setShowNewThread]=useState(false);
  const [posting,setPosting]=useState(false);const [quoteReply,setQuoteReply]=useState(null);
  const [uploading,setUploading]=useState(false);
  const threadContentRef=useRef(null);const replyContentRef=useRef(null);const mediaRef=useRef(null);const replyMediaRef=useRef(null);
  ...
  useEffect(()=>{(async()=>{
    try{
      ...
      // Load latest thread per category for preview
      const lt=await sb.from("forum_threads").select("id,title,topic_id,created_at,profiles(username)",`&order=created_at.desc&limit=20`);
      setCategories(cats);setTopics(tops);setLatestThreads(lt);
    }catch{}setLoading(false);
  })();},[]);

  const loadThreads=async(topicId)=>{try{const rows=await sb.from("forum_threads").select("*,profiles(username,avatar,avatar_url)",`&topic_id=eq.${topicId}&order=pinned.desc,updated_at.desc`);setThreads(rows);}catch{}};
  const loadReplies=async(threadId)=>{try{const rows=await sb.from("forum_replies").select("*,profiles(username,avatar,avatar_url)",`&thread_id=eq.${threadId}&order=created_at.asc`);setReplies(rows);}catch{}};

  const openCategory=(cat)=>{setSelCat(cat);setView("topics");};
  const openTopic=async(topic)=>{setSelTopic(topic);await loadThreads(topic.id);setView("threads");};
  const openThread=async(thread)=>{setSelThread(thread);await loadReplies(thread.id);setView("thread");};
  const goBack=()=>{
    if(view==="thread"){setView("threads");setSelThread(null);setReplies([]);setQuoteReply(null);}
    else if(view==="threads"){setView("topics");setSelTopic(null);setThreads([]);}
    else if(view==="topics"){setView("categories");setSelCat(null);}
  };

  // Rich text toolbar insert
  const insertFmt=(ref,setter,val,tag)=>{
    const ta=ref.current;if(!ta)return;
    const start=ta.selectionStart,end=ta.selectionEnd;const sel=val.substring(start,end);
    let ins="";
    if(tag==="b")ins=`**${sel||"negrito"}**`;
    else if(tag==="i")ins=`*${sel||"itálico"}*`;
    else if(tag==="link")ins=`[${sel||"link"}](https://)`;
    else if(tag==="quote")ins=`\n> ${sel||"citação"}\n`;
    setter(val.substring(0,start)+ins+val.substring(end));
  };

  // Upload media for threads/replies
  const handleForumMedia=async(ref,setter,val,targetRef)=>{
    const f=ref.current?.files?.[0];if(!f)return;
    setUploading(true);
    try{
      const ext=f.name.split(".").pop()||"jpg";
      const path=`forum/${Date.now()}-${Math.random().toString(36).slice(2,5)}.${ext}`;
      const ok=await sbStorage.upload(path,f);
      if(ok){
        const url=sbStorage.getUrl(path);
        const isVideo=f.type.startsWith("video");
        ...
        setter(val.substring(0,pos)+insert+val.substring(pos));
      }
  ...
  const insertYT=(setter,val,targetRef)=>{
    const url=prompt("Cole a URL do YouTube:");
    if(!url)return;
    const m=url.match(/(?:youtube\.com\/(?:watch\?v=|embed\/|shorts\/)|youtu\.be\/)([a-zA-Z0-9_-]{11})/);
    if(!m){alert("URL inválida");return;}
    const insert=`\n[youtube:${m[1]}]\n`;
    const ta=targetRef.current;
    const pos=ta?ta.selectionStart:val.length;
    setter(val.substring(0,pos)+insert+val.substring(pos));
  ...
  // Toolbar component
  const RichToolbar=({contentRef:cRef,content,setContent})=>(
    <div style={{display:"flex",gap:"4px",flexWrap:"wrap",marginBottom:"8px",padding:"8px",background:C.surface2,borderRadius:"8px",border:`1px solid ${C.border}`}}>
      {[["B","b"],["I","i"],["❝","quote"],["🔗","link"]].map(([icon,tag])=>
        <button key={tag} onClick={()=>insertFmt(cRef,setContent,content,tag)} style={{width:"32px",height:"32px",borderRadius:"6px",border:`1px solid ${C.border}`,background:C.cardBg,color:C.text,cursor:"pointer",fontSize:"13px",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:tag==="b"?"800":"400"}}>{icon}</button>
      )}
      <div style={{width:"1px",height:"24px",background:C.border,margin:"4px 2px"}}/>
      <button onClick={()=>mediaRef.current?.click()} title="Inserir foto/vídeo" style={{width:"32px",height:"32px",borderRadius:"6px",border:`1px solid ${C.accent}`,background:C.accentBg,color:C.accent,cursor:"pointer",fontSize:"13px",display:"flex",alignItems:"center",justifyContent:"center"}}>📷</button>
      <button onClick={()=>insertYT(setContent,content,cRef)} title="YouTube" style={{width:"32px",height:"32px",borderRadius:"6px",border:"1px solid #e53e3e",background:"rgba(229,62,62,0.06)",color:"#e53e3e",cursor:"pointer",fontSize:"13px",display:"flex",alignItems:"center",justifyContent:"center"}}>▶️</button>
      <input ref={mediaRef} type="file" accept="image/*,video/*" style={{display:"none"}} onChange={()=>handleForumMedia(mediaRef,setContent,content,cRef)}/>
      {uploading&&<span style={{fontSize:"11px",color:C.accent,fontFamily:F.sans,display:"flex",alignItems:"center"}}>⏳ Enviando...</span>}
    </div>
  );
  ...
  // Get latest thread for a category
  const getLatestForCat=(catId)=>{
    const topicIds=catTopics(catId).map(t=>t.id);
    const match=latestThreads.find(t=>topicIds.includes(t.topic_id));
    return match;
  };

  // Render content with markdown/YouTube/images
  const renderForumContent=(text)=>{
    if(!text)return"";
    return text
      ...
      .replace(/!\[([^\]]*)\]\(([^)]+)\)/g,'<div style="margin:12px 0"><img src="$2" alt="$1" style="max-width:100%;border-radius:10px"/></div>')
      .replace(/\*\*(.+?)\*\*/g,"<strong>$1</strong>")
      .replace(/\*(.+?)\*/g,"<em>$1</em>")
      .replace(/^> (.+)$/gm,'<blockquote style="border-left:3px solid #1B9E42;padding:6px 14px;color:#888;margin:8px 0;font-style:italic;background:rgba(27,158,66,0.04);border-radius:0 8px 8px 0">$1</blockquote>')
  ...
  if(view==="thread"&&selThread) return(
    <div style={{maxWidth:"800px",margin:"0 auto",padding:"24px"}}>
      <button onClick={goBack} style={{padding:"8px 16px",borderRadius:"20px",border:`1px solid ${C.border}`,background:C.cardBg,color:C.muted,cursor:"pointer",fontSize:"13px",fontFamily:F.sans,marginBottom:"20px"}}>← Voltar</button>
      <div style={{background:C.cardBg,borderRadius:"16px",border:`1px solid ${C.border}`,padding:"20px 24px",marginBottom:"16px"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:"10px",marginBottom:"12px"}}>
          <div style={{width:"32px",height:"32px",borderRadius:"50%",background:C.surface2,overflow:"hidden",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"16px",border:`1px solid ${C.border}`}}>{selThread.profiles?.avatar_url?<img src={selThread.profiles.avatar_url} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:selThread.profiles?.avatar||"🌱"}</div>
          <span style={{fontFamily:F.sans,fontSize:"13px",fontWeight:"600"}}>{selThread.profiles?.username}</span>
          <span style={{fontFamily:F.sans,fontSize:"12px",color:C.dim}}>{fmtDate(selThread.created_at)}</span>
        </div>
        <div style={{fontFamily:F.body,fontSize:"15px",color:C.text,lineHeight:"1.7"}} dangerouslySetInnerHTML={{__html:renderForumContent(selThread.content)}}/>
      </div>
      {/* Replies */}
      ...
      {replies.map(r=>(
        <div key={r.id} style={{background:C.cardBg,borderRadius:"12px",border:`1px solid ${C.border}`,padding:"14px 18px",marginBottom:"8px"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"8px"}}>
            <div style={{display:"flex",alignItems:"center",gap:"8px"}}>
              <div style={{width:"28px",height:"28px",borderRadius:"50%",background:C.surface2,overflow:"hidden",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"14px",border:`1px solid ${C.border}`}}>{r.profiles?.avatar_url?<img src={r.profiles.avatar_url} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:r.profiles?.avatar||"🌱"}</div>
              <span style={{fontFamily:F.sans,fontSize:"13px",fontWeight:"600"}}>{r.profiles?.username}</span>
              <span style={{fontFamily:F.sans,fontSize:"11px",color:C.dim}}>{fmtDate(r.created_at)}</span>
            </div>
            <button onClick={()=>{setQuoteReply(r);setReplyText(prev=>prev+`\n> ${r.profiles?.username} disse: ${r.content.substring(0,100)}${r.content.length>100?"...":""}\n\n`);}} title="Citar" style={{padding:"4px 10px",borderRadius:"6px",border:`1px solid ${C.border}`,background:C.surface2,color:C.dim,cursor:"pointer",fontSize:"11px",fontFamily:F.sans}}>❝ Citar</button>
          </div>
          <div style={{fontFamily:F.body,fontSize:"14px",color:C.text,lineHeight:"1.6"}} dangerouslySetInnerHTML={{__html:renderForumContent(r.content)}}/>
        </div>
      ))}
      {/* Reply input with toolbar */}
      <div style={{marginTop:"16px",background:C.cardBg,borderRadius:"12px",border:`1px solid ${C.border}`,padding:"16px"}}>
        ...
        <RichToolbar contentRef={replyContentRef} content={replyText} setContent={setReplyText}/>
        <textarea ref={replyContentRef} style={{...baseInput,minHeight:"100px",resize:"vertical",marginBottom:"10px",fontFamily:"monospace",fontSize:"13px"}} value={replyText} onChange={e=>setReplyText(e.target.value)} placeholder="Escreva sua resposta... Use **negrito**, *itálico*, > citação"/>
        <div style={{display:"flex",gap:"8px",alignItems:"center"}}>
          <button onClick={()=>replyMediaRef.current?.click()} style={{padding:"8px 14px",borderRadius:"8px",border:`1px solid ${C.border}`,background:C.surface2,color:C.muted,cursor:"pointer",fontSize:"12px",fontFamily:F.sans}}>📎 Anexar</button>
          <input ref={replyMediaRef} type="file" accept="image/*,video/*" style={{display:"none"}} onChange={()=>handleForumMedia(replyMediaRef,setReplyText,replyText,replyContentRef)}/>
          <button onClick={postReply} disabled={posting||!replyText.trim()} style={{...btnPrimary,width:"auto",padding:"10px 24px",opacity:(posting||!replyText.trim())?0.5:1}}>{posting?"Enviando...":"Postar Resposta"}</button>
        </div>
      </div>
    </div>
  );
  ...
  if(view==="threads"&&selTopic) return(
    <div style={{maxWidth:"800px",margin:"0 auto",padding:"24px"}}>
      <button onClick={goBack} style={{padding:"8px 16px",borderRadius:"20px",border:`1px solid ${C.border}`,background:C.cardBg,color:C.muted,cursor:"pointer",fontSize:"13px",fontFamily:F.sans,marginBottom:"20px"}}>← Voltar</button>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"24px"}}>
        <div><h1 style={{fontFamily:F.sans,fontSize:"24px",fontWeight:"800",margin:0}}>{selTopic.emoji} {selTopic.name}</h1>{selTopic.description&&<p style={{fontFamily:F.sans,fontSize:"13px",color:C.muted,margin:"4px 0 0"}}>{selTopic.description}</p>}</div>
        <button onClick={()=>setShowNewThread(true)} style={{...btnPrimary,width:"auto",padding:"10px 20px",fontSize:"13px"}}>+ Novo Tópico</button>
      </div>
      {threads.length===0?<div style={{textAlign:"center",padding:"40px",color:C.dim,fontFamily:F.sans}}>Nenhum tópico ainda. Seja o primeiro a criar!</div>:
      threads.map(t=>(
        <div key={t.id} onClick={()=>openThread(t)} style={{background:C.cardBg,borderRadius:"12px",border:`1px solid ${t.pinned?C.accent+"44":C.border}`,padding:"14px 18px",marginBottom:"8px",cursor:"pointer",display:"flex",alignItems:"center",gap:"14px"}}>
          <div style={{width:"40px",height:"40px",borderRadius:"10px",background:C.surface2,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"18px",flexShrink:0,overflow:"hidden",border:`1px solid ${C.border}`}}>{t.profiles?.avatar_url?<img src={t.profiles.avatar_url} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:t.profiles?.avatar||"💬"}</div>
          <div style={{flex:1,minWidth:0}}>
            <div style={{display:"flex",alignItems:"center",gap:"6px"}}>{t.pinned&&<span style={{fontSize:"10px",color:C.accent}}>📌</span>}<h3 style={{fontFamily:F.sans,fontSize:"15px",fontWeight:"700",margin:0,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{t.title}</h3></div>
            <div style={{fontFamily:F.sans,fontSize:"12px",color:C.dim,marginTop:"2px"}}>{t.profiles?.username} · {fmtDate(t.created_at)}</div>
          </div>
        </div>
      ))}
      {/* New thread modal with rich toolbar */}
      {showNewThread&&<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.4)",backdropFilter:"blur(8px)",display:"flex",alignItems:"flex-start",justifyContent:"center",zIndex:300,padding:"40px 20px",overflowY:"auto"}} onClick={()=>setShowNewThread(false)}>
        <div style={{...cardBase,maxWidth:"600px"}} onClick={e=>e.stopPropagation()}>
          ...
          <div style={{marginBottom:"16px"}}><label style={labelSt}>Mensagem *</label>
            <RichToolbar contentRef={threadContentRef} content={newContent} setContent={setNewContent}/>
            <textarea ref={threadContentRef} style={{...baseInput,minHeight:"160px",resize:"vertical",fontFamily:"monospace",fontSize:"13px"}} value={newContent} onChange={e=>setNewContent(e.target.value)} placeholder={"Escreva sua mensagem...\n\n**negrito** *itálico* > citação\n📷 Use a barra para inserir fotos\n▶️ Insira vídeos do YouTube"}/>
          </div>
          <div style={{display:"flex",gap:"12px"}}><button onClick={()=>setShowNewThread(false)} style={btnSecondary}>Cancelar</button><button onClick={createThread} disabled={posting||!newTitle.trim()||!newContent.trim()} style={{...btnPrimary,opacity:(posting||!newTitle.trim()||!newContent.trim())?0.5:1}}>{posting?"Criando...":"Criar Tópico"}</button></div>
        </div>
      </div>}
    </div>
  );
    ...
    <div style={{maxWidth:"800px",margin:"0 auto",padding:"24px"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:"12px"}}>
        {catTopics(selCat.id).map(t=>(
          <div key={t.id} onClick={()=>openTopic(t)} style={{background:C.cardBg,borderRadius:"14px",border:`1px solid ${C.border}`,padding:"18px",cursor:"pointer",transition:"all 0.2s",boxShadow:"0 2px 8px rgba(0,0,0,0.04)"}}>
            <div style={{fontSize:"28px",marginBottom:"8px"}}>{t.emoji}</div>
            <h3 style={{fontFamily:F.sans,fontSize:"16px",fontWeight:"700",margin:"0 0 4px"}}>{t.name}</h3>
            {t.description&&<p style={{fontFamily:F.sans,fontSize:"12px",color:C.muted,margin:0}}>{t.description}</p>}
          </div>
        ))}
      </div>
    </div>
  ...
  // Categories view (main) with latest activity
  return(
    <div style={{maxWidth:"800px",margin:"0 auto",padding:"24px"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:"16px"}}>
        {categories.map(cat=>{
          ...
          return(
          <div key={cat.id} onClick={()=>openCategory(cat)} style={{background:C.cardBg,borderRadius:"16px",border:`1px solid ${C.border}`,padding:"20px",cursor:"pointer",transition:"all 0.2s",boxShadow:"0 2px 8px rgba(0,0,0,0.04)"}}>
            <div style={{display:"flex",alignItems:"center",gap:"14px"}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                ...
                {latest&&<div style={{marginTop:"8px",padding:"6px 10px",background:C.surface2,borderRadius:"8px",display:"flex",alignItems:"center",gap:"6px"}}>
                  <span style={{fontSize:"10px"}}>💬</span>
                  <span style={{fontFamily:F.sans,fontSize:"11px",color:C.dim,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}><strong>{latest.profiles?.username}</strong>: {latest.title} · {fmtDate(latest.created_at)}</span>
                </div>}
              </div>
...
function AppInner() {
  ...
  // Search filter
  const searchFiltered=searchQ?allDiaries.filter(d=>(d.name+" "+d.strain+" "+d.author+(d.strains?d.strains.join(" "):"")+(d.tags?d.tags.join(" "):"")).toLowerCase().includes(searchQ.toLowerCase())):allDiaries;
