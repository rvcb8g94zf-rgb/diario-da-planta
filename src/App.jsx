...
function AppInner() {
  ...
  const newDM=async(username,firstMsg)=>{
    ...
    if(!cleanMsg){alert("Mensagem vazia.");return;}
    ...
    try{
      // Exact match (case-insensitive) via eq.ilike
      ...
      const conv=await sb.from("conversations").insert({is_group:false});
      if(!conv?.id){alert("Erro ao criar conversa. Tente novamente.");console.error("Conv insert returned:",conv);return;}
      const memIns=await sb.from("conversation_members").insert([
      ...
      console.log("Members inserted:",memIns);
      ...
      setMsgs(p=>[{
        ...
        isGroup:false,
        members:[],
        messages:[{id:"m"+Date.now(),from:user.email,text:cleanMsg,time:Date.now()}],
        readAt:Date.now(),
      },...p]);
    }catch(e){
