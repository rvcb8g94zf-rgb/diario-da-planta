...
function CreateDiaryModal({ user, onClose, onSave }) {
  ...
  const handleSave=()=>{
    ...
    onSave({id:"u"+Date.now(),name:name.trim(),strain:validStrains.join(", "),strains:validStrains,author:user.username,authorId:user.email,phase:0,week:0,env,light,substrate,likes:0,comments:0,avatar:user.avatar,cover:Math.floor(Math.random()*6),techniques:techs,weeks:[],isOwn:true,watering,germination,numPlants:parseInt(numPlants)||1,watts:watts?parseInt(watts):null,tags:tags.split(",").map(t=>t.trim().replace(/^#/,"")).filter(Boolean)});
  };
  ...
  return (
    <div style={{maxWidth:"900px",margin:"0 auto",padding:"24px"}}>
      ...
      <div style={{display:"flex",gap:"6px",marginBottom:"20px",flexWrap:"wrap"}}>
        {tabBtn("dashboard","📊","Dashboard")}{tabBtn("users","👥","Usuários")}{tabBtn("diaries","📓","Diários")}{tabBtn("comments","💬","Comentários")}{tabBtn("blog","📰","Blog",blogPosts.filter(p=>p.status==="draft").length)}{tabBtn("reports","🚨","Denúncias",pendingReports)}{tabBtn("announce","📢","Anúncios")}{tabBtn("warnings","⚠️","Avisos")}{tabBtn("log","📋","Auditoria")}
      </div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:"8px"}}>
          {blogPosts.map(p=>(
            <div key={p.id} style={{background:C.cardBg,borderRadius:"12px",border:`1px solid ${C.border}`,padding:"14px 16px",display:"flex",alignItems:"center",gap:"12px"}}>
              ...
              <button onClick={async()=>{if(confirm("Deletar post '"+p.title+"'?")){try{await sb.from("blog_posts").delete(`id=eq.${p.id}`);setBlogPosts(prev=>prev.filter(x=>x.id!==p.id));addAudit("Deletou post",p.title,"");}catch{}}}} style={{padding:"6px 10px",borderRadius:"8px",border:`1px solid ${C.error}33`,background:C.error+"08",color:C.error,cursor:"pointer",fontSize:"11px",fontFamily:F.sans,flexShrink:0}}>🗑️</button>
            </div>
          ))}
        </div>}
      </div>}
...
function AppInner() {
  ...
  const allDiaries=page==="meus"?myDiaries.filter(d=>!d.hidden):page==="favoritos"?allPool.filter(d=>favs[d.id]):page==="gostei"?allPool.filter(d=>likes[d.id]):page==="feed"?allPool.filter(d=>follows.includes(d.authorId)):allPool;
  ...
  const pageSub=page==="meus"?t.manageGrows:page==="feed"?"Diários de quem você segue":page==="favoritos"||page==="gostei"?"":t.followGrowers;
      ...
      <div className="dp-section" style={{padding:"32px 20px",maxWidth:"1100px",margin:"0 auto"}}>
        ...
        {filtered.length>0?(
          ...
          <div style={{textAlign:"center",padding:"60px 24px",color:C.dim}}>
            ...
            <p style={{fontFamily:F.body,fontSize:"16px",marginBottom:"16px"}}>{page==="meus"?t.noDiaries:page==="favoritos"?"Nenhum diário favoritado ainda.":page==="gostei"?"Nenhum diário curtido ainda.":page==="feed"?"Siga cultivadores para ver seus diários aqui!":t.noResults}</p>
