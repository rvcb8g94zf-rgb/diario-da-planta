...
function ForumPage({user,onBack,onReport}){
      ...
      <div style={{background:C.cardBg,borderRadius:"16px",border:"1px solid "+C.border,padding:"20px 24px",marginBottom:"16px"}}>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:"10px",marginBottom:"12px",flexWrap:"wrap"}}>
          ...
          <div style={{display:"flex",gap:"6px"}}>
            ...
            {user&&selThread.author_id!==user.id&&<button type="button" onClick={()=>onReport?.("thread",selThread.id,selThread.title)} title="Denunciar" style={{padding:"6px 12px",borderRadius:"8px",border:`1px solid ${C.border}`,background:C.surface2,color:C.muted,cursor:"pointer",fontSize:"12px",fontFamily:F.sans}}>🚩 Denunciar</button>}
          </div>
        </div>
...
function AppInner() {
  ...
  const sendMsg=async(convId,text)=>{
    ...
    // If this conversation has any system message with a report_number, inherit it
    // so admin can trace replies back to the original denúncia
    const conv=msgs.find(c=>c.id===convId);
    const lastSystemRef=conv?.messages?.filter(m=>m.isSystem&&m.reportNumber).slice(-1)[0]?.reportNumber||null;
    try{
      const payload={conversation_id:convId,sender_id:user.id,text:clean};
      if(lastSystemRef)payload.report_number=lastSystemRef;
      await sb.from("messages").insert(payload);
      setMsgs(p=>p.map(c=>c.id===convId?{...c,messages:[...c.messages,{id:"m"+Date.now(),from:user.email,text:clean,time:Date.now(),reportNumber:lastSystemRef}],readAt:Date.now()}:c));
  ...
  const renderPage=()=>{
    ...
    if(page==="comunidade") return <ForumPage user={user} onBack={()=>setPage("home")} onReport={openReport}/>;
