...
function ForumPage({user,onBack}){
  ...
  const [loading,setLoading]=useState(true);
  const [view,setView]=useState("categories"); // categories | topics | threads | thread
  ...
  const loadThreads=async(topicId)=>{
    try{
      const rows=await sb.from("forum_threads").select("*,profiles(username,avatar,avatar_url)",`&topic_id=eq.${topicId}&order=pinned.desc,updated_at.desc`);
      setThreads(rows);
    }catch{}
  };
  const loadReplies=async(threadId)=>{
    try{
      const rows=await sb.from("forum_replies").select("*,profiles(username,avatar,avatar_url)",`&thread_id=eq.${threadId}&order=created_at.asc`);
      setReplies(rows);
      // Increment views
      try{await fetch(`${SB_URL}/rest/v1/rpc/`,{method:"POST"});}catch{} // views increment is best-effort
    }catch{}
  };
  ...
  const createThread=async()=>{
    ...
    setPosting(true);
    try{
      await sb.from("forum_threads").insert({topic_id:selTopic.id,author_id:user.id,title:sanitize(newTitle,200),content:sanitize(newContent,5000)});
  ...
  const postReply=async()=>{
    ...
    setPosting(true);
    try{
      await sb.from("forum_replies").insert({thread_id:selThread.id,author_id:user.id,content:sanitize(replyText,5000)});
  ...
  if(view==="thread"&&selThread) return(
    <div style={{maxWidth:"800px",margin:"0 auto",padding:"24px"}}>
      ...
      <div style={{background:C.cardBg,borderRadius:"16px",border:`1px solid ${C.border}`,padding:"20px 24px",marginBottom:"16px"}}>
        ...
        <div style={{fontFamily:F.body,fontSize:"15px",color:C.text,lineHeight:"1.7",whiteSpace:"pre-wrap"}}>{selThread.content}</div>
      </div>
      {/* Replies */}
      ...
      {replies.map(r=>(
        <div key={r.id} style={{background:C.cardBg,borderRadius:"12px",border:`1px solid ${C.border}`,padding:"14px 18px",marginBottom:"8px"}}>
          ...
          <div style={{fontFamily:F.body,fontSize:"14px",color:C.text,lineHeight:"1.6",whiteSpace:"pre-wrap"}}>{r.content}</div>
        </div>
      ))}
      {/* Reply input */}
      <div style={{marginTop:"16px",background:C.cardBg,borderRadius:"12px",border:`1px solid ${C.border}`,padding:"16px"}}>
        ...
        <textarea style={{...baseInput,minHeight:"80px",resize:"vertical",marginBottom:"10px"}} value={replyText} onChange={e=>setReplyText(e.target.value)} placeholder="Escreva sua resposta..."/>
  ...
  if(view==="threads"&&selTopic) return(
    <div style={{maxWidth:"800px",margin:"0 auto",padding:"24px"}}>
      ...
      {/* New thread modal */}
      {showNewThread&&<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.4)",backdropFilter:"blur(8px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:300,padding:"20px"}} onClick={()=>setShowNewThread(false)}>
        <div style={{...cardBase,maxWidth:"560px"}} onClick={e=>e.stopPropagation()}>
          ...
          <div style={{marginBottom:"16px"}}><label style={labelSt}>Mensagem *</label><textarea style={{...baseInput,minHeight:"120px",resize:"vertical"}} value={newContent} onChange={e=>setNewContent(e.target.value)} placeholder="Escreva sua mensagem..."/></div>
