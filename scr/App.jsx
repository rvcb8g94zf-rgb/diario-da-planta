...
const sbAuth = {
  signUp: async (email, password, metadata={}) => {
    ...
    const data = await res.json(); if(data.error) throw new Error(data.error.message||data.msg||"Erro no cadastro");
  ...
  signIn: async (email, password) => {
    ...
    const data = await res.json(); if(data.error) throw new Error(data.error_description||data.error||"Email ou senha incorretos");
...
const sbStorage = {
  upload: async (path, file) => {
    const res = await fetch(`${SB_URL}/storage/v1/object/media/${path}`, {
      method:"POST", headers:{"apikey":SB_KEY,"Authorization":`Bearer ${_accessToken}`,"Content-Type":file.type},
      body:file
    });
    return res.ok;
  },
...
const SAMPLE_DIARIES = [
  { id: "s1", name: "Primeira vez indoor", strain: "Amnesia Haze Auto", authorId: "bot1", author: "VerdeBR", phase: 2, week: 6, env: "Indoor", light: "LED", substrate: "Coco", likes: 47, comments: 12, avatar: "🧑‍🌾", cover: 0, techniques: ["LST","Topping"], weeks: [
    { week:1,phase:0,height:"3",temp:"25",humidity:"70",ph:"6.5",waterMl:"100",lightHours:"18",note:"Semente germinada em papel toalha após 48h." },
    { week:2,phase:0,height:"6",temp:"26",humidity:"68",ph:"6.3",waterMl:"150",lightHours:"18",note:"Primeiro par de folhas verdadeiras." },
    { week:3,phase:1,height:"12",temp:"25",humidity:"60",ph:"6.4",waterMl:"250",lightHours:"18",note:"Início da vegetação. Planta vigorosa." },
    { week:4,phase:1,height:"20",temp:"26",humidity:"55",ph:"6.5",waterMl:"400",lightHours:"18",note:"Topping no 5º nó. Respondeu bem." },
    { week:5,phase:2,height:"35",temp:"24",humidity:"50",ph:"6.2",waterMl:"600",lightHours:"12",note:"Mudou para 12/12. Pistilos aparecendo!" },
    { week:6,phase:2,height:"42",temp:"24",humidity:"48",ph:"6.3",waterMl:"700",lightHours:"12",note:"Flores engrossando, cheiro forte." },
  ] },
  { id:"s2",name:"Varanda orgânica",strain:"Northern Lights",authorId:"bot2",author:"GrowSP",phase:3,week:12,env:"Outdoor",light:"Sol Natural",substrate:"Solo",likes:89,comments:34,avatar:"👨‍🌾",cover:1,techniques:["Desfolha"],weeks:[] },
  { id:"s3",name:"Hidro no armário",strain:"White Widow Auto",authorId:"bot3",author:"AquaGrow",phase:1,week:3,env:"Indoor",light:"LED",substrate:"Hidroponia",likes:23,comments:8,avatar:"🌻",cover:2,techniques:["ScrOG"],weeks:[] },
  { id:"s4",name:"Jardim secreto",strain:"Gorilla Glue #4",authorId:"bot4",author:"MataVerde",phase:2,week:8,env:"Outdoor",light:"Sol Natural",substrate:"Solo",likes:156,comments:56,avatar:"🌴",cover:3,techniques:["LST","Desfolha","Topping"],weeks:[] },
  { id:"s5",name:"Estufa tropical",strain:"Sour Diesel",authorId:"bot5",author:"TropicalGrower",phase:1,week:4,env:"Estufa",light:"Sol Natural",substrate:"Solo",likes:31,comments:15,avatar:"🌺",cover:4,techniques:["HST"],weeks:[] },
  { id:"s6",name:"LED Coco Run",strain:"Girl Scout Cookies",authorId:"bot6",author:"CocoBR",phase:3,week:14,env:"Indoor",light:"LED",substrate:"Coco",likes:210,comments:78,avatar:"🍀",cover:5,techniques:["ScrOG","Desfolha","Topping"],weeks:[] },
];
...
function AuthScreen({ onLogin }) {
  ...
  const doSignup2=async()=>{
    ...
    try{
      ...
      if(data.user?.id){
        await sb.from("profiles").update({username:cleanUser,avatar:selectedAvatar,bio:sanitize(bio.trim(),200),city:sanitize(city.trim(),50)},`id=eq.${data.user.id}`);
  ...
  const wrap={minHeight:"100vh",background:"#f7f7f7",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"24px",position:"relative",overflow:"hidden"};
      ...
      <div style={{position:"relative",zIndex:1,width:"100%",maxWidth:"420px"}}>
        ...
        <div style={cardBase}>
          ...
          {step===1?(<>
            ...
            {error&&<div style={errorSt}>⚠️ {error}</div>}
            <div style={{marginBottom:"20px"}}><label style={labelSt}>Avatar</label><div style={{display:"flex",flexWrap:"wrap",gap:"8px"}}>{AVATARS.map(a=><button key={a} onClick={()=>setSelectedAvatar(a)} style={{width:"44px",height:"44px",borderRadius:"10px",border:selectedAvatar===a?`2px solid ${C.accent}`:`1px solid ${C.border}`,background:selectedAvatar===a?C.accentBg:C.surface2,fontSize:"22px",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",transition:"all 0.2s",transform:selectedAvatar===a?"scale(1.1)":"scale(1)"}}>{a}</button>)}</div></div>
...
function ProfilePage({ user, diaries, onUpdateUser, onLogout, onBack, blockedUsers, onUnblockUser, onDeleteAccount, onNavigate }) {
  ...
  const AvatarDisplay=({size,fontSize,editable})=>(
    <div style={{width:size+"px",height:size+"px",borderRadius:"50%",background:avatarImg?"transparent":C.accentBg,border:`3px solid ${C.accent}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:fontSize+"px",margin:"0 auto 4px",overflow:"hidden",position:"relative"}}>
      {avatarImg?<img src={avatarImg} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:
        ...
        <span>{user.avatarImg?<img src={user.avatarImg} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:user.avatar}</span>)}
    </div>
  );
  ...
  return (
    <div style={{maxWidth:"600px",margin:"0 auto",padding:"32px 24px"}}>
      ...
      {!editing&&<div style={{marginTop:"12px"}}>
        <button onClick={()=>setShowDeleteAccount(true)} style={{background:"none",border:"none",color:C.error,cursor:"pointer",fontSize:"12px",fontFamily:F.sans,padding:"4px 0",opacity:0.7}}>Excluir minha conta</button>
      </div>}
  ...
  return (
    <div style={{borderRadius:"14px",overflow:"hidden",border:`1px solid ${C.border}`,background:C.cardBg,transition:"all 0.3s",cursor:"pointer",transform:h?"translateY(-4px)":"none",boxShadow:h?"0 6px 24px rgba(0,0,0,0.1)":"0 1px 4px rgba(0,0,0,0.05)"}} onMouseEnter={()=>setH(true)} onMouseLeave={()=>setH(false)} onClick={onClick}>
      <div style={{height:"140px",background:COVER_GRADIENTS[(diary.cover||0)%6],display:"flex",alignItems:"center",justifyContent:"center",position:"relative",overflow:"hidden"}}>
        {diary.coverImage?<img src={diary.coverImage} alt="" onClick={e=>{e.stopPropagation();onViewImage?.(diary.coverImage);}} style={{width:"100%",height:"100%",objectFit:"cover",cursor:"zoom-in"}}/>:<div dangerouslySetInnerHTML={{__html:generatePlantArt(diary.id.charCodeAt(1)*7+(diary.id.charCodeAt(0)||1),100)}} style={{opacity:0.8}}/>}
      ...
      <div style={{padding:"14px 16px"}}>
        ...
        {diary.techniques?.length>0&&<div style={{display:"flex",gap:"4px",flexWrap:"wrap",marginBottom:"10px"}}>{diary.techniques.slice(0,4).map(t=><span key={t} style={{padding:"2px 8px",borderRadius:"6px",fontSize:"10px",background:C.accentBg,color:C.accent,fontFamily:F.sans,fontWeight:"500"}}>{t}</span>)}</div>}
...
function CreateDiaryModal({ user, onClose, onSave }) {
  ...
  const addStrain=()=>{if(strains.length<8)setStrains(p=>[...p,""]);};
  const updateStrain=(i,v)=>setStrains(p=>p.map((s,j)=>j===i?v:s));
  const removeStrain=i=>setStrains(p=>p.filter((_,j)=>j!==i));

  const handleSave=()=>{
    const validStrains=strains.map(s=>s.trim()).filter(Boolean);
    if(!name.trim()||validStrains.length===0)return;
    onSave({id:"u"+Date.now(),name:name.trim(),strain:validStrains.join(", "),strains:validStrains,author:user.username,authorId:user.email,phase:0,week:0,env,light,substrate,likes:0,comments:0,avatar:user.avatar,cover:Math.floor(Math.random()*6),techniques:techs,weeks:[],isOwn:true,watering,germination,numPlants:parseInt(numPlants)||1,watts:watts?parseInt(watts):null});
  };
      ...
      <div style={{background:C.surface,borderRadius:"16px",border:`1px solid ${C.border}`,width:"100%",maxWidth:"680px",overflow:"hidden"}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{padding:"20px"}}>
          ...
          <div style={{marginBottom:"28px"}}>
            ...
            <label style={labelSt}>Genética / Variedade <span style={{fontWeight:"400",textTransform:"none",letterSpacing:"0",fontSize:"11px",color:C.dim}}>({strains.length}/8)</span></label>
          ...
          {/* Number of plants */}
          <div style={{marginBottom:"28px",maxWidth:"200px"}}>
            <label style={labelSt}>Número de Plantas</label>
            <input style={baseInput} type="number" min="1" max="100" value={numPlants} onChange={e=>setNumPlants(e.target.value)} />
          </div>
...
function AddWeekModal({ diary, onClose, onSave, lang }) {
  const wn=(diary.weeks?.length||0)+1;
  const t=T[lang||"pt"];
      ...
      <div style={{background:C.surface,borderRadius:"16px",border:`1px solid ${C.border}`,width:"100%",maxWidth:"600px",overflow:"hidden"}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{padding:"20px 24px",borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          ...
          <div style={{fontFamily:F.sans,fontSize:"20px",fontWeight:"800",color:C.text,display:"flex",alignItems:"center",gap:"8px"}}>
            <span style={{fontSize:"22px"}}>📝</span> {t.week} {wn}
          </div>
        ...
        <div style={{padding:"24px"}}>
          ...
          <div style={{display:"flex",gap:"12px"}}>
            ...
            <button style={btnPrimary} onClick={()=>onSave({
              week:wn,phase,height:height||null,temp:temp||null,humidity:humidity||null,
...
function DiaryDetail({ diary, onBack, onUpdate, onRemove, onHide, lang, onLike, onFav, isLiked, isFaved, onViewImage, onReport, comments, onAddComment, onDeleteComment, onEditComment, blockedByOwner, onBlockUser, onUnblockUser, onReportUser, currentUserEmail, onAuthorClick }) {
  ...
  const deleteWeek=idx=>{const nw=[...(diary.weeks||[])];nw.splice(idx,1);const last=nw[nw.length-1];onUpdate({...diary,weeks:nw,week:last?last.week:0,phase:last?last.phase:0});setConfirmDeleteWeek(null);setExpandedWeek(null);};
    ...
    <div>
      <div style={{padding:"60px 24px 40px",textAlign:"center",position:"relative"}}>
        ...
        <div style={{position:"relative",zIndex:1}}>
          ...
          {/* Cover image / plant art — clickable to upload */}
          <div style={{position:"relative",display:"inline-block",margin:"0 auto 16px"}}>
            {diary.coverImage?(
              <div onClick={()=>onViewImage?.(diary.coverImage)} style={{width:"120px",height:"120px",borderRadius:"16px",overflow:"hidden",border:"3px solid rgba(255,255,255,0.8)",boxShadow:"0 2px 12px rgba(0,0,0,0.1)",margin:"0 auto",cursor:"pointer"}}>
                <img src={diary.coverImage} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>
              </div>
            ):(
            ...
            {diary.isOwn&&<div style={{display:"flex",gap:"4px",position:"absolute",bottom:diary.coverImage?"-8px":"-4px",right:diary.coverImage?"-12px":"-28px"}}>
          ...
          <div style={{display:"flex",justifyContent:"center",gap:"12px",marginTop:"16px",fontSize:"14px",fontFamily:F.sans,flexWrap:"wrap"}}>
            ...
            <span style={{display:"flex",alignItems:"center",gap:"4px",color:PHASE_COLORS[diary.phase],fontSize:"13px"}}>{PHASE_ICONS[diary.phase]} {PHASES[diary.phase]}</span>
            <span style={{display:"flex",alignItems:"center",color:C.dim,fontSize:"13px"}}>{t.week} {diary.week}</span>
          </div>
        </div>
      </div>
      ...
      <div style={{maxWidth:"700px",margin:"0 auto",padding:"0 24px"}}>
            ...
            <div key={i} style={{background:C.cardBg,borderRadius:"14px",border:`1px solid ${C.border}`,marginBottom:"16px",overflow:"hidden",boxShadow:"0 1px 3px rgba(0,0,0,0.04)"}}>
              ...
              <div onClick={()=>setExpandedWeek(isOpen?null:i)} style={{padding:"16px 20px",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",background:isOpen?C.surface2:C.cardBg,transition:"background 0.2s"}}>
                <div style={{display:"flex",alignItems:"center",gap:"12px"}}>
                  <div style={{fontFamily:F.sans,fontSize:"16px",fontWeight:"700",color:C.accent}}>{t.week} {w.week}</div>
        ...
        <div style={{marginBottom:"40px"}}>
          ...
          {/* Comment input */}
          {!blockedByOwner?<div style={{display:"flex",gap:"10px",marginBottom:"20px"}}>
            <input style={{...baseInput,borderRadius:"24px",padding:"12px 18px",flex:1}} value={commentText} onChange={e=>setCommentText(e.target.value)} placeholder="Escreva um comentário..." onKeyDown={e=>{if(e.key==="Enter"&&commentText.trim()){onAddComment?.(diary.id,commentText.trim());setCommentText("");}}}/>
            <button onClick={()=>{if(commentText.trim()){onAddComment?.(diary.id,commentText.trim());setCommentText("");}}} style={{width:"44px",height:"44px",borderRadius:"50%",border:"none",background:commentText.trim()?C.accent:C.surface2,color:commentText.trim()?C.onAccent:C.dim,cursor:"pointer",fontSize:"18px",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,transition:"all 0.2s"}}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M2.01 21L23 12 2.01 3 2 10l15 2-15 2z"/></svg>
            </button>
          </div>:<div style={{padding:"14px",borderRadius:"12px",background:C.errorBg,border:`1px solid ${C.error33}`,fontFamily:F.sans,fontSize:"13px",color:C.error,textAlign:"center",marginBottom:"20px"}}>🚫 Você foi bloqueado de comentar neste diário.</div>}

          {/* Comments list */}
          {(comments||[]).length>0?(comments||[]).map(c=>(
            <div key={c.id} style={{background:C.cardBg,borderRadius:"12px",border:`1px solid ${C.border}`,padding:"14px 16px",marginBottom:"10px"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:"10px"}}>
                <div style={{display:"flex",gap:"10px",alignItems:"flex-start",flex:1,minWidth:0}}>
                  <div onClick={()=>onAuthorClick?.(c.authorEmail)} style={{width:"32px",height:"32px",borderRadius:"50%",background:C.surface2,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"16px",border:`1px solid ${C.border}`,flexShrink:0,overflow:"hidden",cursor:"pointer"}}>{c.avatarImg?<img src={c.avatarImg} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:c.avatar||"🌿"}</div>
          ...
          )):<div style={{textAlign:"center",padding:"30px",color:C.dim,fontFamily:F.sans,fontSize:"14px"}}>Nenhum comentário ainda. Seja o primeiro!</div>}
        </div>
      </div>
      ...
      {/* Edit Diary Modal */}
      {showEdit&&<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.4)",backdropFilter:"blur(8px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:300,padding:"24px"}} onClick={()=>setShowEdit(false)}>
      ...
      {editingWeekIdx!==null&&(()=>{
        ...
        return <EditWeekModal week={ew} weekIdx={editingWeekIdx} onClose={()=>setEditingWeekIdx(null)} onSave={saveWeekEdit} lang={lang}/>;
      })()}
...
// ─── Edit Week Modal ───
function EditWeekModal({ week, weekIdx, onClose, onSave, lang }) {
  const t=T[lang||"pt"];
  ...
  return (
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.4)",backdropFilter:"blur(8px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:300,padding:"20px"}} onClick={onClose}>
      <div style={{...cardBase,maxWidth:"560px",maxHeight:"85vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"20px"}}>
          <h3 style={{fontFamily:F.sans,fontSize:"20px",fontWeight:"800",margin:0,display:"flex",alignItems:"center",gap:"8px"}}>✏️ Editar {t.week} {week.week}</h3>
        ...
        <div style={{display:"flex",gap:"12px"}}>
          <button style={{...btnSecondary,width:"auto",padding:"12px 24px"}} onClick={onClose}>{t.cancel}</button>
          <button style={btnPrimary} onClick={()=>onSave(weekIdx,{phase,height:height||null,temp:temp||null,humidity:humidity||null,ph:ph||null,waterMl:waterMl||null,lightHours:lightHours||null,note:note||""})}>{t.save}</button>
        </div>
      </div>
    </div>
  );
          ...
          return (<div key={m.id}>
            ...
            <div style={{display:"flex",justifyContent:isMe?"flex-end":"flex-start",alignItems:"flex-end",gap:"6px"}}>
              <div style={{maxWidth:"75%",padding:m.media?"6px":"10px 14px",borderRadius:isMe?"16px 16px 4px 16px":"16px 16px 16px 4px",background:isMe?C.accent:C.msgBubble,color:isMe?C.onAccent:C.text,fontFamily:F.sans,fontSize:"14px",lineHeight:"1.5",position:"relative",overflow:"hidden"}}>
                ...
                {m.media&&m.media.type==="image"&&<img src={m.media.data} alt="" style={{maxWidth:"100%",borderRadius:m.text?"10px 10px 0 0":"10px",display:"block",maxHeight:"240px",objectFit:"cover"}}/>}
...
function AdminPanel({ user, onBack }) {
  ...
  const [allComments,setAllComments]=useState([]); // [{diaryId, diaryName, ...comment}]
  ...
  useEffect(()=>{(async()=>{
    try{
      ...
      // Load all diaries
      const diaries=await sb.from("diaries").select("*,profiles(username,avatar)","&order=created_at.desc");
      const dm={};
      diaries.forEach(d=>{
        const uid=d.user_id;
        if(!dm[uid])dm[uid]=[];
        dm[uid].push({id:d.id,name:d.name,strain:d.strain,author:d.profiles?.username||"",avatar:d.profiles?.avatar||"🌱",phase:d.phase,week:d.current_week,env:d.environment,light:d.lighting,hidden:d.hidden,likes:d.likes_count,comments:d.comments_count,techniques:d.techniques||[],weeks:[]});
      });
      setAllDiariesMap(dm);

      // Load reports
      const reps=await sb.from("reports").select("*,reporter:reporter_id(username),target_user:target_user_id(username,email),target_diary:target_diary_id(name)","&order=created_at.desc");
      ...
      // Load audit log
      const al=await sb.from("audit_log").select("*,admin:admin_id(username,email)","&order=created_at.desc&limit=200");
      ...
      // Load all comments
      const ac=await sb.from("comments").select("id,text,created_at,user_id,diary_id,profiles(username,avatar,avatar_url),diaries(name)","&order=created_at.desc&limit=500");
  ...
  const tabBtn=(id,icon,label,badge)=>(
    <button onClick={()=>setTab(id)} style={{padding:"10px 18px",borderRadius:"10px",border:tab===id?`2px solid ${C.accent}`:`1px solid ${C.border}`,background:tab===id?C.accentBg:C.cardBg,color:tab===id?C.accent:C.muted,cursor:"pointer",fontSize:"12px",fontFamily:F.sans,fontWeight:"600",display:"flex",alignItems:"center",gap:"6px",position:"relative"}}>{icon} {label}{badge>0&&<span style={{minWidth:"18px",height:"18px",borderRadius:"9px",background:C.error,color:C.onAccent,fontSize:"10px",fontWeight:"700",display:"flex",alignItems:"center",justifyContent:"center",padding:"0 4px"}}>{badge}</span>}</button>
  );
  ...
  return (
    <div style={{maxWidth:"900px",margin:"0 auto",padding:"24px"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(100px,1fr))",gap:"8px",marginBottom:"20px"}}>
        {[["👥",userList.length,"Usuários"],["📓",allDiariesFlat.length,"Diários"],["🚫",userList.filter(u=>u.banned).length,"Banidos"],["👑",userList.filter(u=>u.role==="admin").length,"Admins"],["🚨",pendingReports,"Denúncias"]].map(([icon,val,label])=>(
          <div key={label} style={{background:C.cardBg,borderRadius:"10px",border:`1px solid ${C.border}`,padding:"12px",textAlign:"center"}}>
            <div style={{fontSize:"16px"}}>{icon}</div>
            <div style={{fontFamily:F.sans,fontSize:"18px",fontWeight:"700",color:C.accent}}>{val}</div>
            <div style={{fontFamily:F.sans,fontSize:"9px",color:C.dim,textTransform:"uppercase",letterSpacing:"0.8px"}}>{label}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{display:"flex",gap:"6px",marginBottom:"20px",flexWrap:"wrap"}}>
        {tabBtn("users","👥","Usuários")}{tabBtn("diaries","📓","Diários")}{tabBtn("comments","💬","Comentários")}{tabBtn("reports","🚨","Denúncias",pendingReports)}{tabBtn("announce","📢","Anúncios")}{tabBtn("warnings","⚠️","Avisos")}{tabBtn("log","📋","Auditoria")}
      </div>
      ...
      {tab==="users"&&<div>
        ...
        {filteredUsers.map(u=>(
          <div key={u.email} style={{background:C.cardBg,borderRadius:"12px",border:`1px solid ${u.banned?C.error+"44":C.border}`,padding:"12px 14px",marginBottom:"8px",display:"flex",alignItems:"center",gap:"10px",flexWrap:"wrap",opacity:u.banned?0.6:1}}>
            <div style={{width:"36px",height:"36px",borderRadius:"50%",background:C.surface2,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"18px",border:`1px solid ${C.border}`,overflow:"hidden",flexShrink:0}}>{u.avatarImg?<img src={u.avatarImg} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:u.avatar}</div>
      ...
      {tab==="comments"&&<div>
        ...
        {(searchComments?allComments.filter(c=>(c.text+" "+c.username+" "+c.diaryName).toLowerCase().includes(searchComments.toLowerCase())):allComments).map(c=>(
          <div key={c.id} style={{background:C.cardBg,borderRadius:"10px",border:`1px solid ${C.border}`,padding:"12px 14px",marginBottom:"6px",display:"flex",gap:"10px",alignItems:"flex-start"}}>
            <div style={{width:"30px",height:"30px",borderRadius:"50%",background:C.surface2,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"14px",border:`1px solid ${C.border}`,flexShrink:0,overflow:"hidden"}}>{c.avatarImg?<img src={c.avatarImg} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:c.avatar||"🌿"}</div>
            <div style={{flex:1,minWidth:0}}>
      ...
      {tab==="warnings"&&<div>
        ...
        <div style={{background:C.cardBg,borderRadius:"12px",border:`1px solid ${C.border}`,padding:"20px"}}>
          <div style={{marginBottom:"14px"}}><label style={labelSt}>Usuário</label>
            <select style={{...baseInput,cursor:"pointer"}} value={warnTarget||""} onChange={e=>setWarnTarget(e.target.value||null)}>
...
function NavBar({ user, page, setPage, setShowCreate, myDiaries, onLogout, onNavigate, lang, setLang, unreadNotifs, unreadMsgs, notifs, onMarkNotifsRead, dark, onToggleDark }) {
  ...
  const sidebarItems=[
    {icon:"➕",label:t.startDiary,action:()=>{setShowSidebar(false);setShowCreate(true);}},
    {icon:"🏠",label:t.home,action:()=>nav("home"),active:page==="home"},
    {icon:"⭐",label:t.feed,action:()=>nav("home")},
    {icon:"🔍",label:t.diaries,action:()=>nav("explorar"),active:page==="explorar"},
    {icon:"🌿",label:t.strains,action:()=>nav("explorar"),badge:"NEW"},
    {icon:"🎬",label:t.shorts,action:()=>nav("explorar")},
    {icon:"❓",label:t.questions,action:()=>nav("explorar")},
    {icon:"🏆",label:t.contests,action:()=>nav("explorar")},
    {icon:"👥",label:t.growers,action:()=>nav("explorar"),active:false},
    {icon:"🌱",label:t.seeds,action:()=>nav("explorar")},
    {icon:"🧬",label:t.breeders,action:()=>nav("explorar")},
    {icon:"💧",label:t.nutrients,action:()=>nav("explorar")},
    {icon:"🚜",label:t.equipment,action:()=>nav("explorar")},
    {icon:"💬",label:t.blog,action:()=>nav("explorar")},
  ];
  ...
  return (
    <>
      ...
      `}</style>
      <nav style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 16px",borderBottom:`1px solid ${C.border}`,backdropFilter:"blur(20px)",background:"rgba(255,255,255,0.95)",position:"sticky",top:0,zIndex:100,gap:"8px"}}>
        {/* Left: hamburger + logo */}
        ...
        <div style={{display:"flex",gap:"2px",alignItems:"center",flexShrink:0}}>
          ...
          <div ref={notifRef} style={{position:"relative"}}>
            <button onClick={()=>{setShowNotifs(!showNotifs);if(!showNotifs)onMarkNotifsRead?.();}} style={{width:"36px",height:"36px",borderRadius:"50%",border:"none",background:showNotifs?C.surface2:"transparent",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",position:"relative",transition:"all 0.2s"}}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#666" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg>
          ...
          <div ref={ref} style={{position:"relative"}}>
            <button onClick={()=>{setShowMenu(!showMenu);setShowLangSub(false);}} style={{width:"36px",height:"36px",borderRadius:"50%",border:`2px solid ${showMenu?C.accent:C.border}`,background:showMenu?C.accentBg:C.surface2,fontSize:"18px",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",transition:"all 0.2s",overflow:"hidden",padding:0}}>{user.avatarImg?<img src={user.avatarImg} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:user.avatar}</button>
            ...
            {showMenu&&<div style={{position:"absolute",top:"46px",right:0,background:C.cardBg,borderRadius:"14px",border:`1px solid ${C.border}`,padding:"8px",minWidth:"240px",boxShadow:"0 8px 30px rgba(0,0,0,0.12)",zIndex:110}}>
              ...
              <div style={{padding:"12px 16px",display:"flex",alignItems:"center",gap:"12px",borderBottom:`1px solid ${C.border}`,marginBottom:"6px",paddingBottom:"14px"}}>
                <div style={{width:"44px",height:"44px",borderRadius:"50%",background:C.surface2,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"24px",border:`2px solid ${C.border}`,overflow:"hidden"}}>{user.avatarImg?<img src={user.avatarImg} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:user.avatar}</div>
                <div>
...
function AppInner() {
  ...
  useEffect(()=>{if(!user)return;(async()=>{try{
    ...
    setNotifs(n.map(x=>({id:x.id,type:x.type,from:x.from_username,avatar:x.from_avatar,text:x.text,diary:x.diary_name,time:new Date(x.created_at).getTime(),read:x.read})));
  ...
  useEffect(()=>{(async()=>{
    ...
    try{
      ...
      if(authUser?.id){
        ...
        if(profile&&!profile.banned) setUser({...profile,authId:authUser.id});
      }
    }catch{}
  ...
  useEffect(()=>{if(!user)return;(async()=>{try{
    ...
    const mapped=rows.map(d=>({
      id:d.id,name:d.name,strain:d.strain,strains:d.strains||[],author:user.username,authorId:user.id,avatar:user.avatar,
      phase:d.phase,week:d.current_week,env:d.environment,light:d.lighting,watts:d.watts,substrate:d.substrate,
      watering:d.watering,germination:d.germination,techniques:d.techniques||[],numPlants:d.num_plants,
  ...
  const doCreateDiary=async(d)=>{
    if(!user)return;
    try{
      const row=await sb.from("diaries").insert({user_id:user.id,name:sanitize(d.name,100),strain:sanitize(d.strain,100),strains:(d.strains||[]).map(s=>sanitize(s,100)),environment:d.env,lighting:d.light,watts:d.watts?parseInt(d.watts):null,substrate:d.substrate,watering:d.watering,germination:d.germination,techniques:d.techniques||[],num_plants:d.numPlants||1});
      const mapped={...d,id:row.id,authorId:user.id,author:user.username,avatar:user.avatar,likes:0,comments:0,phase:0,week:0,isOwn:true,weeks:[]};
  ...
  const doUpdateDiary=async(updated)=>{
    ...
    try{
      ...
      // Delete weeks removed from local state
      const localIds=weeks.filter(w=>isUUID(w.id)).map(w=>w.id);
      const dbWeeks=await sb.from("weeks").select("id",`&diary_id=eq.${updated.id}`);
      for(const dbw of dbWeeks){
        if(!localIds.includes(dbw.id)) await sb.from("weeks").delete(`id=eq.${dbw.id}`);
      }
  ...
  useEffect(()=>{if(!user)return;(async()=>{try{
    const lk=await sb.from("likes").select("diary_id",`&user_id=eq.${user.id}`);
    const lm={};lk.forEach(l=>lm[l.diary_id]=true);setLikes(lm);
    const fv=await sb.from("favorites").select("diary_id",`&user_id=eq.${user.id}`);
  ...
  const loadComments=async(diaryId)=>{
    try{
      const rows=await sb.from("comments").select("id,text,edited_at,created_at,user_id,profiles(username,avatar,avatar_url)",`&diary_id=eq.${diaryId}&order=created_at.desc`);
      const mapped=rows.map(c=>({id:c.id,text:c.text,username:c.profiles?.username,avatar:c.profiles?.avatar,avatarImg:c.profiles?.avatar_url,authorEmail:c.user_id,time:new Date(c.created_at).getTime(),editedAt:c.edited_at?new Date(c.edited_at).getTime():null}));
      setCommentsMap(p=>({...p,[diaryId]:mapped}));
    }catch{}
  };

  const doAddComment=async(diaryId,text)=>{
    if(!user||!text.trim())return;
    const clean=sanitize(text);
    try{
      await sb.from("comments").insert({diary_id:diaryId,user_id:user.id,text:clean});
      ...
      if(diary?.authorId&&diary.authorId!==user.id){
        await sb.from("notifications").insert({user_id:diary.authorId,type:"comment",from_username:user.username,from_avatar:user.avatar,text:"comentou no seu diário",diary_name:diary.name});
      }
    }catch{}
  };
  ...
  const openPublicProfile=async(targetId)=>{
    ...
    try{
      ...
      const mapped=diaries.map(d=>({id:d.id,name:d.name,strain:d.strain,strains:d.strains||[],author:target.username,authorId:target.id,avatar:target.avatar,phase:d.phase,week:d.current_week,env:d.environment,light:d.lighting,likes:d.likes_count,comments:d.comments_count,cover:0,coverImage:d.cover_url,hidden:false,techniques:d.techniques||[],weeks:[]}));
  ...
  const renderPage=()=>{
    ...
    if(page==="admin"&&user.role==="admin") return <AdminPanel user={user} onBack={()=>setPage("home")}/>;
    if(selectedDiary){
  ...
  const allDiaries=page==="meus"?myDiaries.filter(d=>!d.hidden):page==="favoritos"?allPool.filter(d=>favs[d.id]):page==="gostei"?allPool.filter(d=>likes[d.id]):allPool;
  const envIds=ENVIRONMENTS.map(e=>e.id);
  // Search filter
  const searchFiltered=searchQ?allDiaries.filter(d=>(d.name+" "+d.strain+" "+d.author+(d.strains?d.strains.join(" "):"")).toLowerCase().includes(searchQ.toLowerCase())):allDiaries;
  ...
  const pageTitle=page==="meus"?t.myDiaries:page==="explorar"?t.exploreDiaries:page==="favoritos"?"⭐ "+t.favorites:page==="gostei"?"❤️ "+t.liked:t.recentDiaries;
  const pageSub=page==="meus"?t.manageGrows:page==="favoritos"||page==="gostei"?"":t.followGrowers;
      ...
      <div className="dp-section" style={{padding:"32px 20px",maxWidth:"1100px",margin:"0 auto"}}>
        ...
        {filtered.length>0?(
          ...
          <div style={{textAlign:"center",padding:"60px 24px",color:C.dim}}>
            ...
            <p style={{fontFamily:F.body,fontSize:"16px",marginBottom:"16px"}}>{page==="meus"?t.noDiaries:page==="favoritos"?"Nenhum diário favoritado ainda.":page==="gostei"?"Nenhum diário curtido ainda.":t.noResults}</p>
